...
本文は選び方中心にし、更新で壊れる固定数字を避ける。**数値の真実は frontmatter `products[]` の1か所だけ**で、比較・ランキング・単価は frontmatter から生成される上部比較表（`ComparisonTable`、単価は `calcPricePerUnit` で表示時再計算）に委ねる。

- 価格、レビュー件数、ランキング順位、年間コスト、特定商品の単価を本文に固定しない
- **価格・単価を並べた手書き表（`| … 円 |` / `円/mL` 等のテーブル）を本文に作らない**。具体数値が要るときは「ページ上部の比較表（自動更新）でご確認ください」と誘導する
- frontmatter の `faqs[].answer` にも商品別の固定数値（`約0.67円/mL` 等）を書かない。定性表現（「業務用大容量タイプほど単価が安い傾向」）にする
- 許容: 「30〜50% 安くなる傾向」「未開封で約3年」など特定商品に紐づかない一般目安、比較日・対象範囲・出典を伴う断定（景表法対応）
- 商品名の列挙は必要最小限にする。ランキングや比較は frontmatter の比較表に任せる
- **定型H2（「○○の選び方ガイド」「タイプ別の特徴と使い分け」「コスパ比較のポイント」）を使わない。** 全記事同型の見出しは AdSense「有用性の低いコンテンツ」の不合格要因で、2026-09 に既存記事から順次撤去した（凍結中の残りも解除後に是正する）。見出しは記事固有の判断軸にする（例:「どの容量を買うかを『使い切れる量』から逆算する」）。上部比較表が答えられないこと（場面別の選び分け）＋「この記事が向いていない人」の小節を必ず入れる
- 本文に書く商品・ブランド名は `products[]` に実在するものだけにする（存在しない銘柄への言及は区分C の誤表示として後で是正対象になる）
- FAQは3〜4問。医薬品的な効果効能、過度な断定、根拠のない推奨を避ける
- 検収: 本文・`faqs[].answer` に `scripts/lib/article-body-lint.ts` が検出する禁止数値（手書き価格表・`円/単位`・商品価格）が無いこと。`pnpm test`（`tests/article-body-lint.test.ts`）で自動チェックされる
...
```markdown
## [記事固有の判断軸を表す見出し]（例: 「どの容量を買うかを『使い切れる量』から逆算する」）
### [商材に合った観点 2〜4個]

## [上部比較表では答えられない場面別の選び分け]

## この記事が向いていない人

## 関連記事への導線（本文中の文脈リンク2〜4本。見出しは任意）
...
- 定型H2（選び方ガイド／タイプ別の特徴と使い分け／コスパ比較のポイント）が本文に無い
- **内部リンク**: 新記事の本文から関連記事へ2〜4本、かつ**既存記事から新記事へ最低1本**張る（被リンク0の孤立ページは表示すら立たない。deodorizing-bag は内部リンク0本でクロールが3週間止まり未離陸だった）。`pnpm check-internal-links` で孤立していないことを確認する
- **リンク先・リンク元の記事が凍結中でないか** `data/measurement-holds.json` と突き合わせる。凍結記事の本文を触らなくても、被リンクを足せばその記事の測定条件が変わる（2026-09 に凍結中の wet-tissue / floor-cleaner へ張ってしまい差し替えた）

## 公開後

- **公開した日に `data/measurement-holds.json` とメモリ `project_measurement_holds` の両方へ記帳する**（起点日＝公開日・解除日・指標・`searchNeed`。`releaseDate` は解除日の翌日）。kireikirei-vs-biore は記帳漏れで遡及登録になった
- **判定は公開2週後の順位帯で行う。** インデックス3〜5日・順位帯は約2週で確定し以後ほぼ動かない（「2〜3ヶ月かかる」は誤り）。クリック実数が一桁なら CTR ではなく順位帯で見る
- 一般名詞の新規カテゴリ記事は表示が立ちにくい。指名クエリの受け皿が実測できているジャンル（vs 記事の採択基準は `kura-article-recommend` 参照）のほうが打率が高い
