...
- 商品を削除したら `title` / `description` / 本文内の件数表現も合わせる。
- `title` や `description` の価格訴求は、残った `products[]` の実値と一致させる。
  - 例: タイトルに「1枚あたり最安2円台」とあるなら、記事内に `pricePerUnit` が2円台の商品が必要。
  - 該当商品がなくなった場合は「1枚あたりで比較」「まとめ買いで比較」など、断定的な最安価格を外す。
  - 「最安」「No.1」「2円台」などの具体訴求は、残存商品と比較表・本文の両方で矛盾がないか確認する。
...
- JSONL各件の分類
- 記事から削除/修正した商品
- タイトル/description/本文の価格・件数表現を見直したか
- 追加・修正した抽出ロジック
