...
**本文の手書き数値・価格表（単一ソース原則）**
- 本文（frontmatter を除く Markdown）と `faqs[].answer` に、商品別の固定数値や手書き価格表が無いか確認する。
- 検出は `scripts/lib/article-body-lint.ts` の `lintArticleBody(content)` を使う（`pnpm test` の `tests/article-body-lint.test.ts` と同じ判定）。
  - `unit-price`（`円/mL` 等）・`price-table`（`| … 円 |`）= `warning`（更新で陳腐化・景表法リスク）。定性表現へ置換し、具体数値は上部自動比較表へ誘導する。
  - `price`（3桁以上+円）= `info`（一般記述の誤検知に注意。商品別の固定価格なら修正候補）。
- 許容: 「30〜50%」「約3年」等の特定商品非依存の一般目安、比較日・出典を伴う断定。
...
**SEO 勝ちトークン（title / description）**
- `title` に検索需要の大きい勝ちトークンが含まれるか確認する。GSC実データで、これらを欠く「○○比較」だけの title は上位表示でも CTR ほぼ0% だった。
  - グループA（おすすめ系）: 「おすすめ」かつ件数表現（「○選」等）
  - グループB（最強・ランキング系）: 「コスパ最強」「ランキング」「最強」のいずれか
- AとB両方を満たすのが実証済みの強パターン（例: `{カテゴリ} おすすめ{N}選【{年}版】コスパ最強を{単位}単価比較`）。dental-floss / body-soap が該当しサイト内最多クリック。
- A・Bのどちらかのグループが1語も無ければ `info` として改善候補に挙げる（両方欠ける場合は優先度を上げて報告）。
- `description` 冒頭にも結論（「{カテゴリ}のおすすめ{N}選を…比較」）が入っているか軽く確認する。
- 注意: 直近で title を変更した記事は効果測定中の可能性がある。再変更提案は測定期間（変更から1〜2週間）を踏まえ、measurement hygiene に配慮して報告する。

## 問題の分類

- `blocking`: ビルドが失敗する可能性（URL形式違反、スキーマ文字数超過）
- `warning`: 品質・法務リスク（pricePerUnit要更新、根拠なし断定表現、アフィリエイトURL以外）
- `info`: 軽微・任意対応（価格の最新化推奨、表現の改善提案、SEO勝ちトークン不足）
