...
describe("extractCapacityFromItemName", () => {
  ...
  // ─── 販促枠の除去 ───────────────────────────────────────────────────
  it("販促枠の日付を容量として拾わない", () => {
    // 「日」は液体蚊取り向けの単位。"5月15日限定" の "15日" を容量にしていた
    expect(
      extractCapacityFromItemName(
        "【5月15日限定最大100%ポイントバック】エコメイト 風呂釜クリーナー [3回分] ECOMATE"
      )
    ).toBe("3回");
  });
  ...
  it("販促枠の個数を容量として拾わない", () => {
    expect(
      extractCapacityFromItemName(
        "【送料お得・まとめ買い×9個セット】あわだたない こどもハミガキ ぶどう香味 60g エスケー石鹸"
      )
    ).toBe("60g");
  });

  it("販促枠に総量の乗算チェーンがある場合は残す", () => {
    // 枠を消すと本体の "36枚" しか残らず総量を取りこぼす
    expect(
      extractCapacityFromItemName(
        "【送料込・まとめ買い36枚 ×6個セット】P&G パンパース はじめての肌へのいちばん テープ 3S 36枚"
      )
    ).toBe("36枚×6個");
  });

  // ─── パターン1g: "N単位入 M袋/箱/個" ────────────────────────────────
  it("「入」を挟むスペース区切りの入数を乗算する", () => {
    expect(
      extractCapacityFromItemName(
        "ライフリー おしりふき Pro やわらか厚手 超大判 30枚入 12袋 大人用 介護用"
      )
    ).toBe("30枚×12袋");
  });
  ...
  it("「入り」＋袋セット表記を乗算する", () => {
    expect(
      extractCapacityFromItemName(
        "finish ビッグパック 大容量 150個入り 3袋セット フィニッシュ 食洗機用洗剤"
      )
    ).toBe("150個×3袋");
  });

  it("入数と箱数のあいだの括弧注釈を読み飛ばす", () => {
    expect(
      extractCapacityFromItemName(
        "ニッシン フィジオクリーン キラリ錠剤 30錠入(3g×30) 3箱 リニューアル品 義歯洗浄剤"
      )
    ).toBe("30錠×3箱");
  });

  it("括弧内の「N枚入*M袋セット」を乗算する", () => {
    expect(
      extractCapacityFromItemName(
        "ソフィ はだおもい オーガニックコットン 特に多い昼用 230 羽つき 15枚 生理用品(15枚入*3袋セット)"
      )
    ).toBe("15枚×3袋");
  });

  it("「入」が無い数量の並記は乗算しない（サイズ選択の誤認防止）", () => {
    // "80個 90個" はサイズ選択の並記であり 80×90 ではない
    expect(
      extractCapacityFromItemName(
        "ティッシュ ソフトパックティッシュ 大容量 80個 90個 nepia 200組 (400枚) 備蓄 防災"
      )
    ).toBe("（400枚）");
  });

  it("1錠あたりの重量を総量として拾わない", () => {
    // "2.8g×90錠×2個" の 2.8g は1錠あたりの重量。総量は 90錠×2個=180錠
    expect(
      extractCapacityFromItemName(
        "SANWA DENTAL スマイルクリーナータブレット 2.8g×90錠×2個セット 中性 入れ歯洗浄剤"
      )
    ).toBe("90錠×2個");
  });

  it("剤形単位でなければ先頭の実容量を保持する", () => {
    // "3.8L×3本" の 3.8L は実容量なので落としてはいけない
    expect(extractCapacityFromItemName("CHARMY Magica 無香料 3.8L×3本")).toBe("3.8L×3本");
  });

  it("先頭の集合単位が内訳と一致する場合は内訳を落とす", () => {
    // "60箱 (5箱×12パック)" は 60箱。60×5×12=3600箱 ではない
    expect(
      extractCapacityFromItemName(
        "ティッシュペーパー 60箱 (5箱×12パック) 150組 日本製 イットコティッシュ イトマン 業務用"
      )
    ).toBe("60箱");
  });
