---
title: "洗顔料 コスパ最強ランキング【2026年版】1gあたり最安で比較"
description: "洗顔料を1gあたりのコストで徹底比較。ビオレ・ロゼット・ダヴなど人気10選を成分・洗い上がり・コスパで解説。泡タイプとパウダーで単価を比べられない理由も整理します。"
category: "face-wash"
publishedAt: "2026-05-01"
articleType: "comparison"
updatedAt: "2026-09-19"
products:
  - rank: 1
    name: "洗顔フォーム 毛穴 ロゼット洗顔パスタ 海泥スムース WEB限定2倍"
    brand: "ロゼット"
    price: 1210
    capacity: "240g"
    pricePerUnit: "約5.0円/g"
    rating: 4.72
    reviewCount: 82
    features:
      - "海泥（かいでい）が毛穴汚れを吸着除去"
      - "きめ細かいクリーミーな泡立ち"
      - "イオウ成分配合で皮脂をすっきりオフ"
    pros:
      - "1本あたり約550円で圧倒的コスパ"
      - "毛穴の黒ずみ・ザラつきに効果的"
      - "ドラッグストアで手軽に購入可能"
    cons:
      - "乾燥肌にはやや洗浄力が強め"
      - "チューブの口が大きく出しすぎやすい"
    recommendedFor: "毛穴汚れが気になる方・コスパ最優先の方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00u529n.3rdw64b3.g00u529n.3rdw7138/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Frosette-shop%2Fpasta_ks240_1ps%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Frosette-shop%2Fi%2F10000066%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/rosette-shop/cabinet/thumbna/pasta_ks240_1ps/imgrc0108556464.jpg?_ex=128x128"
    genreId: "405130"
  - rank: 2
    name: "ダヴ ビューティモイスチャー 洗顔料 130g"
    brand: "ユニリーバ（ダヴ）"
    price: 1577
    capacity: "130g"
    pricePerUnit: "約12円/g"
    rating: 4.64
    reviewCount: 14
    features:
      - "ニュートリアムモイスチャー配合で洗い上がりしっとり"
      - "マイルドな洗浄成分で肌に負担が少ない"
      - "クリーミーな泡が肌を包み込むように洗う"
    pros:
      - "洗い上がりのつっぱり感がほとんどない"
      - "敏感肌でも使いやすいマイルド処方"
      - "大手ブランドの安心感"
    cons:
      - "さっぱり感を求める方には不向き"
      - "毛穴汚れへの洗浄力はやや控えめ"
    recommendedFor: "敏感肌・乾燥肌の方・マイルドな洗い心地を好む方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00r136n.3rdw697f.g00r136n.3rdw7383/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Frakuten24%2F563823%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Frakuten24%2Fi%2F11186459%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/rakuten24/cabinet/823/563823.jpg?_ex=128x128"
    offers:
      - provider: "yahoo"
        label: "Yahoo!"
        url: "https://ck.jp.ap.valuecommerce.com/servlet/referral?sid=3770852&pid=892615315&vc_url=https%3A%2F%2Fstore.shopping.yahoo.co.jp%2Fsundrugec%2F4902111736655.html"
        matchStatus: "matched"
        updatedAt: "2026-09-07"
        price: 348
        imageUrl: "https://item-shopping.c.yimg.jp/i/j/sundrugec_4902111736655"
        available: true
        rating: 4.5
        reviewCount: 8
    genreId: "100627"
  - rank: 3
    name: "DoMeCare マイクロクレイウォッシュ 110g"
    brand: "DoMeCare"
    price: 1980
    capacity: "110g"
    pricePerUnit: "約18円/g"
    rating: 4.61
    reviewCount: 1999
    features:
      - "顔の洗浄に使う洗顔料"
      - "継続利用しやすい定番構成"
      - "楽天市場で購入できる商品"
    pros:
      - "肌質や洗い上がりで選びやすい"
      - "買い足し候補として比較しやすい"
      - "既存商品と単価で比べやすい"
    cons:
      - "使用感や香りの好みを確認したい"
      - "購入時は最新の販売条件を確認したい"
    recommendedFor: "肌質や洗い上がりで選びたい方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00tfrwn.3rdw669e.g00tfrwn.3rdw74a4/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Fdomecare%2Fdmc_micro-cray-wash%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Fdomecare%2Fi%2F10000008%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/domecare/cabinet/campain1/50per/mcwsamnew_50per.jpg?_ex=128x128"
    offers:
      - provider: "yahoo"
        label: "Yahoo!"
        url: "https://ck.jp.ap.valuecommerce.com/servlet/referral?sid=3770852&pid=892615315&vc_url=https%3A%2F%2Fstore.shopping.yahoo.co.jp%2Fyua-shop%2Fl01-045.html"
        matchStatus: "matched"
        updatedAt: "2026-09-19"
        price: 2500
        imageUrl: "https://item-shopping.c.yimg.jp/i/j/yua-shop_l01-045"
        available: true
        rating: 0
        reviewCount: 0
    genreId: "405130"
  - rank: 4
    name: "レステモ フェイシャル ソープ 150g"
    brand: "レステモ"
    price: 2680
    capacity: "150g"
    pricePerUnit: "約18円/g"
    rating: 4.61
    reviewCount: 3558
    features:
      - "顔の洗浄に使う洗顔料"
      - "継続利用しやすい定番構成"
      - "楽天市場で購入できる商品"
    pros:
      - "肌質や洗い上がりで選びやすい"
      - "買い足し候補として比較しやすい"
      - "既存商品と単価で比べやすい"
    cons:
      - "使用感や香りの好みを確認したい"
      - "購入時は最新の販売条件を確認したい"
    recommendedFor: "肌質や洗い上がりで選びたい方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00qdaen.3rdw686c.g00qdaen.3rdw7002/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Flesthemo%2Fsoap150g%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Flesthemo%2Fi%2F10000018%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/lesthemo/cabinet/shg/soap_150_2500px_05.jpg?_ex=128x128"
    offers:
      - provider: "yahoo"
        label: "Yahoo!"
        url: "https://ck.jp.ap.valuecommerce.com/servlet/referral?sid=3770852&pid=892615315&vc_url=https%3A%2F%2Fstore.shopping.yahoo.co.jp%2Flesthemo%2Fsoap150g.html"
        matchStatus: "pending"
        updatedAt: "2026-09-19"
        price: 2680
        imageUrl: "https://item-shopping.c.yimg.jp/i/j/lesthemo_soap150g"
        available: true
        rating: 4.68
        reviewCount: 214
    genreId: "405130"
  - rank: 5
    name: "ルアド 洗顔パウダー 67g×3袋"
    brand: "ルアド"
    price: 4125
    capacity: "67g×3袋"
    pricePerUnit: "約21円/g"
    rating: 4.83
    reviewCount: 5619
    features:
      - "顔の洗浄に使う洗顔料"
      - "継続利用しやすい定番構成"
      - "楽天市場で購入できる商品"
    pros:
      - "肌質や洗い上がりで選びやすい"
      - "買い足し候補として比較しやすい"
      - "既存商品と単価で比べやすい"
    cons:
      - "使用感や香りの好みを確認したい"
      - "購入時は最新の販売条件を確認したい"
    recommendedFor: "肌質や洗い上がりで選びたい方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00pytjn.3rdw6c43.g00pytjn.3rdw73fd/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Fadpentel%2F590765%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Fadpentel%2Fi%2F10000082%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/adpentel/cabinet/pro/imgrc0079724986.jpg?_ex=128x128"
    genreId: "405183"
  - rank: 6
    name: "カーボニック クリーニング パック 150g×2点セット"
    brand: "PLuS"
    price: 6280
    capacity: "150g×2点（300g）"
    pricePerUnit: "約21円/g"
    rating: 4.45
    reviewCount: 3814
    features:
      - "炭酸泡タイプの洗顔・パック兼用アイテム"
      - "コラーゲン・ヒアルロン酸など複数の保湿成分を配合"
      - "朝晩どちらでも使える設計"
    pros:
      - "洗顔とパックを1本で兼ねられる"
      - "2点セットでストックしながら使いやすい"
      - "保湿成分配合で洗い上がりのつっぱりを抑えやすい"
    cons:
      - "パック用途も兼ねるため通常の洗顔料より価格は高め"
      - "炭酸の刺激が苦手な人には不向き"
    recommendedFor: "洗顔とパックを一本で済ませたい方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00po0zn.3rdw627e.g00po0zn.3rdw7621/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Fluire%2Fpscp11000%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Fluire%2Fi%2F10005954%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/luire/cabinet/plus04/clpack-2_01.jpg?_ex=128x128"
    genreId: "405130"
  - rank: 7
    name: "ファンケル ピュアモイスト泡洗顔料 150ml"
    brand: "ファンケル"
    price: 1320
    capacity: "150ml"
    pricePerUnit: "約8.8円/ml"
    rating: 4.4
    reviewCount: 5
    features:
      - "無添加処方でセラミド・アミノ酸配合の泡洗顔料"
      - "敏感肌・乾燥肌に向けた低刺激設計"
      - "約60日分の使用量を1本で賄える設計"
    pros:
      - "無添加処方で肌刺激を抑えたい方に向く"
      - "セラミド配合で洗後の乾燥を感じにくい"
      - "ファンケル公式ショップから直接購入しやすい"
    cons:
      - "一般的な洗顔フォームより価格は高め"
      - "泡立てずに使えるタイプではないためひと手間かかる"
    recommendedFor: "無添加・セラミド配合で敏感肌や乾燥肌のケアをしたい方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00r136n.3rdw697f.g00r136n.3rdw7383/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Frakuten24%2F4908049579715%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Frakuten24%2Fi%2F11103692%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/rakuten24/cabinet/715/4908049579715.jpg?_ex=128x128"
    offers:
      - provider: "yahoo"
        label: "Yahoo!"
        url: "https://ck.jp.ap.valuecommerce.com/servlet/referral?sid=3770852&pid=892615315&vc_url=https%3A%2F%2Fstore.shopping.yahoo.co.jp%2Fcosmecom%2F1000154951.html"
        matchStatus: "matched"
        updatedAt: "2026-08-19"
        price: 1320
        rating: 0
        reviewCount: 0
        imageUrl: "https://item-shopping.c.yimg.jp/i/j/cosmecom_1000154951"
        available: true
    genreId: "100627"
  - rank: 8
    name: "ビオレ おうちdeエステ マッサージ洗顔ジェル 150g"
    brand: "花王（ビオレ）"
    price: 1070
    capacity: "150g"
    rating: 4.5
    reviewCount: 2
    features:
      - "マッサージしながら洗顔できるジェルタイプの洗顔料"
      - "肌をやわらかくする保湿ケア成分配合"
      - "リラックスアロマの香り付き"
    pros:
      - "ドラッグストアで入手しやすい定番ブランド"
      - "洗顔とマッサージを同時にできる時短設計"
      - "毛穴ケアと保湿を両立したい方に向く"
    cons:
      - "アロマ香料が苦手な方には向かない場合がある"
      - "ジェルタイプのため泡立てが必要"
    recommendedFor: "洗顔にマッサージ感も求める方・花王ビオレブランドを好む方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00upwzn.3rdw6b3d.g00upwzn.3rdw7ca6/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Fessence-of-life%2F4901301342904%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Fessence-of-life%2Fi%2F10000613%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/essence-of-life/cabinet/13169583/106.jpg?_ex=128x128"
    offers:
      - provider: "yahoo"
        label: "Yahoo!"
        url: "https://ck.jp.ap.valuecommerce.com/servlet/referral?sid=3770852&pid=892615315&vc_url=https%3A%2F%2Fstore.shopping.yahoo.co.jp%2Fshop-ypp%2F3799-004426.html"
        matchStatus: "matched"
        updatedAt: "2026-08-26"
        price: 1280
        rating: 4.17
        reviewCount: 6
        imageUrl: "https://item-shopping.c.yimg.jp/i/j/shop-ypp_3799-004426"
        available: true
    priceMax: 2290
    genreId: "405165"
  - rank: 9
    name: "どろばーゆ 泡洗顔"
    brand: "ナチュボーテ"
    price: 1380
    capacity: "120g"
    rating: 4.58
    reviewCount: 1576
    features:
      - "竹炭・海シルト・馬油配合の泥洗顔"
      - "無香料タイプとバラの香りタイプを展開"
      - "日本製のクレイ洗顔フォーム"
    pros:
      - "毛穴汚れが気になる方に選びやすい処方"
      - "無香料を選べば香り付きが苦手な人にも使いやすい"
      - "メンズにも使いやすいシンプルな設計"
    cons:
      - "泥タイプのためすすぎ残しに注意が必要"
      - "毎日の使用ではやや洗浄力が強く感じる場合がある"
    recommendedFor: "毛穴汚れを重点的にケアしたい方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00r97bn.3rdw60e8.g00r97bn.3rdw72a7/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Fa-cueb%2Fawasoapa-2set%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Fa-cueb%2Fi%2F10005406%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/a-cueb/cabinet/03069387/natubeaute/claywash/doro2602_sn1.jpg?_ex=128x128"
    offers:
      - provider: "yahoo"
        label: "Yahoo!"
        url: "https://ck.jp.ap.valuecommerce.com/servlet/referral?sid=3770852&pid=892615315&vc_url=https%3A%2F%2Fstore.shopping.yahoo.co.jp%2Facuebshop%2Fdorobayu.html"
        matchStatus: "matched"
        updatedAt: "2026-09-07"
        price: 1540
        rating: 0
        reviewCount: 0
        imageUrl: "https://item-shopping.c.yimg.jp/i/j/acuebshop_dorobayu"
        available: true
    priceMax: 1824
    genreId: "405130"
  - rank: 10
    name: "オルナ オーガニック 泥洗顔 130g"
    brand: "ALLNA ORGANIC"
    price: 1100
    capacity: "130g"
    rating: 4.62
    reviewCount: 1256
    features:
      - "顔の洗浄に使う洗顔料"
      - "継続利用しやすい定番構成"
      - "楽天市場で購入できる商品"
    pros:
      - "肌質や洗い上がりで選びやすい"
      - "買い足し候補として比較しやすい"
      - "既存商品と単価で比べやすい"
    cons:
      - "使用感や香りの好みを確認したい"
      - "購入時は最新の販売条件を確認したい"
    recommendedFor: "肌質や洗い上がりで選びたい方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00t45pn.3rdw68a0.g00t45pn.3rdw796e/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Ftsurunishi%2F905b01n0bsytw%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Ftsurunishi%2Fi%2F10000261%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/tsurunishi/cabinet/salesamune/event/20260918m/905b01n0bsytw-50.jpg?_ex=128x128"
    offers:
      - provider: "yahoo"
        label: "Yahoo!"
        url: "https://ck.jp.ap.valuecommerce.com/servlet/referral?sid=3770852&pid=892615315&vc_url=https%3A%2F%2Fstore.shopping.yahoo.co.jp%2Fadew%2F905b01n0bsytw.html"
        matchStatus: "review"
        updatedAt: "2026-07-15"
        price: 2200
        imageUrl: "https://item-shopping.c.yimg.jp/i/j/adew_905b01n0bsytw"
        available: false
        rating: 4.55
        reviewCount: 278
        matchNotes: "capacity不一致: capacity不一致"
    priceMax: 2970
    genreId: "216467"
tags:
  - "洗顔料 おすすめ"
  - "洗顔料 コスパ"
  - "洗顔料 比較"
  - "洗顔フォーム"
  - "まとめ買い"
draft: false
faqs:
  - question: "Q1: 洗顔料は朝と夜で使い分けるべき？"
    answer: "理想的には朝はマイルドなもの、夜は洗浄力のあるものを使い分けるのがおすすめです。ただしコスパ重視なら1本で済ませても問題ありません。"
  - question: "Q2: 泡立てネットは必要？"
    answer: "フォームタイプの洗顔料はネットを使うときめ細かい泡が作れ、少量でもしっかり泡立つため結果的にコスパが向上します。100円ショップで購入できるのでぜひ活用しましょう。"
  - question: "Q3: 洗顔料の使用期限は？"
    answer: "未開封なら約3年、開封後は3〜6ヶ月が目安です。まとめ買いする場合は半年〜1年で使い切れる量にしましょう。"
---


## g単価は「1回に使う量」が違うと比べられない

洗顔料は容量あたりの単価で横並びにできますが、**1回に使う量がタイプごとに大きく違う**ため、単価だけでは実際の負担が読めません。チューブのフォームは泡立てネットを使うかどうかで使用量が変わり、パウダーは1回分がごく少量、ポンプの泡タイプはひと押しの量が決まっています。

とくに**ポンプの泡タイプは中身に水分が多く含まれている**ため、同じ数字でも濃さがまったく違います。フォームやパウダーと容量あたりの単価をそのまま並べると、泡タイプが不当に割高に見えたり、逆に容量が大きいぶん割安に見えたりします。上部の比較表は同じ形状どうしで見比べるのが実際的です。

パウダー洗顔は容量あたりの単価が高めでも1回の使用量が少ないため、**使い切るまでの期間で見ると順位が入れ替わることがあります**。単価表は買うときの目安、実際の負担は使う量、と分けて考えてください。

## 容器の形で「使い切れる量」が変わる

チューブは最後まで絞り出せますが、ジャー（広口の容器）は指ですくうため底に残りやすく、ポンプはノズルまで上がらなくなった分が残ります。**表記容量をすべて使い切れるとは限らない**という差は、単価の比較には現れません。

大容量を選ぶときは、浴室に置いたままにするかどうかも合わせて考えます。浴室は湿度が高く、**開けたまま置くと水が入って傷みやすくなります**。詰め替えのない商品を大容量で買うなら、使い切るまでの期間が現実的かを先に確かめてください。

## 洗浄力の強いタイプは毎日向きとは限らない

泥や炭を配合したタイプ、スクラブ入りのタイプは、さっぱりした洗い上がりが特徴です。ただし**皮脂が少ない時期や乾燥しやすい肌では、毎日使うとつっぱりの原因**になります。こうしたタイプは週に数回にとどめ、普段は洗浄力のおだやかなものを使う、という併用のほうが扱いやすくなります。

朝と夜で洗顔料を変える運用も現実的です。皮脂やメイクを落とす夜と、寝ている間の汗を流すだけの朝では必要な洗浄力が違います。**1本ですべてをまかなおうとすると、どちらかで過不足が出ます**。

成分表示は配合量の多い順に並んでいます。先頭に来ている成分を見れば、石けんを主体にしたものか、洗浄成分がおだやかなものかの見当がつきます。

## 泡立てネットを使うかどうかで話が変わる

フォームタイプは泡立てネットを使うと少量でもきめ細かい泡になり、**同じ量でも洗える回数が増えます**。手のひらだけで泡立てると空気が入りきらず、量を足すことになりがちです。ネットを持っているかどうかで、同じ商品の使用量が変わるという前提があります。

一方、ポンプの泡タイプやジェルタイプは泡立ての手間がない代わりに、使用量を自分で調整しにくい構造です。**手早さを買っているのか、使用量の自由度を買っているのか**で選ぶと、あとから不満が出にくくなります。

## 買ってから気づきやすい注意点

- **こすらずに洗う**: 泡のクッションで包むように洗い、指の腹が肌に直接当たらないようにします。洗浄力より摩擦のほうが負担になることがあります
- **すすぎ残しは生え際と小鼻**: 顔の中心はすすげていても、生え際・あご下・小鼻の脇は残りやすい場所です。ここだけ意識して流すと仕上がりが変わります
- **お湯の温度を上げない**: 熱いお湯は必要な皮脂まで流します。ぬるま湯で十分に落ちます
- **開封後は季節をまたがない量で**: 使い切れる量かを先に決めてから、容量あたりの単価が低いものを選ぶ順番にします

## まとめ

洗顔料は容量あたりの単価で並べられますが、**1回に使う量と容器に残る量が形状ごとに違う**ため、単価表は同じ形状どうしで見比べるのが実際的です。泡タイプは水分を多く含む前提で、フォームやパウダーと直接比べないでください。

洗浄力の強いタイプは週に数回、普段はおだやかなものと使い分けると、1本ですべてをまかなうより無理がありません。価格と容量あたりの単価は、ページ上部の比較表（自動更新）でご確認ください。

スキンケアの前後もあわせて見直すと効果が出やすくなります。メイクを落とす工程は [クレンジングの比較](/articles/cleansing-comparison/)、部分的な肌荒れには [ニキビパッチの比較](/articles/acne-patch-comparison/) で整理しています。

---

※ 価格は記事執筆時点の楽天市場での販売価格です。価格は変動する場合があります。
