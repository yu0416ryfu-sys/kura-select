---
title: "歯磨き粉 おすすめランキング【2026年版】虫歯予防・ホワイトニング・歯周病ケア"
description: "歯磨き粉をフッ素濃度・成分・コスパで徹底比較。虫歯予防・ホワイトニング・歯周病ケア・知覚過敏のおすすめ10選を紹介。目的に合った歯磨き粉が楽天で見つかります。"
category: "toothpaste"
publishedAt: "2026-04-29"
articleType: "comparison"
updatedAt: "2026-09-10"
products:
  - rank: 1
    name: "クリニカ アドバンテージ ハミガキ クールミント 130g×3本"
    brand: "ライオン（クリニカ）"
    price: 1062
    capacity: "130g×3本"
    pricePerUnit: "約2.7円/g"
    rating: 4.71
    reviewCount: 332
    features:
      - "フッ素1450ppm配合の薬用ハミガキ（医薬部外品）"
      - "歯垢を分解する酵素デキストラナーゼを配合"
      - "3本セットで1本あたりの価格を抑えられる"
    pros:
      - "記事内で1gあたりが最も安い"
      - "ドラッグストア定番でリピートしやすい"
      - "虫歯予防の基本処方をひと通り備える"
    cons:
      - "ホワイトニング専用の処方ではない"
      - "3本セットなので置き場所を取る"
    recommendedFor: "虫歯予防を優先しつつ単価も抑えたい方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00r136n.3rdw697f.g00r136n.3rdw7383/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Frakuten24%2F65916%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Frakuten24%2Fi%2F10905882%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/rakuten24/cabinet/916/65916.jpg?_ex=128x128"
    genreId: "204759"
  - rank: 2
    name: "システマ ハグキプラスW ハミガキ 95g"
    brand: "ライオン（システマ）"
    price: 404
    capacity: "95g"
    pricePerUnit: "約4.3円/g"
    rating: 4.75
    reviewCount: 12
    features:
      - "歯ぐきケア向けの薬用ハミガキ（医薬部外品）"
      - "ホワイトニング処方で着色汚れのケアにも対応"
      - "ドラッグストア定番の単品売りで買い足しやすい"
    pros:
      - "1本あたりの価格が手ごろで続けやすい"
      - "ライオンの歯科ブランドで入手性が高い"
      - "歯ぐきケアと着色ケアを1本でまかなえる"
    cons:
      - "まとめ買い向けのセット構成ではない"
      - "薬用らしい味わいで好みが分かれる"
    recommendedFor: "歯ぐきのケアを1本から手ごろに始めたい方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00rbnvn.3rdw6e69.g00rbnvn.3rdw7dd2/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Ftsuruha%2F10091291%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Ftsuruha%2Fi%2F10037527%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/tsuruha/cabinet/syohin91/10091291.jpg?_ex=128x128"
    offers:
      - provider: "yahoo"
        label: "Yahoo!"
        url: "https://lohaco.yahoo.co.jp/store/h-lohaco/item/1898149/"
        matchStatus: "pending"
        updatedAt: "2026-09-07"
        price: 449
        rating: 4.6
        reviewCount: 25
        imageUrl: "https://item-shopping.c.yimg.jp/i/j/rd-lohaco_1898149"
        available: true
    genreId: "204759"
  - rank: 3
    name: "キラルンペースト 100g"
    brand: "RUKEN"
    price: 1298
    capacity: "100g"
    pricePerUnit: "約13円/g"
    rating: 4.3
    reviewCount: 11022
    features:
      - "毎日の歯みがきに使える歯磨き粉"
      - "ホワイトニングや口臭ケア系も選べる"
      - "容量で比較しやすい"
    pros:
      - "日常ケアに取り入れやすい"
      - "目的別に選べる"
      - "ストックしやすい"
    cons:
      - "成分や使用感の好みは分かれる"
      - "医薬部外品表記の確認が必要"
    recommendedFor: "歯磨き粉を目的と容量で比較したい方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00qk8an.3rdw62a6.g00qk8an.3rdw7d28/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Fru-collagen%2Ftoothp%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Fru-collagen%2Fi%2F10000131%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/ru-collagen/cabinet/mailmagazine/r2022/toothp_sale_tmb09.jpg?_ex=128x128"
    offers:
      - provider: "yahoo"
        label: "Yahoo!"
        url: "https://ck.jp.ap.valuecommerce.com/servlet/referral?sid=3770852&pid=892615315&vc_url=https%3A%2F%2Fstore.shopping.yahoo.co.jp%2Fru-collagen%2F007.html"
        matchStatus: "matched"
        updatedAt: "2026-09-07"
        price: 1298
        imageUrl: "https://item-shopping.c.yimg.jp/i/j/ru-collagen_007"
        available: true
        rating: 4.31
        reviewCount: 1070
    genreId: "204759"
  - rank: 4
    name: "キラルン 2点ミニセット パウダー5g＋ペースト15g"
    brand: "RUKEN"
    price: 328
    capacity: "パウダー5g + ペースト15g（計20g）"
    pricePerUnit: "約16.4円/g"
    rating: 4.19
    reviewCount: 4742
    features:
      - "毎日の歯みがきに使える歯磨き粉"
      - "ホワイトニングや口臭ケア系も選べる"
      - "容量で比較しやすい"
    pros:
      - "日常ケアに取り入れやすい"
      - "目的別に選べる"
      - "ストックしやすい"
    cons:
      - "成分や使用感の好みは分かれる"
      - "医薬部外品表記の確認が必要"
    recommendedFor: "歯磨き粉を目的と容量で比較したい方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00qk8an.3rdw62a6.g00qk8an.3rdw7d28/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Fru-collagen%2Ftoothsmple%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Fru-collagen%2Fi%2F10000104%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/ru-collagen/cabinet/kilalun_n/toothpsnew.jpg?_ex=128x128"
    genreId: "204759"
  - rank: 5
    name: "キラルンパウダー"
    brand: "RUKEN"
    price: 1000
    capacity: "55g"
    pricePerUnit: "約18円/g"
    rating: 4.31
    reviewCount: 3282
    features:
      - "毎日の歯みがきに使える歯磨き粉"
      - "ホワイトニングや口臭ケア系も選べる"
      - "容量で比較しやすい"
    pros:
      - "日常ケアに取り入れやすい"
      - "目的別に選べる"
      - "ストックしやすい"
    cons:
      - "成分や使用感の好みは分かれる"
      - "医薬部外品表記の確認が必要"
    recommendedFor: "歯磨き粉を目的と容量で比較したい方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00qk8an.3rdw62a6.g00qk8an.3rdw7d28/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Fru-collagen%2Ftoothmini%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Fru-collagen%2Fi%2F10000114%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/ru-collagen/cabinet/kilalun_n/05964132/item00kirarunpwd.jpg?_ex=128x128"
    offers:
      - provider: "yahoo"
        label: "Yahoo!"
        url: "https://ck.jp.ap.valuecommerce.com/servlet/referral?sid=3770852&pid=892615315&vc_url=https%3A%2F%2Fstore.shopping.yahoo.co.jp%2Fru-collagen%2F008.html"
        matchStatus: "matched"
        updatedAt: "2026-09-07"
        price: 1254
        imageUrl: "https://item-shopping.c.yimg.jp/i/j/ru-collagen_008"
        available: true
        rating: 4.24
        reviewCount: 254
    genreId: "204759"
  - rank: 6
    name: "だいじょうぶなもの ホワイトニングトゥースペースト シルクパウダー"
    brand: "だいじょうぶなもの"
    price: 1760
    capacity: "100g"
    pricePerUnit: "約18円/g"
    rating: 4.38
    reviewCount: 679
    features:
      - "マグネシウムとシルクパウダーを配合したペースト"
      - "合成着色料などを避けた処方をうたう"
      - "旧メイドオブオーガニクスの後継ライン"
    pros:
      - "原材料にこだわりたい人が選びやすい"
      - "発泡が穏やかで長めに磨きやすい"
      - "家族で共用しやすい味わい"
    cons:
      - "一般的な市販品より価格帯が高い"
      - "フッ素配合の有無は表示を確認する必要がある"
    recommendedFor: "成分表示にこだわって歯磨き粉を選びたい方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00q5inn.3rdw6732.g00q5inn.3rdw7aa0/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Faimere%2Fm0090025%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Faimere%2Fi%2F10003860%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/aimere/cabinet/08234025/imgrc0132742671.jpg?_ex=128x128"
    offers:
      - provider: "yahoo"
        label: "Yahoo!"
        url: "https://ck.jp.ap.valuecommerce.com/servlet/referral?sid=3770852&pid=892615315&vc_url=https%3A%2F%2Fstore.shopping.yahoo.co.jp%2Fyoga-pi%2Fmoo-18026.html"
        matchStatus: "pending"
        updatedAt: "2026-09-07"
        price: 1760
        rating: 4.43
        reviewCount: 23
        imageUrl: "https://item-shopping.c.yimg.jp/i/j/yoga-pi_moo-18026"
        available: true
    genreId: "204759"
  - rank: 7
    name: "アパガードリナメル"
    brand: "アパガード"
    price: 2870
    capacity: "120g"
    pricePerUnit: "約24円/g"
    rating: 4.6
    reviewCount: 804
    features:
      - "毎日の歯みがきに使える歯磨き粉"
      - "ホワイトニングや口臭ケア系も選べる"
      - "容量で比較しやすい"
    pros:
      - "日常ケアに取り入れやすい"
      - "目的別に選べる"
      - "ストックしやすい"
    cons:
      - "成分や使用感の好みは分かれる"
      - "医薬部外品表記の確認が必要"
    recommendedFor: "歯磨き粉を目的と容量で比較したい方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00r6h3n.3rdw6e54.g00r6h3n.3rdw719f/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Fd-fit%2F84449%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Fd-fit%2Fi%2F10005817%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/d-fit/cabinet/2026_0908_p5400/mini/84449_omake2m.jpg?_ex=128x128"
    offers:
      - provider: "yahoo"
        label: "Yahoo!"
        url: "https://ck.jp.ap.valuecommerce.com/servlet/referral?sid=3770852&pid=892615315&vc_url=https%3A%2F%2Fstore.shopping.yahoo.co.jp%2Fd-fit%2F34284.html"
        matchStatus: "matched"
        updatedAt: "2026-09-07"
        price: 3020
        imageUrl: "https://item-shopping.c.yimg.jp/i/j/d-fit_34284"
        available: true
        rating: 4.51
        reviewCount: 1084
    genreId: "204759"
  - rank: 8
    name: "しろえ ホワイトニングパウダー"
    brand: "しろえ"
    price: 1798
    capacity: "20g"
    pricePerUnit: "約90円/g"
    rating: 4.35
    reviewCount: 767
    features:
      - "毎日の歯みがきに使える歯磨き粉"
      - "ホワイトニングや口臭ケア系も選べる"
      - "容量で比較しやすい"
    pros:
      - "日常ケアに取り入れやすい"
      - "目的別に選べる"
      - "ストックしやすい"
    cons:
      - "成分や使用感の好みは分かれる"
      - "医薬部外品表記の確認が必要"
    recommendedFor: "歯磨き粉を目的と容量で比較したい方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00rx8qn.3rdw6a27.g00rx8qn.3rdw73bb/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Fshizenshop%2Fwhitening-powder%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Fshizenshop%2Fi%2F10000299%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/shizenshop/cabinet/whitening-powder/powder_0519.jpg?_ex=128x128"
    genreId: "204759"
  - rank: 9
    name: "WHITH WHITE 薬用歯磨き"
    brand: "WHITH WHITE"
    price: 2580
    capacity: "120g"
    rating: 4.47
    reviewCount: 2334
    features:
      - "毎日の歯みがきに使える歯磨き粉"
      - "ホワイトニングや口臭ケア系も選べる"
      - "容量で比較しやすい"
    pros:
      - "日常ケアに取り入れやすい"
      - "目的別に選べる"
      - "ストックしやすい"
    cons:
      - "成分や使用感の好みは分かれる"
      - "医薬部外品表記の確認が必要"
    recommendedFor: "歯磨き粉を目的と容量で比較したい方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00tknln.3rdw6c40.g00tknln.3rdw7e6d/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Fwhithwhite%2F906b07qzwqjrm%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Fwhithwhite%2Fi%2F10000130%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/whithwhite/cabinet/salesamune/20260904ss/906b07qzwqjrm-50.jpg?_ex=128x128"
    priceMax: 6840
    genreId: "204759"
  - rank: 10
    name: "薬用しろえ 歯磨きジェル"
    brand: "しろえ"
    price: 2200
    capacity: "50g"
    rating: 4.37
    reviewCount: 2248
    features:
      - "毎日の歯みがきに使える歯磨き粉"
      - "ホワイトニングや口臭ケア系も選べる"
      - "容量で比較しやすい"
    pros:
      - "日常ケアに取り入れやすい"
      - "目的別に選べる"
      - "ストックしやすい"
    cons:
      - "成分や使用感の好みは分かれる"
      - "医薬部外品表記の確認が必要"
    recommendedFor: "歯磨き粉を目的と容量で比較したい方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00rx8qn.3rdw6a27.g00rx8qn.3rdw73bb/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Fshizenshop%2Fshiroewhitening%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Fshizenshop%2Fi%2F10000095%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/shizenshop/cabinet/04836660/shiroe_260525.jpg?_ex=128x128"
    priceMax: 10300
    genreId: "204759"
tags:
  - "歯磨き粉"
  - "コスパ"
  - "虫歯予防"
  - "日用品"
draft: false
faqs:
  - question: "フッ素1450ppmは安全？"
    answer: "日本の厚生労働省が認可した上限濃度であり、通常の使用方法であれば安全とされています。ただし6歳未満の子どもには低濃度のフッ素配合品を使用し、飲み込まないよう注意が必要です。"
  - question: "研磨剤入りは歯を傷つける？"
    answer: "適切な力加減で磨けば問題ないとされていますが、強い力でゴシゴシ磨く癖がある方はエナメル質を傷つける可能性があります。心配な方は低研磨タイプや研磨剤不使用の歯磨き粉を検討してみてください。"
  - question: "子供用と大人用の違いは？"
    answer: "主な違いはフッ素濃度と味付けです。子供用は低濃度フッ素（500〜950ppm）で甘めの味付けが多く、大人用は高濃度フッ素（1450ppm）でミント系の味付けが一般的です。15歳以上であれば大人用の使用が推奨されています。"
  - question: "歯磨き粉は高い方が良い？"
    answer: "必ずしも価格が高ければ効果が高いとは限りません。大切なのは自分の口腔トラブルに合った有効成分が配合されているかどうかです。虫歯予防が目的なら、高濃度フッ素配合のコスパの良い商品で十分な効果が期待できます。"
---

## フッ素濃度は「上限まで使えるか」で決まる

歯磨き粉を虫歯予防の観点で選ぶとき、最初に見る数字がフッ化物の濃度です。日本国内で市販できる上限は1450ppmで、成人向けの製品の多くがこの濃度か、それに近い値に設定されています。

濃度が高いほど予防効果が期待できるとされていますが、誰でも上限を選べばよいわけではありません。フッ化物には年齢ごとの推奨量があり、**うがいがきちんとできるかどうか**が実質的な分かれ目になります。飲み込んでしまう年齢では、濃度を抑えた製品を少量使うのが原則です。

| 濃度の目安 | 主な対象 |
|---|---|
| 1450ppm前後 | 成人・おおむね15歳以上 |
| 950ppm前後 | 学童期 |
| 500ppm前後 | 未就学の年齢 |

もうひとつ見落とされやすいのが、**使ったあとのすすぎ方**です。何度も強くすすぐと、せっかく歯面に触れたフッ化物が流れてしまいます。少量の水で軽く一回すすぐ程度にとどめるほうが、残る量は多くなります。濃度を上げることより、この習慣を変えるほうが効果が大きい場合もあります。

## 研磨剤は「悪者」ではないが、使い方で差が出る

研磨剤（清掃剤）は着色汚れを物理的に落とすための成分です。コーヒーやお茶による表面の着色には効果がありますが、強い力で長く磨けば歯の表面を削ることにもつながります。

問題になるのは成分そのものより、**ブラッシングの圧**です。研磨剤入りの歯磨き粉を軽い力で使うのと、研磨剤なしの製品を強い力で使うのとでは、後者のほうが歯や歯ぐきを痛めることもあります。

歯ぐきが下がって根元が露出している場合、その部分はエナメル質に覆われていないため摩耗しやすくなります。しみる症状がある、根元がえぐれてきた、という自覚があるなら低研磨または研磨剤無配合のタイプを選ぶのが無難です。逆に着色が気になり、歯ぐきに問題がないなら、研磨剤入りを普通の力で使って構いません。

## 目的別に見るべき成分が違う

歯磨き粉は「よく効くもの」を選ぶより、**いま困っていることに合わせる**ほうが選びやすくなります。

**虫歯を防ぎたい場合**は、フッ化物の濃度が主な判断軸です。他の機能をあれこれ足すより、上限に近い濃度の製品を毎日使い、すすぎを控えめにするほうが素直な選択です。

**歯ぐきの腫れや出血が気になる場合**は、歯肉炎に働きかける薬用成分が配合されたタイプが候補になります。ただし歯磨き粉だけで歯周病が治るわけではなく、汚れを落とす行為そのものが主役である点は変わりません。

**着色が気になる場合**は、研磨剤やステインを浮かせる成分を含むタイプです。ここで注意したいのは、もともとの歯の色を白く変えるものではないという点です。表面に付いた着色を落とすことと、歯そのものの色を変えることは別の話です。

**冷たいものがしみる場合**は、刺激の伝わりを抑える成分を配合したタイプがあります。効果を感じるまで継続して使う前提の製品が多く、数日で判断せず一定期間続けてみるのが一般的な使い方です。

## 味と刺激は続けられるかを左右する

歯磨き粉は毎日使うものなので、味や刺激の強さは無視できない要素です。強いミント感が苦手で歯磨きが短くなってしまうくらいなら、刺激の弱い製品に替えたほうが結果的に汚れは落ちます。

発泡剤の量も使い勝手に関わります。泡立ちが多いと磨けた感覚は得やすい一方、口の中が泡でいっぱいになって早めに吐き出してしまい、磨く時間が短くなることがあります。低発泡タイプは、鏡を見ながら磨きたい人や、時間をかけて磨きたい人に向いています。

## 保管とまとめ買いの考え方

歯磨き粉は未開封であれば比較的長く保管できる製品です。そのため、使う銘柄が決まっているならまとめ買いと相性がよい部類に入ります。

保管は高温多湿を避けた常温が基本です。開封後は成分が乾いてチューブの口が固まることがあるので、キャップを閉め、口元を清潔に保ってください。洗面所に出しっぱなしにする場合は、直射日光が当たらない位置に置くほうが無難です。

まとめ買いで気をつけたいのは、**味や使用感が自分に合うかを確かめてから**にすることです。刺激の強さは同じブランドでも製品ラインで違い、届いてから合わないと分かると使い切るのが苦痛になります。

具体的な価格や1gあたりの単価は変動するため、ページ上部の比較表（自動更新）でご確認ください。

## 歯磨き粉だけでは届かない場所がある

最後に押さえておきたいのは、歯磨き粉の成分がどれだけ優れていても、**ブラシが届かない場所には作用しない**という点です。歯と歯が接している面は歯ブラシの毛が入らないため、そこに虫歯や歯周病が起きやすくなります。

成分を吟味するより、歯間のケアを一つ足すほうが効果が大きいことがあります。ブラシ側の選び方は [歯ブラシの比較](/articles/toothbrush-comparison/)、歯間のケアは [歯間ブラシの比較](/articles/interdental-brush-comparison/) で整理しています。お子さま向けの選び方は [子ども用歯磨き粉の比較](/articles/kids-toothpaste-comparison/) をご覧ください。

## まとめ

歯磨き粉は、フッ化物の濃度を年齢に合わせて選び、すすぎを控えめにするのが虫歯予防の基本です。研磨剤は成分そのものより磨く力の強さが問題になります。歯ぐき、着色、しみる症状など、いま困っていることに合わせて成分を選び、味と刺激は毎日続けられる範囲にしてください。

> **価格表記について**: 掲載している価格は執筆時点のものです。最新価格は楽天市場でご確認ください。
