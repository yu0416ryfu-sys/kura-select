---
title: "オキシクリーンと過炭酸ナトリウムを徹底比較【2026年版】コスパ・成分どっちがいい？"
description: "過炭酸ナトリウムとオキシクリーンは何が違う？中身・洗浄力・1gあたりのコスパを4軸で比較し、どっちを選ぶべきかを解説。洗濯槽掃除やオキシ漬け、食器の除菌まで用途別の使い分けと、界面活性剤の有無による向き不向きも整理しました。価格は毎週自動更新。"
category: "laundry-detergent"
publishedAt: "2026-07-28"
articleType: "comparison"
updatedAt: "2026-09-19"
draft: false
products:
  - rank: 1
    name: "NICHIGA 酸素系漂白剤 過炭酸ナトリウム 3kg"
    brand: "NICHIGA"
    price: 1130
    capacity: "3kg"
    pricePerUnit: "約0.38円/g"
    rating: 4.78
    reviewCount: 832
    features:
      - "過炭酸ナトリウム100%の大容量パック"
      - "洗濯槽掃除と衣類の漂白を想定した汎用タイプ"
      - "袋入りで保管しやすい形状"
    pros:
      - "定期的に洗濯槽掃除をする家庭でも在庫が持ちやすい"
      - "使う量を気にせず使えるので用途を広げやすい"
      - "組成がシンプルで用途に応じて濃度を調整しやすい"
    cons:
      - "開封後は密閉容器に移すなど湿気対策が要る"
      - "香りや泡立ちがないため使用感を求める方には向かない"
    recommendedFor: "洗濯槽掃除を習慣にしている方・コスト重視の方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00qwojn.3rdw6a14.g00qwojn.3rdw7cba/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Fauc-garlic%2F10000016%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Fauc-garlic%2Fi%2F10000016%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/auc-garlic/cabinet/01359158/r-hyouhaku-3kg.jpg?_ex=128x128"
    offers:
      - provider: "yahoo"
        label: "Yahoo!"
        url: "https://ck.jp.ap.valuecommerce.com/servlet/referral?sid=3770852&pid=892615315&vc_url=https%3A%2F%2Fstore.shopping.yahoo.co.jp%2Fnichiga%2Fkatannsann3k.html"
        matchStatus: "matched"
        updatedAt: "2026-09-19"
        price: 1130
        rating: 4.71
        reviewCount: 244
        imageUrl: "https://item-shopping.c.yimg.jp/i/j/nichiga_katannsann3k"
        available: true
    genreId: "564510"
  - rank: 2
    name: "NICHIGA 酸素系漂白剤 過炭酸ナトリウム 4.5kg"
    brand: "NICHIGA"
    price: 2140
    capacity: "4.5kg"
    pricePerUnit: "約0.48円/g"
    rating: 4.84
    reviewCount: 763
    features:
      - "過炭酸ナトリウム100%の特大サイズ"
      - "洗濯槽掃除と浸け置き洗いを想定した容量"
      - "袋入りの粉末タイプ"
    pros:
      - "浸け置き洗いを頻繁にしても在庫が切れにくい"
      - "まとめ買いで買い足しの手間を減らせる"
      - "掃除用と洗濯用を分けて使いやすい容量"
    cons:
      - "重量があり保管場所を選ぶ"
      - "使い切るまで時間がかかると吸湿で固まることがある"
    recommendedFor: "浸け置き洗いを日常的にする方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00qwojn.3rdw6a14.g00qwojn.3rdw7cba/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Fauc-garlic%2Fkatannsann5k%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Fauc-garlic%2Fi%2F10000294%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/auc-garlic/cabinet/01359158/r-hyouhaku-4500g.jpg?_ex=128x128"
    offers:
      - provider: "yahoo"
        label: "Yahoo!"
        url: "https://ck.jp.ap.valuecommerce.com/servlet/referral?sid=3770852&pid=892615315&vc_url=https%3A%2F%2Fstore.shopping.yahoo.co.jp%2Fnichiga%2Fkatannsann5k.html"
        matchStatus: "matched"
        updatedAt: "2026-09-19"
        price: 1480
        rating: 4.77
        reviewCount: 255
        imageUrl: "https://item-shopping.c.yimg.jp/i/j/nichiga_katannsann5k"
        available: true
    genreId: "564510"
  - rank: 3
    name: "NICHIGA 酸素系漂白剤 過炭酸ナトリウム 950g"
    brand: "NICHIGA"
    price: 498
    capacity: "950g"
    pricePerUnit: "約0.52円/g"
    rating: 4.71
    reviewCount: 969
    features:
      - "過炭酸ナトリウム100%の酸素系漂白剤"
      - "添加物を加えていないシンプルな組成"
      - "洗濯槽クリーナーとしての用途を想定した表示"
    pros:
      - "まず少量から試せるサイズで失敗しにくい"
      - "無香料無添加のため用途を選びにくい"
      - "レビュー数が多く使い方の情報を集めやすい"
    cons:
      - "計量スプーンが付属しないため別途用意が必要"
      - "泡立ちがほとんどなく洗えている実感は得にくい"
    recommendedFor: "まず過炭酸ナトリウムを試してみたい方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00qwojn.3rdw6a14.g00qwojn.3rdw7cba/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Fauc-garlic%2F10000015%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Fauc-garlic%2Fi%2F10000015%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/auc-garlic/cabinet/01359158/r-hyouhaku-950g.jpg?_ex=128x128"
    offers:
      - provider: "yahoo"
        label: "Yahoo!"
        url: "https://ck.jp.ap.valuecommerce.com/servlet/referral?sid=3770852&pid=892615315&vc_url=https%3A%2F%2Fstore.shopping.yahoo.co.jp%2Fnichiga%2Fkatannsann1.html"
        matchStatus: "matched"
        updatedAt: "2026-09-19"
        price: 598
        rating: 4.56
        reviewCount: 2068
        imageUrl: "https://item-shopping.c.yimg.jp/i/j/nichiga_katannsann1"
        available: true
    genreId: "564510"
  - rank: 4
    name: "国産 過炭酸ナトリウム 酸素系漂白剤 3kg"
    brand: "アレスヘルスケア"
    price: 1991
    capacity: "3kg"
    pricePerUnit: "約0.66円/g"
    rating: 4.77
    reviewCount: 921
    features:
      - "国内製造の過炭酸ナトリウム"
      - "粉末タイプの酸素系漂白剤"
      - "衣類と洗濯槽の両方を想定した汎用タイプ"
    pros:
      - "国内製造を選びたい場合の候補になる"
      - "レビュー数が多く使用例を参考にしやすい"
      - "大容量で用途を絞らずに使える"
    cons:
      - "同じ過炭酸ナトリウムでも輸入品より単価は上がりやすい"
      - "スプーンなしの構成があるため商品ページの確認が要る"
    recommendedFor: "国内製造にこだわりたい方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00qa34n.3rdw6dcf.g00qa34n.3rdw74c1/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Fares-hc%2F700056%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Fares-hc%2Fi%2F10000097%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/ares-hc/cabinet/00480640/katansan3image-1000.jpg?_ex=128x128"
    offers:
      - provider: "yahoo"
        label: "Yahoo!"
        url: "https://ck.jp.ap.valuecommerce.com/servlet/referral?sid=3770852&pid=892615315&vc_url=https%3A%2F%2Fstore.shopping.yahoo.co.jp%2Fares%2F700056.html"
        matchStatus: "matched"
        updatedAt: "2026-09-19"
        price: 1991
        rating: 4.66
        reviewCount: 77
        imageUrl: "https://item-shopping.c.yimg.jp/i/j/ares_700056"
        available: true
    genreId: "568344"
  - rank: 5
    name: "オキシクリーン マルチパーパスクリーナー 5.26kg"
    brand: "OxiClean（アメリカ版）"
    price: 3690
    capacity: "5.26kg"
    pricePerUnit: "約0.70円/g"
    rating: 4.52
    reviewCount: 189
    features:
      - "アメリカ向け仕様のマルチパーパスクリーナー"
      - "界面活性剤と香料を配合した処方"
      - "コストコなどで流通する業務用サイズ"
    pros:
      - "泡立ちがあり洗浄剤としての使用感がわかりやすい"
      - "大容量のため頻繁に買い足す手間が少ない"
      - "青い粒入りの見た目で溶け残りを確認しやすい"
    cons:
      - "香料入りのため香りが苦手な方には向かない"
      - "大容量で保管スペースと湿気対策が必要になる"
    recommendedFor: "泡立ちのある使用感を求める方・大量に使う方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00toizn.3rdw6228.g00toizn.3rdw730f/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Fmonoworld%2Fzakka-0250%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Fmonoworld%2Fi%2F10001131%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/monoworld/cabinet/08879859/compass1647750527.jpg?_ex=128x128"
    offers:
      - provider: "yahoo"
        label: "Yahoo!"
        url: "https://ck.jp.ap.valuecommerce.com/servlet/referral?sid=3770852&pid=892615315&vc_url=https%3A%2F%2Fstore.shopping.yahoo.co.jp%2Fkameyamastore%2Fcos-28137.html"
        matchStatus: "matched"
        updatedAt: "2026-08-24"
        price: 3230
        rating: 4.74
        reviewCount: 94
        imageUrl: "https://item-shopping.c.yimg.jp/i/j/kameyamastore_cos-28137"
        available: true
    genreId: "210182"
  - rank: 6
    name: "国産 過炭酸ナトリウム 酸素系漂白剤 900g"
    brand: "アレスヘルスケア"
    price: 638
    capacity: "900g"
    pricePerUnit: "約0.71円/g"
    rating: 4.82
    reviewCount: 394
    features:
      - "国内製造の過炭酸ナトリウム"
      - "少量から試せる小容量パッケージ"
      - "粉末タイプの酸素系漂白剤"
    pros:
      - "国内製造の商品を少量から試せる"
      - "小容量なので保管場所を取らない"
      - "使い切りやすく品質が落ちにくい"
    cons:
      - "大容量パックと比べて1回あたりのコストは高くなる"
      - "洗濯槽掃除を続けるには量が足りない場合がある"
    recommendedFor: "国内製造を少量から試したい方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00qa34n.3rdw6dcf.g00qa34n.3rdw74c1/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Fares-hc%2F700181001%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Fares-hc%2Fi%2F10000478%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/ares-hc/cabinet/00480640/katansan900image.jpg?_ex=128x128"
    offers:
      - provider: "yahoo"
        label: "Yahoo!"
        url: "https://ck.jp.ap.valuecommerce.com/servlet/referral?sid=3770852&pid=892615315&vc_url=https%3A%2F%2Fstore.shopping.yahoo.co.jp%2Fares%2F700181001.html"
        matchStatus: "matched"
        updatedAt: "2026-09-19"
        price: 638
        rating: 4.85
        reviewCount: 41
        imageUrl: "https://item-shopping.c.yimg.jp/i/j/ares_700181001"
        available: true
    genreId: "564510"
  - rank: 7
    name: "オキシクリーン 1500g×3個 業務用"
    brand: "グラフィコ（日本版）"
    price: 3980
    capacity: "1500g×3個（4500g）"
    pricePerUnit: "約0.88円/g"
    rating: 4.66
    reviewCount: 70
    features:
      - "日本版オキシクリーンのまとめ買いセット"
      - "無香料の粉末タイプ"
      - "同一サイズを複数用意する構成"
    pros:
      - "1袋ずつ開封できるため湿気の影響を受けにくい"
      - "洗濯用と掃除用で保管場所を分けられる"
      - "買い足しの頻度を減らせる"
    cons:
      - "一度の支払額はまとまる"
      - "保管スペースを確保する必要がある"
    recommendedFor: "日常的にオキシ漬けをする方・買い置きしたい方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00rqlbn.3rdw622d.g00rqlbn.3rdw72ca/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Fk-home%2F7112916%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Fk-home%2Fi%2F10072715%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/k-home/cabinet/thum/naosi/7112916.jpg?_ex=128x128"
    genreId: "564510"
  - rank: 8
    name: "オキシクリーン つめかえ用 2000g 粉末タイプ"
    brand: "グラフィコ（日本版）"
    price: 2280
    capacity: "2000g"
    pricePerUnit: "約1.1円/g"
    rating: 4.77
    reviewCount: 166
    features:
      - "日本版オキシクリーンの詰め替えパック"
      - "無香料の粉末タイプ"
      - "袋入りで保管しやすい形状"
    pros:
      - "本体容器を使い回せるためゴミを減らせる"
      - "袋入りで収納の自由度が高い"
      - "無香料のため用途を選びにくい"
    cons:
      - "袋のままだと湿気が入りやすく密閉容器への移し替えが要る"
      - "計量スプーンが付属しない場合がある"
    recommendedFor: "すでに本体を持っている方・詰め替えで続けたい方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00ru0on.3rdw60b7.g00ru0on.3rdw7edf/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Fat-life%2F103-4571169854644-wa%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Fat-life%2Fi%2F12260109%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/at-life/cabinet/d/dd/imgrc0081065780.jpg?_ex=128x128"
    offers:
      - provider: "yahoo"
        label: "Yahoo!"
        url: "https://ck.jp.ap.valuecommerce.com/servlet/referral?sid=3770852&pid=892615315&vc_url=https%3A%2F%2Fstore.shopping.yahoo.co.jp%2Firisplaza%2F7180967.html"
        matchStatus: "matched"
        updatedAt: "2026-08-26"
        price: 2280
        rating: 4.61
        reviewCount: 36
        imageUrl: "https://item-shopping.c.yimg.jp/i/j/irisplaza_7180967"
        available: true
    genreId: "564510"
  - rank: 9
    name: "オキシクリーン 1500g 無香料 粉末タイプ"
    brand: "グラフィコ（日本版）"
    price: 1525
    capacity: "1500g"
    rating: 4.53
    reviewCount: 1068
    features:
      - "日本国内向けに販売されている無香料タイプ"
      - "界面活性剤と柔軟成分を配合していない処方"
      - "計量スプーン付きで使用量を量りやすい"
    pros:
      - "香りが残らないため衣類や食器まわりにも使いやすい"
      - "情報量が多く使い方を調べやすい定番品"
      - "スーパーやドラッグストアでも入手しやすい"
    cons:
      - "純粋な過炭酸ナトリウム製品と比べると単価は上がりやすい"
      - "泡立ちを期待すると物足りなく感じる場合がある"
    recommendedFor: "まず定番から試したい方・無香料がよい方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00rqlbn.3rdw622d.g00rqlbn.3rdw72ca/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Fk-home%2F7091275%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Fk-home%2Fi%2F10060922%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/k-home/cabinet/06135991/09344783/7091275-1.jpg?_ex=128x128"
    offers:
      - provider: "yahoo"
        label: "Yahoo!"
        url: "https://ck.jp.ap.valuecommerce.com/servlet/referral?sid=3770852&pid=892615315&vc_url=https%3A%2F%2Fstore.shopping.yahoo.co.jp%2Fbrisk-online%2F20251011231548-00401.html"
        matchStatus: "matched"
        updatedAt: "2026-08-02"
        price: 1395
        rating: 0
        reviewCount: 1
        imageUrl: "https://item-shopping.c.yimg.jp/i/j/brisk-online_20251011231548-00401"
        available: true
    priceMax: 7980
    genreId: "564510"
tags:
  - "オキシクリーン 比較"
  - "過炭酸ナトリウム コスパ"
  - "酸素系漂白剤 おすすめ"
  - "洗濯槽クリーナー"
faqs:
  - question: "オキシクリーンと過炭酸ナトリウムは中身が同じですか？"
    answer: "主成分はどちらも過炭酸ナトリウムで、系統としては同じ酸素系漂白剤です。ただし日本版オキシクリーンは無香料で界面活性剤・柔軟成分を配合していないのに対し、アメリカ版は界面活性剤と香料が配合されています。完全に同一というわけではないため、香りや泡立ちの有無で選び分けるとよいでしょう。"
  - question: "洗濯槽の掃除にはどちらが向いていますか？"
    answer: "どちらも洗濯槽クリーナーとして使えます。洗濯槽の掃除は一度に使う量が多くなるため、大容量パックが選びやすい過炭酸ナトリウムのほうがコストは抑えやすい傾向にあります。ドラム式は使用可否や使用量がメーカーごとに異なるため、必ず洗濯機の取扱説明書を確認してください。"
  - question: "使ってはいけないものはありますか？"
    answer: "アルミ製品、ウールやシルクなどの動物性繊維、水洗いできない衣類には使えません。これはオキシクリーン・過炭酸ナトリウムのどちらにも共通する注意点です。また、塩素系漂白剤と混ぜて使うことは避け、換気をしながら使用してください。"
  - question: "開封後はどのくらい持ちますか？"
    answer: "過炭酸ナトリウムは湿気を吸うと少しずつ分解が進みます。未開封であれば長期保存できますが、開封後は密閉容器に移し、直射日光と高温多湿を避けた場所で保管してください。固まってしまった場合は効果が落ちている可能性があります。"
---

## 結論：主成分は同じ。使い勝手を買うか、単価を取るか

オキシクリーンと過炭酸ナトリウムは、**主成分が同じ酸素系漂白剤**です。オキシクリーンは過炭酸ナトリウムをベースにした製品名で、まったく別のものではありません。

そのうえで違いを一言でまとめると、こうなります。

- **オキシクリーン**：ブランド品として使い方の情報が整っていて、計量スプーンやパッケージも扱いやすい。そのぶん単価は上がる
- **過炭酸ナトリウム**：中身は同じ系統で、余計なものを足していない。単価を抑えやすいが、使い方は自分で調べる必要がある

どちらを選んでも「酸素系漂白剤を使う」という点では同じです。判断の分かれ目は、使い方を調べる手間をお金で買うかどうかです。

## オキシクリーンとは：過炭酸ナトリウムをベースにしたブランド品

オキシクリーンは、過炭酸ナトリウムを主成分とする酸素系漂白剤のブランドです。日本で流通しているものには大きく2種類あります。

**日本版（グラフィコ）**は無香料で、界面活性剤や柔軟成分を配合していません。中身としては過炭酸ナトリウムに近い構成です。

**アメリカ版**は界面活性剤と香料が配合され、青い粒が混ざっているのが見た目の特徴です。泡立ちがあるぶん「洗剤を使っている」という実感を得やすい一方、香りの好みは分かれます。

「オキシ漬け」という言葉が定着しているとおり、浸け置き洗いのやり方をブランド側が発信しているため、初めてでも手順を追いやすいのが強みです。

## 過炭酸ナトリウムとは：中身をシンプルにした酸素系漂白剤

過炭酸ナトリウムは、水に溶けると酸素を発生させる粉末で、酸素系漂白剤の主成分そのものです。商品としては「過炭酸ナトリウム100%」「酸素系漂白剤」といった表記で売られています。

香料も界面活性剤も入っていないため、泡立ちはほとんどありません。洗濯槽クリーナー、衣類の浸け置き、食器の茶渋落としなど用途は広く、大容量パックが多いのが特徴です。

一方で、使用量や適した温度、使えない素材といった情報はパッケージの記載が中心になるため、自分で調べて使い方を決める前提の商品といえます。

## 4つの軸で徹底比較

### 1. コスパ：まとめ買いのしやすさで差がつく

同じ量を使うなら、過炭酸ナトリウムの大容量パックのほうが1gあたりの単価は下がる傾向にあります。オキシクリーンはブランド品としてのパッケージや流通が価格に乗るため、単価はやや上がります。

最新の単価はページ上部の比較表（自動更新）でご確認ください。サイズが違う商品どうしを比べるときは、価格ではなく1gあたりの単価で見るのがポイントです。

### 2. 成分：日本版オキシクリーンはかなり近い

成分で見ると、日本版オキシクリーンと過炭酸ナトリウム製品の差は小さめです。どちらも過炭酸ナトリウムが主体で、香料や柔軟成分を足していません。

差がはっきり出るのはアメリカ版オキシクリーンで、界面活性剤と香料が入るぶん、汚れ落ちの体感と香りの残り方が変わります。香りが残るのを避けたい場合は、日本版か過炭酸ナトリウムを選ぶのが無難です。

### 3. 使いやすさ：付属品と情報量はブランド品が有利

オキシクリーンは計量スプーンが付属する商品が多く、使用量の目安もパッケージやブランドサイトで案内されています。初めて浸け置き洗いをする場合は迷いにくいでしょう。

過炭酸ナトリウムはスプーンなしの商品も多く、計量は自分で用意する必要があります。袋入りが主流なので、開封後は密閉容器に移すなどの湿気対策も自分で行います。

### 4. 用途：どちらも洗濯槽掃除と浸け置きが主戦場

洗濯槽の掃除、衣類の浸け置き、食器や水筒の茶渋落としなど、想定される用途はどちらもほぼ同じです。塩素系漂白剤と違って色柄物に使えるのも共通しています。

ただし、どちらも**アルミ製品やウール・シルクなどの動物性繊維には使えません**。この注意点はブランドの違いに関係なく共通なので、素材の確認は必ず行ってください。

## タイプ別おすすめ診断

**オキシクリーンが向いている人**

- 浸け置き洗いを初めてやるので、手順を調べる手間を減らしたい
- 計量スプーン付きですぐ使い始めたい
- 泡立ちや香りがあるほうが「洗えている」と感じられる（アメリカ版）

**過炭酸ナトリウムが向いている人**

- 使い方はすでに分かっていて、コストを下げたい
- 洗濯槽掃除を定期的にやるので、大容量で持っておきたい
- 香料が残るのを避けたい

## 使い分けという選択肢

用途で分ける方法もあります。たとえば、量を多く使う洗濯槽の掃除には大容量の過炭酸ナトリウム、衣類の部分的な染み抜きなど少量を丁寧に使いたい場面には計量しやすいオキシクリーン、という分け方です。

主成分が同じなので、どちらか一方に決めなければならない理由はありません。まず少量サイズで試してから、使う量が増えてきたら大容量に切り替える、という順番も現実的です。

## まとめ

オキシクリーンと過炭酸ナトリウムは、主成分が同じ酸素系漂白剤です。日本版オキシクリーンは無香料で過炭酸ナトリウム製品に近く、アメリカ版は界面活性剤と香料が入る点が違います。

使い方をこれから覚える段階なら、情報と付属品が整っているオキシクリーン。使い方が分かっていて量を使うなら、大容量で単価を抑えられる過炭酸ナトリウム。この基準で選べば大きく外しません。最新の単価はページ上部の比較表（自動更新）で確認してください。

---

※ 価格は記事執筆時点の楽天市場での販売価格です。価格は変動する場合があります。
