---
title: "浴室排水口ヘアキャッチャー コスパ比較【2026年版】10選"
description: "浴室排水口ヘアキャッチャーを1個あたりのコストで比較。ステンレス製、ネットホルダー型、くるっと集めるタイプなどを選びます。"
category: "bath-drain-hair-catcher"
publishedAt: "2026-06-04"
articleType: "comparison"
updatedAt: "2026-08-26"
draft: false
products:
  - rank: 1
    name: "SANEI くるっポイ PH397 ユニットバス用ヘアキャッチャー"
    brand: "SANEI"
    price: 990
    capacity: "1個"
    pricePerUnit: "約990円/個"
    rating: 5
    reviewCount: 1
    features:
      - "ユニットバス用のヘアキャッチャー"
      - "渦の流れで髪の毛を集める設計"
      - "ポリプロピレン素材"
    pros:
      - "排水口まわりをシンプルに整えたい方に向く"
      - "髪の毛をまとめて捨てる動線を作りやすい"
      - "定番メーカー品を選びたい家庭で検討しやすい"
    cons:
      - "排水口の形状によって合わない場合がある"
      - "ステンレス製を探している方には向かない"
    recommendedFor: "樹脂製の定番ヘアキャッチャーを選びたい方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00ssgkn.3rdw616d.g00ssgkn.3rdw768f/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Fti-am%2F10000658%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Fti-am%2Fi%2F10000487%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/ti-am/cabinet/tuujou01/11sanei/ph397-0.jpg?_ex=128x128"
  - rank: 2
    name: "髪の毛くるくるポイ"
    brand: "ウエルスジャパン"
    price: 1030
    capacity: "1個"
    pricePerUnit: "約1030円/個"
    rating: 4.19
    reviewCount: 647
    features:
      - "浴室排水口向けのヘアキャッチャー"
      - "渦の流れで髪の毛を集める設計"
      - "抗菌加工をうたう樹脂タイプ"
    pros:
      - "髪の毛をまとめて捨てる動線を作りやすい"
      - "樹脂製で扱いやすいタイプを選びやすい"
      - "ネットを使わない候補として比較しやすい"
    cons:
      - "対応する排水口形状を確認する必要がある"
      - "ステンレス製を探している方には向かない"
    recommendedFor: "ネット不要のヘアキャッチャーを試したい方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00qbprn.3rdw6618.g00qbprn.3rdw72bd/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Fauc-tokutoku%2Fkurukurupoictype%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Fauc-tokutoku%2Fi%2F10048019%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/auc-tokutoku/cabinet/09003099/raku2/tokutoku/45803568400492set.jpg?_ex=128x128"
    offers:
      - provider: "yahoo"
        label: "Yahoo!"
        url: "https://ck.jp.ap.valuecommerce.com/servlet/referral?sid=3770852&pid=892615315&vc_url=https%3A%2F%2Fstore.shopping.yahoo.co.jp%2Ffabric-cute%2F4580356840049.html"
        matchStatus: "matched"
        updatedAt: "2026-08-25"
        price: 1150
        rating: 4.63
        reviewCount: 19
        imageUrl: "https://item-shopping.c.yimg.jp/i/j/fabric-cute_4580356840049"
        available: true
  - rank: 3
    name: "TOKAIZ ステンレス 排水口ゴミ受け ヘアキャッチャー"
    brand: "TOKAIZ"
    price: 1580
    capacity: "1個"
    pricePerUnit: "約1580円/個"
    rating: 4.22
    reviewCount: 956
    features:
      - "ステンレス製の排水口ゴミ受け"
      - "排水口ネット付きのセット"
      - "洗面台や浴室向けの小径タイプ"
    pros:
      - "ネットも一緒に用意したい方に向く"
      - "金属製の小型ゴミ受けを比較しやすい"
      - "洗面所兼用の候補として見やすい"
    cons:
      - "浴室排水口のサイズ適合を確認する必要がある"
      - "ネット不要派には付属品が余る場合がある"
    recommendedFor: "ネット付きで始めたい方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00s0w7n.3rdw68ea.g00s0w7n.3rdw7982/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Fysmya%2F80002296%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Fysmya%2Fi%2F10012656%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/ysmya/cabinet/13755558/80002296logotest1.jpg?_ex=128x128"
  - rank: 4
    name: "Magico かくれんぼネットホルダー"
    brand: "Magico"
    price: 1980
    capacity: "1個"
    pricePerUnit: "約1980円/個"
    rating: 4.66
    reviewCount: 630
    features:
      - "浴室排水口向けのネットホルダー"
      - "排水口ネットを隠しやすい構造"
      - "日本製の風呂用ゴミ受け"
    pros:
      - "ネットが見えにくい見た目を重視しやすい"
      - "ゴミ捨てをネット単位で済ませたい方に向く"
      - "浴室の排水口まわりをすっきり見せやすい"
    cons:
      - "対応する排水口形状を確認する必要がある"
      - "交換用リングやネットの管理が必要になる"
    recommendedFor: "ネット併用でも見た目を整えたい方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00rzdpn.3rdw67f2.g00rzdpn.3rdw7df7/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Fkweg%2Fmagico_drainnetholder%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Fkweg%2Fi%2F10000238%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/kweg/cabinet/items/magico/drainnetholder_v2/kakurenbo_kumogata.jpg?_ex=128x128"
  - rank: 5
    name: "HUROYAMA ヘアキャッチャー Ag+"
    brand: "HUROYAMA"
    price: 1980
    capacity: "1個"
    pricePerUnit: "約1980円/個"
    rating: 4.08
    reviewCount: 53
    features:
      - "シリコン素材のヘアキャッチャー"
      - "サイズ展開のある排水口カバー"
      - "浴室や洗面所向けのゴミ受け"
    pros:
      - "柔らかい素材感を選びたい方に向く"
      - "サイズ違いを確認しながら選びやすい"
      - "金属製以外の候補として比較しやすい"
    cons:
      - "設置場所に合うサイズ選択が必要"
      - "硬質な目皿を探している方には向かない"
    recommendedFor: "シリコン系のヘアキャッチャーを選びたい方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00s6aun.3rdw6e7e.g00s6aun.3rdw7227/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Finter-bb%2Fhy-hc%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Finter-bb%2Fi%2F10000222%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/inter-bb/cabinet/huroyama/haircatcher/sqr/hc_new_thumbnail02.jpg?_ex=128x128"
  - rank: 6
    name: "ヘアーキャッチャー お風呂の排水口用 ミューファン"
    brand: "シンカテック"
    price: 2090
    capacity: "1個"
    pricePerUnit: "約2090円/個"
    rating: 4
    reviewCount: 1
    features:
      - "市販の排水口ネットを取り付けるホルダー型"
      - "ミューファン配合素材"
      - "日本製の浴室排水口用"
    pros:
      - "ゴミをネットごと捨てたい方に向く"
      - "ネット併用で手入れの負担を抑えやすい"
      - "樹脂製で扱いやすいタイプを選びやすい"
    cons:
      - "排水口ネットを別途用意する必要がある"
      - "ネットの取り付けに慣れが必要な場合がある"
    recommendedFor: "排水口ネットを使ってゴミを捨てたい方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00q728n.3rdw6c21.g00q728n.3rdw754f/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Flivingut%2F465592%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Flivingut%2Fi%2F10210964%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/livingut/cabinet/maker_sinka1/465592.jpg?_ex=128x128"
  - rank: 7
    name: "HUBATH お風呂 マグネット ヘアーキャッチャー TT104"
    brand: "シンカテック"
    price: 2420
    capacity: "1個"
    pricePerUnit: "約2420円/個"
    rating: 4.7
    reviewCount: 2354
    features:
      - "TOTO対応サイズのヘアーキャッチャー"
      - "浴室排水口向けのステンレス製"
      - "マグネット式のごみ受け"
    pros:
      - "対応メーカーを確認して選びやすい"
      - "金属製で見た目を整えやすい"
      - "ネット不要タイプの候補にしやすい"
    cons:
      - "対応サイズ外では使えない場合がある"
      - "購入前に排水口仕様を確認したい"
    recommendedFor: "TOTO系浴室の目皿交換候補を探す方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00s7h3n.3rdw6d0e.g00s7h3n.3rdw7585/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Fsinkatec%2Fhu_thair_catcher%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Fsinkatec%2Fi%2F10000431%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/sinkatec/cabinet/sale/tt_p5_260824.jpg?_ex=128x128"
  - rank: 8
    name: "HUBATH お風呂 マグネット ヘアーキャッチャー STD140"
    brand: "シンカテック"
    price: 2530
    capacity: "1個"
    pricePerUnit: "約2530円/個"
    rating: 4.62
    reviewCount: 4780
    features:
      - "浴室排水口向けのステンレス製"
      - "マグネット式のヘアーキャッチャー"
      - "スタンダードサイズ向け設計"
    pros:
      - "金属製のごみ受けを選びたい方に向く"
      - "置くだけで使うタイプを比較しやすい"
      - "浴室の目皿交換候補にしやすい"
    cons:
      - "購入前に排水口サイズを測る必要がある"
      - "磁石が使える条件を確認したい"
    recommendedFor: "ステンレス製の標準サイズを探している方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00s7h3n.3rdw6d0e.g00s7h3n.3rdw7585/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Fsinkatec%2Fhu_mhair_catcher%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Fsinkatec%2Fi%2F10000428%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/sinkatec/cabinet/sale/cd_p5_260824.jpg?_ex=128x128"
  - rank: 9
    name: "dailysmile ステンレス 排水口カバー ヘアーキャッチャー"
    brand: "dailysmile"
    price: 2580
    capacity: "1個"
    pricePerUnit: "約2580円/個"
    rating: 4.19
    reviewCount: 210
    features:
      - "ステンレス製の排水口カバー"
      - "ネット付きのヘアーキャッチャー"
      - "浴室排水口のゴミ受けタイプ"
    pros:
      - "カバーとネットをまとめて用意しやすい"
      - "金属製の見た目を重視しやすい"
      - "掃除しやすい形状を比較できる"
    cons:
      - "付属ネットの管理が必要になる"
      - "対応サイズを事前に確認したい"
    recommendedFor: "ネット付きステンレス型を検討したい方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00un7an.3rdw60d8.g00un7an.3rdw7e7a/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Fdailysmile%2Fl5hm069%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Fdailysmile%2Fi%2F10000213%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/dailysmile/cabinet/homekitchen2/l5hm069_test.jpg?_ex=128x128"
  - rank: 10
    name: "KEEPFINE 抗菌ヘアキャッチャー 102mm用"
    brand: "KEEPFINE"
    price: 3510
    capacity: "1個"
    pricePerUnit: "約3510円/個"
    rating: 4.71
    reviewCount: 7
    features:
      - "102mm排水口向けのステンレス製"
      - "SUS304素材"
      - "置くだけで使うパンチングタイプ"
    pros:
      - "金属製のごみ受けを選びたい方に向く"
      - "ネットなしで使えるタイプを探しやすい"
      - "ユニットバスの目皿交換候補にしやすい"
    cons:
      - "対応サイズを事前に測る必要がある"
      - "樹脂製より重量感や質感の好みが分かれる"
    recommendedFor: "ステンレス製の置くだけタイプを選びたい方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00ur6mn.3rdw6a14.g00ur6mn.3rdw75cb/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Ffinelifestyle%2Fncc0302%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Ffinelifestyle%2Fi%2F10000025%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/finelifestyle/cabinet/keepfine/haircatch142kokin/imgrc0109222928.jpg?_ex=128x128"
tags:
  - "浴室排水口ヘアキャッチャー おすすめ"
  - "お風呂 排水口 ゴミ受け コスパ"
  - "ヘアキャッチャー 比較"
  - "お風呂掃除"
faqs:
  - question: "どの排水口にも使えますか？"
    answer: "商品ごとに対応サイズや形状が異なります。購入前に排水口の直径、目皿の厚み、既存カバーの形を確認してください。"
  - question: "ネットホルダー型とネット不要タイプはどちらが便利ですか？"
    answer: "ゴミをネットごと捨てたいならネットホルダー型、補充品を減らしたいならネット不要タイプが向きます。掃除の頻度や家族の使い方に合わせて選びましょう。"
  - question: "ステンレス製はサビませんか？"
    answer: "ステンレス製でも浴室用洗剤や塩素系洗剤、入浴剤の成分、汚れの残り方によってサビや変色が起きることがあります。使用後のすすぎや定期的な手入れを意識しましょう。"
---

## 買う前に測る。ここを飛ばすと全部が無駄になる

この商品カテゴリは、**性能や価格より先にサイズで決まります。** 排水口に合わなければ、どれだけ評判が良くても使えません。返品の理由もほぼこれです。

測るのは次の3か所です。

- **排水口の内径**：既存の目皿を外し、受け皿側の円の直径を測ります
- **目皿の外径**：ふちに乗せるタイプは、この外径のほうが効いてきます
- **深さ**：内径が合っていても、深すぎると底に届かず、浅すぎると排水口のフタが閉まりません

浴室の排水口は、円形の目皿を外すと下にゴミ受けが入っている二段構造が一般的です。**どの段に置き換える商品なのか**を商品説明で確認してから買います。ここを取り違えると、届いてから「入るけれど固定されない」という状態になります。

賃貸で部品を外すのが不安な場合でも、目皿は基本的に載せてあるだけなので持ち上げれば外れます。外した部品は捨てず、退去時に戻せるよう取っておきます。

## ネットを使うか、使わないかで二派に分かれる

サイズが合う商品が絞れたら、次はここです。運用がまったく変わります。

**ネットを使う（ネットホルダー型）**

- ゴミに触れずネットごと捨てられる
- 排水口そのものはほとんど汚れない
- 代わりに、ネットという消耗品を買い続けることになる
- ネットの目が詰まると水はけが落ち、こまめな交換が必要

**ネットを使わない（ごみ受け単体）**

- 補充品が不要で、買ったあとの出費がない
- 溜まった髪の毛を自分でつまんで捨てる必要がある
- 本体は洗って繰り返し使う

**「掃除の手間を減らしたい」ならネット型、「買い足しをなくしたい」ならネット不要型**です。どちらが優れているという話ではなく、髪の毛を触れるかどうかという好みの問題に近いところがあります。家族の髪が長い家庭ほど、ネット型の恩恵は大きくなります。

## 表面がなめらかなほど、掃除が楽になる

素材はステンレスと樹脂に大きく分かれます。

- **ステンレス**：見た目がすっきりし、汚れが張り付きにくいのが利点です。ただし浴室用の塩素系洗剤や入浴剤の成分が残るとサビや変色が出ることがあります。使ったあとに流しておけば大きな問題にはなりません
- **樹脂**：軽くて扱いやすく、割れにくいものが多いです。表面が細かく荒れてくると、ぬめりが付きやすくなります

素材以上に効くのが**形状**です。穴が細かすぎるものは、髪の毛が絡んで指では取れなくなります。逆に穴が大きすぎると細かいゴミが通り抜けます。カーブがなく、平らで洗いやすい形のほうが、結果的に長く使えます。

「くるっと集める」タイプのように、水流で髪の毛を中央に寄せる構造の商品もあります。捨てるときにまとめてつまめるので、掃除の負担は確実に下がります。

## 本体は消耗品ではないが、永久には持たない

ヘアキャッチャーは繰り返し使うものなので、1個あたりの価格だけで比べると判断を誤ります。見るべきは**買ったあとに何がかかるか**です。

- ネット型：本体代 ＋ ネットの補充が続く
- ネット不要型：本体代のみ

一方で、本体も永久ではありません。樹脂は劣化して割れ、ステンレスも変色します。汚れが取れなくなったら替えどきで、**そのときに買い直せる価格かどうか**も選ぶ基準に入れておくと気が楽です。高価な1個を大事に使うより、扱いやすいものを定期的に替えるほうが浴室は清潔に保てます。

## 詰まりを防ぐという意味では、これがいちばん効く

排水口の詰まりやにおいの原因は、管の中に流れ込んだ髪の毛と皮脂です。管に入ってしまえば薬剤で溶かすしかありませんが、**入る前に受け止めるほうが、あとの手間はずっと小さくて済みます**。

- 入浴のたびに、溜まった髪の毛をまとめて捨てる
- 週に一度、本体を外して洗う
- ぬめりが出てきたら、浴室用洗剤で洗う

この3つを続けているだけで、パイプクリーナーの出番はかなり減ります。詰まりが頻繁に起きている家は、キャッチャーの性能ではなく、捨てる頻度のほうに原因があることが多いです。

管の内側のケアについては [排水口クリーナーのコスパ比較](/articles/drain-cleaner-comparison/) も合わせてご覧ください。

## まとめ

浴室排水口ヘアキャッチャーは、**サイズが合うかどうかが9割**です。内径・外径・深さを測ってから候補を絞り、そのあとでネットを使うかどうかを決めます。

素材はステンレスか樹脂かよりも、洗いやすい形かどうかが日々の手間に効きます。本体は消耗品ではありませんが、劣化したら替える前提で選んでおくと清潔に保てます。

上部の比較表では1個あたりの価格で並べていますが、**まずは自宅の排水口に合う商品かどうかを確認してから**価格を見てください。

---

※ 価格は記事執筆時点の楽天市場での販売価格です。価格は変動する場合があります。
