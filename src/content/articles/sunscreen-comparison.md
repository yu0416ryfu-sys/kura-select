---
title: "日焼け止め コスパ最強ランキング【2026年版】1mLあたり最安で比較"
description: "日焼け止めを1mLあたりのコストで徹底比較。ビオレUV・スキンアクア・ニベアUV・アリィーなど人気10選をUV防御力・コスパで解説します。"
category: "sunscreen"
publishedAt: "2026-05-01"
articleType: "comparison"
updatedAt: "2026-09-23"
products:
  - rank: 1
    name: "ロート製薬 スキンアクア スーパーモイスチャージェル ポンプ SPF50+ PA"
    brand: "ロート製薬（スキンアクア）"
    price: 897
    capacity: "（140g）"
    pricePerUnit: "約6.4円/g"
    rating: 5
    reviewCount: 5
    features:
      - "4種のうるおい成分配合で保湿しながらUVケア"
      - "SPF50+/PA++++の最高レベルUV防御"
      - "せっけんで落とせる肌にやさしい処方"
    pros:
      - "保湿力が高く乾燥肌でも使いやすい"
      - "最高レベルのUVカット効果"
      - "110gの大容量でコスパが良い"
    cons:
      - "ジェル特有のモロモロが出る場合がある"
      - "真夏のアウトドアにはやや心もとない"
    recommendedFor: "乾燥が気になる方・日常使いのUVケアをしたい方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00rbnvn.3rdw6e69.g00rbnvn.3rdw7dd2/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Ftsuruha%2F10156053%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Ftsuruha%2Fi%2F10092000%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/tsuruha/cabinet/syohin90/10156053.jpg?_ex=128x128"
    offers:
      - provider: "yahoo"
        label: "Yahoo!"
        url: "https://ck.jp.ap.valuecommerce.com/servlet/referral?sid=3770852&pid=892615315&vc_url=https%3A%2F%2Fstore.shopping.yahoo.co.jp%2Fy-kojima%2F4987241190867.html"
        matchStatus: "matched"
        updatedAt: "2026-08-04"
        price: 858
        imageUrl: "https://item-shopping.c.yimg.jp/i/j/y-kojima_4987241190867"
        available: true
        rating: 4.63
        reviewCount: 8
    genreId: "503054"
  - rank: 2
    name: "花王 ニベアUV　ニベアサン ウォータージェル SPF50 詰替え"
    brand: "花王（ニベア）"
    price: 975
    capacity: "125g"
    pricePerUnit: "約7.8円/g"
    rating: 4.8
    reviewCount: 15
    features:
      - "大容量160gでたっぷり使えるポンプタイプ"
      - "ジェルタイプで伸びが良くベタつかない"
      - "SPF50/PA+++の高いUV防御力"
    pros:
      - "ポンプ式で量の調節がしやすい"
      - "160gの大容量で家族全員で使える"
      - "化粧水感覚で使えるみずみずしいテクスチャー"
    cons:
      - "汗・水に弱くこまめな塗り直しが必要"
      - "ウォータープルーフではない"
    recommendedFor: "毎日使いでコスパを重視する方・家族で共有したい方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00ru0on.3rdw60b7.g00ru0on.3rdw7edf/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Fat-life%2F103-4901301298706%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Fat-life%2Fi%2F10627469%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/at-life/cabinet/re2021/4901301298706.jpg?_ex=128x128"
    offers:
      - provider: "yahoo"
        label: "Yahoo!"
        url: "https://ck.jp.ap.valuecommerce.com/servlet/referral?sid=3770852&pid=892615315&vc_url=https%3A%2F%2Fstore.shopping.yahoo.co.jp%2Fhimejiryutsuu%2F103-4901301298706.html"
        matchStatus: "pending"
        updatedAt: "2026-09-07"
        price: 962
        imageUrl: "https://item-shopping.c.yimg.jp/i/j/himejiryutsuu_103-4901301298706"
        available: true
        rating: 0
        reviewCount: 0
    genreId: "503054"
  - rank: 3
    name: "ビオレUV アクアリッチ ウォータリーエッセンス 120g"
    brand: "花王（ビオレ）"
    price: 1269
    capacity: "120g"
    pricePerUnit: "約11円/g"
    rating: 4.57
    reviewCount: 7
    features:
      - "世界初のミクロディフェンス処方で塗りムラを防止"
      - "水感エッセンスで軽いつけ心地"
      - "スーパーウォータープルーフなのにせっけんで落とせる"
    pros:
      - "軽い使用感で化粧下地としても使える"
      - "ウォータープルーフで汗・水に強い"
      - "白浮きしにくい透明タイプ"
    cons:
      - "70gと容量がやや少なめ"
      - "グラム単価は高め"
    recommendedFor: "化粧下地としても使いたい方・汗をかく場面が多い方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00ugynn.3rdw6d73.g00ugynn.3rdw7c64/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Frakutensokuhaimart%2F4901301449450%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Frakutensokuhaimart%2Fi%2F10026575%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/rakutensokuhaimart/cabinet/rakuten24/450/4901301449450.jpg?_ex=128x128"
    offers:
      - provider: "yahoo"
        label: "Yahoo!"
        url: "https://ck.jp.ap.valuecommerce.com/servlet/referral?sid=3770852&pid=892615315&vc_url=https%3A%2F%2Fstore.shopping.yahoo.co.jp%2Fpaatner%2F4901301416957.html"
        matchStatus: "rejected"
        updatedAt: "2026-08-08"
        price: 2480
        imageUrl: "https://item-shopping.c.yimg.jp/i/j/paatner_4901301416957"
        available: true
        rating: 4
        reviewCount: 7
    genreId: "503054"
  - rank: 4
    name: "アリィー クロノビューティ トーンアップUV 01 60g"
    brand: "カネボウ（アリィー）"
    price: 2332
    capacity: "60g"
    rating: 4
    reviewCount: 9
    features:
      - "摩擦・汗・水に強いトリプルプルーフ処方"
      - "美容液成分配合でスキンケア効果"
      - "SPF50+/PA++++で最高レベルの防御力"
    pros:
      - "摩擦にも強く長時間落ちにくい"
      - "アウトドア・レジャーでも安心の防御力"
      - "ジェルタイプで伸びが良い"
    cons:
      - "価格はプレミアム帯でやや高め"
      - "落とすときは丁寧なクレンジングが必要"
    recommendedFor: "屋外でのスポーツ・レジャーが多い方・絶対焼きたくない方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00r136n.3rdw697f.g00r136n.3rdw7383/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Frakuten24%2F4973167057667%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Frakuten24%2Fi%2F11097710%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/rakuten24/cabinet/667/4973167057667.jpg?_ex=128x128"
    genreId: "503054"
    pricePerUnit: "約39円/g"
  - rank: 5
    name: "レステモ 美白BBクリーム"
    brand: "レステモ"
    price: 2560
    capacity: "35g"
    pricePerUnit: "約73円/g"
    rating: 3.97
    reviewCount: 12946
    features:
      - "UVケアに使える日焼け止め・化粧下地"
      - "顔用や子ども向けも選べる"
      - "日常使いしやすい"
    pros:
      - "毎日の紫外線対策に使いやすい"
      - "メイク前にも取り入れやすい"
      - "肌質に合わせて選べる"
    cons:
      - "色味や使用感の好みは分かれる"
      - "肌に合うか確認が必要"
    recommendedFor: "日焼け止めを使用感と用途で比較したい方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00qdaen.3rdw686c.g00qdaen.3rdw7002/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Flesthemo%2Fbb%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Flesthemo%2Fi%2F10000033%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/lesthemo/cabinet/shg/bbcream_01_02_3500px.jpg?_ex=128x128"
    offers:
      - provider: "yahoo"
        label: "Yahoo!"
        url: "https://ck.jp.ap.valuecommerce.com/servlet/referral?sid=3770852&pid=892615315&vc_url=https%3A%2F%2Fstore.shopping.yahoo.co.jp%2Flesthemo%2Fbb-cream.html"
        matchStatus: "matched"
        updatedAt: "2026-09-07"
        price: 2560
        imageUrl: "https://item-shopping.c.yimg.jp/i/j/lesthemo_bb-cream"
        available: true
        rating: 4.27
        reviewCount: 150
    genreId: "567445"
  - rank: 6
    name: "ラロッシュポゼ UVイデアXL"
    brand: "ラロッシュポゼ"
    price: 4070
    capacity: "1個"
    pricePerUnit: "約4070円/個"
    rating: 4.61
    reviewCount: 1323
    features:
      - "UVケアに使える日焼け止め・化粧下地"
      - "顔用や子ども向けも選べる"
      - "日常使いしやすい"
    pros:
      - "毎日の紫外線対策に使いやすい"
      - "メイク前にも取り入れやすい"
      - "肌質に合わせて選べる"
    cons:
      - "色味や使用感の好みは分かれる"
      - "肌に合うか確認が必要"
    recommendedFor: "日焼け止めを使用感と用途で比較したい方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00qi2mn.3rdw6aae.g00qi2mn.3rdw7998/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Fpycno%2Frarossyu35%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Fpycno%2Fi%2F10000752%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/pycno/cabinet/lrp/idea_hqs.jpg?_ex=128x128"
    genreId: "503054"
  - rank: 7
    name: "ラロッシュポゼ UVイデアXL BB"
    brand: "ラロッシュポゼ"
    price: 4070
    capacity: "1個"
    pricePerUnit: "約4070円/個"
    rating: 4.51
    reviewCount: 294
    features:
      - "UVケアに使える日焼け止め・化粧下地"
      - "顔用や子ども向けも選べる"
      - "日常使いしやすい"
    pros:
      - "毎日の紫外線対策に使いやすい"
      - "メイク前にも取り入れやすい"
      - "肌質に合わせて選べる"
    cons:
      - "色味や使用感の好みは分かれる"
      - "肌に合うか確認が必要"
    recommendedFor: "日焼け止めを使用感と用途で比較したい方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00qn2mn.3rdw6125.g00qn2mn.3rdw7702/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Fsangakushop%2Frarossyu-bb-kit%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Fsangakushop%2Fi%2F10000726%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/sangakushop/cabinet/lrp/rarossyu-bb-kit_hqs2.jpg?_ex=128x128"
    genreId: "216307"
  - rank: 8
    name: "ラロッシュポゼ UVイデアXL 色なし"
    brand: "ラロッシュポゼ"
    price: 4070
    capacity: "1個"
    pricePerUnit: "約4070円/個"
    rating: 4.6
    reviewCount: 292
    features:
      - "UVケアに使える日焼け止め・化粧下地"
      - "顔用や子ども向けも選べる"
      - "日常使いしやすい"
    pros:
      - "毎日の紫外線対策に使いやすい"
      - "メイク前にも取り入れやすい"
      - "肌質に合わせて選べる"
    cons:
      - "色味や使用感の好みは分かれる"
      - "肌に合うか確認が必要"
    recommendedFor: "日焼け止めを使用感と用途で比較したい方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00qn2mn.3rdw6125.g00qn2mn.3rdw7702/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Fsangakushop%2Frarossyu35%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Fsangakushop%2Fi%2F10000773%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/sangakushop/cabinet/lrp/03044hqs_p01.jpg?_ex=128x128"
    genreId: "503054"
  - rank: 9
    name: "レステモ ノンケミカル UVミルク"
    brand: "レステモ"
    price: 2480
    capacity: "50mL"
    pricePerUnit: "約50円/mL"
    rating: 4.33
    reviewCount: 2400
    features:
      - "紫外線吸収剤を使わないノンケミカル処方"
      - "ミルクタイプで伸ばしやすいテクスチャー"
      - "ウォータープルーフ仕様"
    pros:
      - "肌あたりが穏やかで子どもと一緒に使いやすい"
      - "日常使いから外出時まで用途を選びにくい"
      - "白浮きしにくいミルク状で顔にも使いやすい"
    cons:
      - "ノンケミカル処方は塗り直しの手間がかかる"
      - "専用クレンジングの要否は表示を確認したい"
    recommendedFor: "紫外線吸収剤を避けたい方・家族で共用したい方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00qdaen.3rdw686c.g00qdaen.3rdw7002/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Flesthemo%2Fuv50g%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Flesthemo%2Fi%2F10000061%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/lesthemo/cabinet/shg/uvmilk_03_2480y.jpg?_ex=128x128"
    genreId: "503054"
  - rank: 10
    name: "ラブミータッチ シルキーUVミルク 50g"
    brand: "ラブミータッチ"
    price: 3080
    capacity: "50g"
    rating: 4.84
    reviewCount: 3900
    features:
      - "楽天市場で取り扱いのある追加候補"
      - "日用品として使いやすい定番タイプ"
      - "既存比較にない選択肢を補える構成"
    pros:
      - "まとめ買い候補として比較しやすい"
      - "用途や好みに合わせて選びやすい"
      - "既存商品と価格帯を比べやすい"
    cons:
      - "サイズや仕様は購入前に確認が必要"
      - "ショップにより在庫や配送条件が変わる"
    recommendedFor: "既存候補以外も比較して選びたい人"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00ugd0n.3rdw6460.g00ugd0n.3rdw79b9/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Fthe-beautopia%2F20094-025%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Fthe-beautopia%2Fi%2F10000008%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/the-beautopia/cabinet/item/lovme/silky_uv/imgrc0127171164.jpg?_ex=128x128"
    offers:
      - provider: "yahoo"
        label: "Yahoo!"
        url: "https://ck.jp.ap.valuecommerce.com/servlet/referral?sid=3770852&pid=892615315&vc_url=https%3A%2F%2Fstore.shopping.yahoo.co.jp%2Fmedlf%2F20094-025.html"
        matchStatus: "matched"
        updatedAt: "2026-08-08"
        price: 3080
        rating: 4.75
        reviewCount: 16
        imageUrl: "https://item-shopping.c.yimg.jp/i/j/medlf_20094-025"
        available: true
    priceMax: 13629
    genreId: "503054"
tags:
  - "日焼け止め おすすめ"
  - "日焼け止め コスパ"
  - "日焼け止め 比較"
  - "UVケア"
  - "まとめ買い"
draft: false
faqs:
  - question: "Q1: 日焼け止めの塗り直し頻度は？"
    answer: "一般的には2〜3時間おきの塗り直しが推奨されます。汗をかいた場合やタオルで拭いた後はその都度塗り直しましょう。"
  - question: "Q2: 去年の日焼け止めは使える？"
    answer: "未開封なら約3年間使用可能です。開封済みの場合は、変色・異臭がなければ使えますが、品質が落ちている可能性があるため、できれば新しいものを使いましょう。"
  - question: "Q3: 日焼け止めと化粧下地は両方必要？"
    answer: "化粧下地効果のある日焼け止め（ビオレUVなど）を使えば1本で済みます。ただしメイクの仕上がりにこだわる場合は別途下地を使う方がきれいに仕上がります。"
---


## 日焼け止めの選び方ガイド

日焼け止めは夏だけでなく**年間を通じて使うもの**です。顔だけでも1回あたり約0.8g（パール粒2個分）、全身に塗ると1回あたり約15〜20gを消費します。毎日使うからこそグラム単価を意識した選び方が重要です。

### SPF・PAの基準

SPFは紫外線B波（シミ・日焼けの原因）、PAは紫外線A波（シワ・たるみの原因）をカットする指標です。日常使いならSPF30/PA++以上、アウトドアならSPF50+/PA++++を選びましょう。ただしSPF値が高いほど肌への負担が大きい傾向があるため、シーンに合わせて使い分けるのが理想的です。

### テクスチャーの違い

日焼け止めのテクスチャーは大きく「ジェル」「ミルク」「クリーム」「スプレー」に分かれます。使用感の好みだけでなく、塗り直しやすさやコスパにも影響するため、生活スタイルに合ったタイプを選びましょう。

## 顔用と体用は同じ1本で兼ねられない

日焼け止めは容量あたりの単価で並べると大容量が有利に見えますが、**顔に塗れるかどうかで選べる商品が変わります**。比較表を読むときは次の3点を先に決めてください。

### 紫外線吸収剤か、ノンケミカルか

吸収剤を使ったタイプは伸びがよく白浮きしにくい反面、肌が敏感な時期にはしみることがあります。ラロッシュポゼ UVイデアXL やレステモ ノンケミカル UVミルクのように**紫外線散乱剤だけで作られたタイプ**は肌当たりが穏やかで、子どもや敏感肌向けに選ばれます。単価は高めですが、合わないものを買って使わずに終われば単価はゼロ円ではなく無限大です。

### 化粧下地を兼ねるか

BBクリームタイプや色つきのUVイデアXL BB は下地とファンデーションを兼ねられるので、**メイク工程がひとつ減ります**。一方、色なしタイプは体にも使えます。「顔だけ・下地兼用」で選ぶか「全身・大容量」で選ぶかで、比較すべき相手が変わります。

### ウォータープルーフが要る場面か

汗や水に強い処方は落ちにくいぶん、専用のクレンジングが必要になる製品があります。屋内中心の生活で毎日使うなら、石けんで落とせる表記のあるタイプのほうが手間もコストも小さく収まります。**プールや炎天下の外出がある日だけ強いものを使う**、と使い分けるのが現実的です。

### 詰め替えがあるか

ニベアUV ウォータージェルのように詰め替え用が用意されている製品は、2本目以降の単価が下がります。長く同じ銘柄を使うつもりなら、詰め替えの有無は年間コストにそのまま効いてきます。

## 1本の持ちは「塗る量」で決まる

容量と価格から出した単価は、**規定量を塗った場合の持ち**を表していません。日焼け止めは推奨量より薄く塗ると表示されたSPF・PAの性能が出ないため、節約のつもりで薄く延ばすと、塗っている意味が薄れます。

- **顔だけでも1回に必要な量は少なくありません**。頬・額・鼻・あごに分けて置いてから伸ばすと、量の感覚がつかめます
- **2〜3時間ごとの塗り直しが前提**。汗をかく日や屋外で過ごす日は、1日に何度も使うので消費は一気に増えます
- **体に使うと消費は桁が変わります**。腕・脚まで塗る使い方なら、顔用の小容量では数日でなくなります

### 開封後は1シーズンで使い切る

日焼け止めは開封すると分離や酸化が進み、伸びや使用感が変わってきます。**去年の残りを翌年に持ち越すのは避けたい**タイプの消耗品です。大容量を買うなら、そのシーズン中に使い切れる量かどうかで判断してください。分離しているもの、においが変わったものは無理に使わないほうが安全です。

### 保管場所で劣化が早まる

車のダッシュボードやベランダなど、高温になる場所に置いたままにすると中身の状態が変わります。持ち歩き用は小容量、家に置く分は大容量、と分けると使い切りやすくなります。

**具体的な価格・容量・1本あたりの単価は、ページ上部の比較表（自動更新）でご確認ください**。

## まとめ

日焼け止めは「毎日使うならグラム単価重視」「アウトドアなら防御力重視」で選ぶのがポイントです。コスパ最優先なら大容量のニベアUVウォータージェル160gが圧倒的。汗・水に強い高性能を求めるならビオレUVアクアリッチやアリィーがおすすめです。年間通じて使うものなので、まとめ買いで賢くUVケアしましょう。

---

※ 価格は記事執筆時点の楽天市場での販売価格です。価格は変動する場合があります。
