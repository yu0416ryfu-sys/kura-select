---
title: "衣類・布用消臭スプレー コスパ最強ランキング【2026年版】除菌力・消臭力で比較9選"
description: "衣類・布用消臭スプレーを1プッシュあたりのコストで徹底比較。ファブリーズ・リセッシュ・A2Care・ハイジアなど人気9選を消臭力で解説します。"
category: "sanitizing-spray"
publishedAt: "2026-05-01"
articleType: "comparison"
updatedAt: "2026-08-16"
products:
  - rank: 1
    name: "リセッシュ 除菌EX デオドラントパワー 香りが残らないタイプ 本体 360mL"
    brand: "花王（リセッシュ）"
    price: 437
    capacity: "360mL"
    pricePerUnit: "約1.2円/mL"
    rating: 5
    reviewCount: 2
    features:
      - "99.9%除菌＆ウイルス除去（エンベロープタイプ）"
      - "汗臭・加齢臭・タバコ臭をしっかり消臭"
      - "6時間後も消臭効果が持続する長時間処方"
    pros:
      - "1プッシュあたり約0.47円で圧倒的コスパ"
      - "大容量詰め替えで買い足し頻度が減る"
      - "除菌・消臭ともに高い効果"
    cons:
      - "香りの好みが分かれる"
      - "本体スプレーボトルが別途必要"
    recommendedFor: "コスパ重視の方・大量に使う方・加齢臭対策に・部屋干し臭対策に"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00qtgwn.3rdw6d4e.g00qtgwn.3rdw7c15/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Fcotacota%2Fg-4901301349033%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Fcotacota%2Fi%2F10036436%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/cotacota/cabinet/2024-g/4901301349033.jpg?_ex=128x128"
    offers:
      - provider: "yahoo"
        label: "Yahoo!"
        url: "https://ck.jp.ap.valuecommerce.com/servlet/referral?sid=3770852&pid=892615315&vc_url=https%3A%2F%2Fstore.shopping.yahoo.co.jp%2Fy-kojima%2F4901301349033.html"
        matchStatus: "matched"
        updatedAt: "2026-08-01"
        price: 363
        imageUrl: "https://item-shopping.c.yimg.jp/i/j/y-kojima_4901301349033"
        available: true
        rating: 0
        reviewCount: 1
  - rank: 2
    name: "ファブリーズ W除菌+消臭 詰め替え 1280ml×2セット"
    brand: "P&G（ファブリーズ）"
    price: 3297
    capacity: "1280ml×2セット（2,560ml）"
    pricePerUnit: "約1.3円/ml"
    rating: 4.64
    reviewCount: 11
    features:
      - "独自のW除菌成分で菌を99.9%除去"
      - "トウモロコシ由来の消臭成分で臭いを分解"
      - "スーツ・カーテン・ソファなど布製品全般に使用可"
    pros:
      - "知名度No.1ブランドの安心感"
      - "臭いを包み込むのではなく分解する消臭メカニズム"
      - "無香料タイプもあり香り嫌いの方にも対応"
    cons:
      - "リセッシュと比べるとやや割高"
      - "噴霧が粗く感じることがある"
    recommendedFor: "ブランド重視の方・臭いを根本から消したい方・無香料派の方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00r136n.3rdw697f.g00r136n.3rdw7383/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Frakuten24%2F598757%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Frakuten24%2Fi%2F11321967%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/rakuten24/cabinet/757/598757.jpg?_ex=128x128"
  - rank: 3
    name: "ランドリン ファブリックミスト クラシックフローラル 370mL"
    brand: "ネイチャーラボ（ランドリン）"
    price: 602
    capacity: "370mL"
    pricePerUnit: "約1.6円/mL"
    rating: 4.8
    reviewCount: 45
    features:
      - "W除菌・消臭機能とフレグランスを兼ね備えたファブリックミスト"
      - "オーガニック植物エキス配合のクラシックフローラルの香り"
      - "衣類と部屋の両方に使えるスプレータイプ"
    pros:
      - "消臭・除菌しながら香りをプラスできる一体型設計"
      - "植物エキス配合で肌への刺激を抑えたい方に向く"
      - "ランドリン柔軟剤と香りを統一して使いやすい"
    cons:
      - "香りが強めのため無香料を好む方には向かない"
      - "ファブリーズなど主流ブランドと比べて流通量がやや少ない"
    recommendedFor: "消臭と香りを同時に楽しみたい方・ランドリンシリーズを使っている方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00sxiin.3rdw647c.g00sxiin.3rdw7851/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Fnaturelab%2F4582469501411%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Fnaturelab%2Fi%2F10003087%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/naturelab/cabinet/naturelab/08143526/4582469501411.jpg?_ex=128x128"
    offers:
      - provider: "yahoo"
        label: "Yahoo!"
        url: "https://ck.jp.ap.valuecommerce.com/servlet/referral?sid=3770852&pid=892615315&vc_url=https%3A%2F%2Fstore.shopping.yahoo.co.jp%2Ftoku00301%2Fpbtuqj2ih5k3phffpwr6vs7ywy.html"
        matchStatus: "matched"
        updatedAt: "2026-07-28"
        price: 617
        rating: 0
        reviewCount: 2
        imageUrl: "https://item-shopping.c.yimg.jp/i/j/toku00301_pbtuqj2ih5k3phffpwr6vs7ywy"
        available: true
  - rank: 4
    name: "フレナチュラ 除菌スプレー 消臭スプレー 400mL"
    brand: "フレナチュラ"
    price: 2200
    capacity: "400mL"
    pricePerUnit: "約5.5円/mL"
    rating: 4.49
    reviewCount: 505
    features:
      - "99%除菌効果の無香料タイプ"
      - "速乾性処方で衣類が乾きやすい"
      - "カーテン・ソファ・ベッドなど布製品全般に使用可"
    pros:
      - "無香料なので香りが苦手な方や香水を使う方にも向く"
      - "衣類だけでなくインテリア布製品にも幅広く使える"
      - "速乾性で衣類が濡れたままにならない"
    cons:
      - "価格情報の確認が必要"
      - "無香料のため消臭効果の実感がわかりにくい場合がある"
    recommendedFor: "無香料派の方・衣類以外の布製品にも消臭したい方・香り残りを避けたい方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00rvz8n.3rdw6c0a.g00rvz8n.3rdw7b90/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Fauc-seiketu%2Ffrais_natula_c3%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Fauc-seiketu%2Fi%2F10001146%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/auc-seiketu/cabinet/shohin01/05356845/imgrc0071384228.jpg?_ex=128x128"
    offers:
      - provider: "yahoo"
        label: "Yahoo!"
        url: "https://ck.jp.ap.valuecommerce.com/servlet/referral?sid=3770852&pid=892615315&vc_url=https%3A%2F%2Fstore.shopping.yahoo.co.jp%2Fseiketu-online%2Fnew-frais-natula-350-set.html"
        matchStatus: "matched"
        updatedAt: "2026-07-29"
        price: 3630
        imageUrl: "https://item-shopping.c.yimg.jp/i/j/seiketu-online_new-frais-natula-350-set"
        available: true
        rating: 0
        reviewCount: 1
  - rank: 5
    name: "フレナチュラ グレージュ カラーボトル 400mL"
    brand: "フレナチュラ"
    price: 2200
    capacity: "400mL"
    pricePerUnit: "約5.5円/mL"
    rating: 4.38
    reviewCount: 309
    features:
      - "インテリアになじむグレージュカラーのボトルデザイン"
      - "99%除菌対応の無臭処方"
      - "キッチン・トイレ・ペット用品など多用途に使用可"
    pros:
      - "おしゃれなボトルデザインで洗面台や玄関に出しっぱなしにできる"
      - "無臭タイプで使用場所を選ばない"
      - "ペットのいる家庭でも使いやすい処方"
    cons:
      - "価格情報の確認が必要"
      - "デザイン重視のためコスパより見た目優先の商品"
    recommendedFor: "インテリアにこだわる方・見た目にもこだわりたい方・ペットのいる家庭"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00rvz8n.3rdw6c0a.g00rvz8n.3rdw7b90/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Fauc-seiketu%2Ffrais_natula%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Fauc-seiketu%2Fi%2F10001136%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/auc-seiketu/cabinet/shohin01/05356845/06732711/imgrc0075825306.jpg?_ex=128x128"
    offers:
      - provider: "yahoo"
        label: "Yahoo!"
        url: "https://ck.jp.ap.valuecommerce.com/servlet/referral?sid=3770852&pid=892615315&vc_url=https%3A%2F%2Fstore.shopping.yahoo.co.jp%2Fseiketu-online%2Fnew-frais-natula-350-set.html"
        matchStatus: "matched"
        updatedAt: "2026-08-16"
        price: 3630
        imageUrl: "https://item-shopping.c.yimg.jp/i/j/seiketu-online_new-frais-natula-350-set"
        available: true
        rating: 0
        reviewCount: 1
  - rank: 6
    name: "K・ガード プラスnine 500mLスプレー"
    brand: "K・ガード"
    price: 3080
    capacity: "500mL"
    pricePerUnit: "約6.2円/mL"
    rating: 4.51
    reviewCount: 634
    features:
      - "次亜塩素酸ナトリウムを有効成分とした除菌消臭スプレー"
      - "衣類・布製品・ペット用品など幅広い用途に対応"
      - "スプレータイプで素早く噴霧できる"
    pros:
      - "次亜塩素酸ナトリウム系で高い除菌効果"
      - "500mLの使いやすいサイズ感"
      - "ペットのいる家庭にも使いやすい処方"
    cons:
      - "次亜塩素酸系のため素材によっては注意が必要"
      - "知名度がやや低く入手場所が限られる"
    recommendedFor: "ペットのいる家庭・高い除菌効果を求める方・幅広い用途で使いたい方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00r7p2n.3rdw6864.g00r7p2n.3rdw7093/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Fminna-kaigo%2Fk-guard%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Fminna-kaigo%2Fi%2F10000942%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/minna-kaigo/cabinet/kguard/kguardnew01.jpg?_ex=128x128"
    offers:
      - provider: "yahoo"
        label: "Yahoo!"
        url: "https://ck.jp.ap.valuecommerce.com/servlet/referral?sid=3770852&pid=892615315&vc_url=https%3A%2F%2Fstore.shopping.yahoo.co.jp%2Foasismse%2F342501-500.html"
        matchStatus: "matched"
        updatedAt: "2026-08-16"
        price: 2772
        imageUrl: "https://item-shopping.c.yimg.jp/i/j/oasismse_342501-500"
        available: true
        rating: 0
        reviewCount: 0
  - rank: 7
    name: "スリーフ 除菌スプレー 500ml＋1L"
    brand: "Sourif（スリーフ）"
    price: 3200
    capacity: "500ml＋1L"
    pricePerUnit: "約6.4円/ml"
    rating: 4.54
    reviewCount: 1054
    features:
      - "本体500mLと詰め替え1000mLのセット販売"
      - "速乾性の高いスプレータイプ"
      - "衣類・布製品への消臭除菌に対応"
    pros:
      - "本体と大容量詰め替えがセットでそのまま使い始められる"
      - "詰め替え用で繰り返し使いやすい設計"
      - "まとめ買いで買い足し頻度を減らせる"
    cons:
      - "大容量のため保管スペースが必要"
      - "初期コストが高め"
    recommendedFor: "まとめ買いで手間を減らしたい方・詰め替え運用でコスト管理したい方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00sughn.3rdw62df.g00sughn.3rdw7388/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Fcubic-square%2F10000009%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Fcubic-square%2Fi%2F10000011%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/cubic-square/cabinet/mem_item/asrk/daigae/10000009btl-line.jpg?_ex=128x128"
    offers:
      - provider: "yahoo"
        label: "Yahoo!"
        url: "https://ck.jp.ap.valuecommerce.com/servlet/referral?sid=3770852&pid=892615315&vc_url=https%3A%2F%2Fstore.shopping.yahoo.co.jp%2Fcselect%2Fsourif-500-1000-set.html"
        matchStatus: "review"
        updatedAt: "2026-08-09"
        price: 3200
        imageUrl: "https://item-shopping.c.yimg.jp/i/j/cselect_sourif-500-1000-set"
        available: false
        rating: 0
        reviewCount: 0
        matchNotes: "capacity不一致: capacity不一致"
  - rank: 8
    name: "A2Care 除菌消臭スプレー 300mL"
    brand: "A2Care"
    price: 2398
    capacity: "300mL（約600回分）"
    pricePerUnit: "約8.0円/mL"
    rating: 4.49
    reviewCount: 452
    features:
      - "ANA国際線ファーストクラスで採用実績"
      - "MA-T（二酸化塩素系）で強力除菌"
      - "無色・無臭で衣類に残らない"
    pros:
      - "航空会社採用の高い信頼性"
      - "完全無臭で衣類の香りを邪魔しない"
      - "赤ちゃん・ペットのいる家庭でも安心"
    cons:
      - "1プッシュあたり約2.2円と最も高い"
      - "入手しにくいことがある"
    recommendedFor: "品質最優先の方・無臭にこだわる方・赤ちゃん・ペットのいる家庭"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00thfbn.3rdw6868.g00thfbn.3rdw7b07/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Fa2care-official%2F10000003%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Fa2care-official%2Fi%2F10000003%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/a2care-official/cabinet/ef/kago/2607_300ml.jpg?_ex=128x128"
    offers:
      - provider: "yahoo"
        label: "Yahoo!"
        url: "https://ck.jp.ap.valuecommerce.com/servlet/referral?sid=3770852&pid=892615315&vc_url=https%3A%2F%2Fstore.shopping.yahoo.co.jp%2Fantena5%2Fa2care300ml.html"
        matchStatus: "matched"
        updatedAt: "2026-08-02"
        price: 2618
        imageUrl: "https://item-shopping.c.yimg.jp/i/j/antena5_a2care300ml"
        available: true
        rating: 4.67
        reviewCount: 6
  - rank: 9
    name: "シュシュキッキ リフレッシャー除菌消臭ミスト 20mL"
    brand: "ナカノザダイレクト"
    price: 550
    capacity: "20mL"
    pricePerUnit: "約28円/mL"
    rating: 4.26
    reviewCount: 945
    features:
      - "100%植物由来成分使用の日本製処方"
      - "無臭タイプでスーツやマスクにも使いやすい"
      - "携帯サイズで持ち運びに便利"
    pros:
      - "カバンに入れて外出先でもすぐ使える携帯サイズ"
      - "植物由来成分で肌に触れる衣類にも安心"
      - "完全無臭で衣類の香りを変えない"
    cons:
      - "20mLと容量が少なく自宅使いにはコスパが低い"
      - "美容室専売品のため購入経路が限られる"
    recommendedFor: "外出先での応急消臭に・スーツやジャケットを持ち歩きケアしたい方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00ptlfn.3rdw66e3.g00ptlfn.3rdw7472/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Fnakano-d%2Fkesisaru-20%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Fnakano-d%2Fi%2F10015727%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/nakano-d/cabinet/brand/bracebe/pic/chouchoukikki_20_1.jpg?_ex=128x128"
tags:
  - "消臭スプレー"
  - "ファブリーズ"
  - "リセッシュ"
  - "コスパ"
  - "節約"
  - "日用品"
draft: false
faqs:
  - question: "ファブリーズとリセッシュ、どっちが良い？"
    answer: "消臭力はほぼ互角です。コスパで選ぶなら**リセッシュ**、ブランド力や無香料タイプの豊富さで選ぶなら**ファブリーズ**がおすすめ。楽天での実売価格はリセッシュがやや安い傾向です。"
  - question: "消臭スプレーは衣類にシミになる？"
    answer: "通常の使用量ではシミになりにくいですが、シルクやレーヨンなど**水に弱い素材**には注意が必要です。初めて使う衣類は目立たない部分で試し吹きしてから使いましょう。"
  - question: "消臭スプレーだけで洗濯は不要？"
    answer: "消臭スプレーは**洗濯の代わりにはなりません**。汗や皮脂は蓄積するため、定期的な洗濯は必要です。スーツのように頻繁に洗えない衣類の「つなぎ」として使うのが正しい使い方です。"
---

## においの正体は菌なので、香りでは消えない

布のにおいがどこから来ているかを知ると、商品の選び方が変わります。**衣類やソファのにおいは、汗や皮脂を栄養にして繁殖した菌が出しているもの**です。においそのものが布に染み付いているわけではありません。

ここから2つのことが言えます。

- **香りをかぶせるタイプでは、時間が経つと元に戻る**。香りが飛べば、菌はそのまま残っています
- **効くのは、においの分子を分解するか、原因の菌を減らすタイプ**。パッケージの「消臭」「除菌」という表示は、この違いを指しています

安い商品が悪いという話ではなく、**香りづけが目的の商品と、においを断つ目的の商品は別物**だということです。買う前に、自分がどちらを求めているのかを決めておくと失敗しません。寝具やソファのにおいが気になって探しているなら、香りの強さで選ぶと外します。

## 方式の違いを、選ぶ基準として使う

布用スプレーは、においへの効き方でおおまかに分かれます。

| 方式 | やっていること | 向いている場面 |
|------|--------|--------|
| 分解型 | におい分子を包み込む、または化学的に変える | 焼肉や煙など、その場で消したいにおい |
| 除菌型 | においを出す菌を減らす | 汗のにおいが繰り返し出る衣類・寝具 |
| 香り付与型 | 香りを足す | においではなく気分を変えたいとき |

多くの商品は分解と除菌の両方をうたっています。**繰り返し同じにおいが戻ってくる場所には除菌寄り、一時的なにおいには分解寄り**、と考えると選びやすくなります。

なお「除菌」は無菌になるという意味ではありません。試験の条件下で菌が減ったことを示す表示なので、洗濯の代わりにはならないと考えておくのが正しい距離感です。

## 濡らしすぎると、逆ににおう

使い方の失敗は、ほぼ量の問題です。においが強いほど多くかけたくなりますが、これが裏目に出ます。

- **布が乾かないまま放置されると、菌が増える**。においを取るつもりが、条件を与えることになります
- **乾いていない状態で畳んだりしまったりしない**。クローゼットの中は乾きにくく、生乾きのにおいに変わります
- **かけたあとは風を通す**。窓を開ける、換気扇を回す、ハンガーにかけて時間を置く

適量は「表面が湿る程度」です。びしょびしょになるまでかける必要はありません。**多くかけるより、乾かす時間を取るほうが結果が出ます。**

シミも量に関係します。とくに色の濃い布や、水に弱い素材では輪ジミが残ることがあります。初めての布に使うときは、目立たない場所で試してから広げてください。

## 場所ごとに、向き不向きがある

同じスプレーでも、かける相手によって効きが変わります。

- **スーツ・制服**：毎回洗えないものにいちばん向いています。帰宅後にかけて、翌朝までハンガーで乾かす流れが基本です
- **ソファ・カーテン**：面積が広いので消費が早くなります。詰め替えのある商品を選ぶかどうかが効いてくる場所です
- **寝具・マットレス**：湿気がこもりやすいので、かけすぎ厳禁。天日干しや除湿と組み合わせないと逆効果になります
- **靴**：靴用の専用品があるほど条件が厳しい場所です。布用でも使えますが、乾く時間を必ず取ります
- **子どもやペットが触れるもの**：成分表示と使用上の注意を先に確認します

**面積の広いものに使い始めると、消費量が一気に増えます。** 使う場所が増えそうなら、最初から詰め替えのある商品を選んでおくほうが後で楽です。

## 本体か詰め替えかは、使う場所の数で決まる

この商品は、スプレーボトル付きの本体と、詰め替え用の大容量に分かれています。

- **1か所だけで使う**：本体1本で足ります。使い切ってから詰め替えを検討すれば十分です
- **家じゅうで使う**：本体を複数持ち、詰め替えで補充する形が扱いやすくなります。玄関、寝室、洗面所と分けて置けば、使うたびに取りに行かずに済みます
- **試している段階**：小容量から始めます。香りが合わないと大容量が丸ごと余ります

詰め替えは単価が下がりますが、**使い切れる量かどうかが先**です。開封後に何年も置いておくものではないので、消費ペースが遅い家庭では大容量が有利とは限りません。

## まとめ

布用の消臭スプレーは、洗濯の代わりではなく、**洗えないものを次に洗うまで持たせるための道具**です。においの原因は菌なので、香りで隠すタイプでは戻ります。

使うときは量を控えめにして、乾かす時間を確保する。これだけで結果がかなり変わります。広い場所で使うなら詰め替えのある商品を、まだ試している段階なら小容量を選ぶのが無駄になりません。

上部の比較表では容量あたりの価格で並べています。詰め替えに切り替えるかどうかは、どこで何回使うかが見えてから決めれば十分です。

> ※ 掲載価格は執筆時点での楽天市場における販売価格です。価格は変動する場合があります。最新の価格は各商品ページでご確認ください。
