---
title: "ボールペン コスパ最強ランキング【2026年版】1本あたり最安で比較"
description: "ボールペンを書き心地・インク・デザインで徹底比較。uni・ZEBRA・Parker・CROSSなど人気10選を紹介。名入れの注意点と替え芯の入手しやすさまで解説します。"
category: "ballpoint-pen"
publishedAt: "2026-05-06"
articleType: "comparison"
updatedAt: "2026-09-05"
draft: false
products:
  - rank: 1
    name: "ハーバリウムボールペン 本体 10本セット 替え芯付き"
    brand: "リトルハンズ"
    price: 1400
    capacity: "10本セット（替え芯付き）"
    pricePerUnit: "約140円/本"
    rating: 4.59
    reviewCount: 285
    features:
      - "150色以上から選べるハーバリウムデザインの装飾ボールペン10本セット"
      - "替え芯付きで長く使えるコスパ設計、送料無料"
      - "手作りキット・カスタマイズ対応でオリジナルギフトに最適"
    pros:
      - "150色以上の豊富なカラー展開でイベント・記念品に個性を出せる"
      - "10本まとめ買いでウェディング・卒業イベントの大量配布に便利"
      - "評価4.83・2,400件超の高評価で満足度が証明されている"
    cons:
      - "書き心地より装飾性を重視した設計のため実用向けには不向き"
      - "0.7mm油性インクのみで軸径・インク色の選択肢がない"
    recommendedFor: "ウェディング・イベント記念品を探している方・個性的なフラワーデザインを希望する方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00srmbn.3rdw681f.g00srmbn.3rdw7f24/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Fdenshitabaco%2Fherbarium-ballpen-10%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Fdenshitabaco%2Fi%2F10000566%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_gold/denshitabaco/image02/herbarium-pen-new/10set.jpg?_ex=128x128"
  - rank: 2
    name: "ジェットストリーム 名入れ 多機能ボールペン 限定"
    brand: "uni（三菱鉛筆）"
    price: 1980
    capacity: "1本（4色ボールペン+シャープペンシル、0.5mm）"
    pricePerUnit: "約1980円/本"
    rating: 4.74
    reviewCount: 716
    features:
      - "限定カラー展開の4色ボールペン＋シャープ0.5mmの多機能タイプ（MSXE5-1000-05）"
      - "名前刻印サービス付きで卒団・卒業祝いなどの特別な記念品に対応"
      - "評価4.80・4,900件超の高評価を獲得した人気限定モデル"
    pros:
      - "限定色で特別感を演出でき、記念品としての印象が高まる"
      - "評価4.80と高評価で品質の安定性が証明されている"
      - "名入れ無料サービス付きでプレゼントとしてのコスパが高い"
    cons:
      - "限定品のため在庫がなくなり次第終了となる可能性がある"
      - "名入れ後は返品・交換不可"
    recommendedFor: "限定カラーで個性を出したい方・特別な卒団・卒業記念品を贈りたい方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00qbsyn.3rdw622d.g00qbsyn.3rdw79de/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Fbugyo%2Fe06-21-660%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Fbugyo%2Fi%2F10006340%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/bugyo/cabinet/icon/sale/e06-21-660_d5p3.jpg?_ex=128x128"
  - rank: 3
    name: "パーカー IM ボールペン"
    brand: "Parker（パーカー）"
    price: 3470
    capacity: "1本（替え芯対応）"
    pricePerUnit: "約3470円/本"
    rating: 4.73
    reviewCount: 2839
    features:
      - "創業135年以上の英国老舗ブランド・パーカーのエントリーモデル"
      - "長さ140mm・直径11mm・重さ27gのバランスの良い設計"
      - "Mサイズ替え芯対応で長期間使用可能"
    pros:
      - "英国ブランドの信頼性と品質感がコスパよく手に入る"
      - "ビジネス・プレゼント用途に最適なギフトボックス付き"
      - "複数カラー展開でシーンに合わせた選択が可能"
    cons:
      - "純正替え芯が国産ペンと比べてやや高額"
      - "油性インクのためゲルモデルよりインクのくっきり感が劣る場合がある"
    recommendedFor: "海外ブランドの品質感を求める方・ビジネスギフトを探している方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00rviqn.3rdw6978.g00rviqn.3rdw793e/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Fnaireya%2Fparkerim%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Fnaireya%2Fi%2F10000000%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/naireya/cabinet/item/parker/parker-bp-im-mbk.jpg?_ex=128x128"
  - rank: 4
    name: "ジェットストリーム 名入れ 多機能ボールペン 新色"
    brand: "uni（三菱鉛筆）"
    price: 1000
    capacity: "1本（4色ボールペン+シャープペンシル）"
    rating: 4.78
    reviewCount: 13613
    features:
      - "2025年新色展開の4色油性ボールペン＋シャープペンシル搭載多機能タイプ（0.38/0.5/0.7mm選択可）"
      - "名前刻印サービス付きで卒業・卒団・誕生日などの記念品に対応"
      - "ジェットストリーム低摩擦油性インク採用で滑らかな書き心地"
    pros:
      - "新色展開で他と差別化できる個性的な1本になる"
      - "1本で4色＋シャープを網羅し、持ち歩きを減らせる"
      - "評価4.78・13,000件超の高評価で品質が保証済み"
    cons:
      - "名入れ後は返品・交換不可のため注文前の確認が必要"
      - "多機能ペンのため単機能ペンより軸が太め"
    recommendedFor: "新色で個性を出したい方・卒業・誕生日の記念品を探している方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00qbsyn.3rdw622d.g00qbsyn.3rdw79de/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Fbugyo%2Fb06-21-277%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Fbugyo%2Fi%2F10004467%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/bugyo/cabinet/icon/sale/b06-21-277_d5p3.jpg?_ex=128x128"
    offers:
      - provider: "yahoo"
        label: "Yahoo!"
        price: 870
        url: "https://ck.jp.ap.valuecommerce.com/servlet/referral?sid=3770852&pid=892615315&vc_url=https%3A%2F%2Fstore.shopping.yahoo.co.jp%2Fakishimado%2Fnashimsxe5-1000soukomi.html"
        imageUrl: "https://item-shopping.c.yimg.jp/i/j/akishimado_nashimsxe5-1000soukomi"
        available: true
        matchStatus: "rejected"
        updatedAt: "2026-05-22"
    priceMax: 1627
  - rank: 5
    name: "PARKER IM ボールペン"
    brand: "PARKER"
    price: 2680
    capacity: "1本"
    rating: 4.57
    reviewCount: 1323
    features:
      - "本体タイプのボールペンで日常の筆記に使いやすい"
      - "まとめ買いやギフト用途でも選びやすい"
      - "職場や学校で使いやすいシンプルな仕様"
    pros:
      - "普段使いに取り入れやすい"
      - "用途に合わせて選びやすい"
      - "ストックや配布用にも向く"
    cons:
      - "色や芯径は購入時に確認が必要"
      - "価格は更新タイミングで変わる"
    recommendedFor: "仕事や学習用のボールペンを追加で比較したい方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00pzren.3rdw6689.g00pzren.3rdw7116/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Fpenroom%2Fparker0028%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Fpenroom%2Fi%2F10016658%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/penroom/cabinet/parker3/1900_r.jpg?_ex=128x128"
    offers:
      - provider: "yahoo"
        label: "Yahoo!"
        price: 6567
        url: "https://ck.jp.ap.valuecommerce.com/servlet/referral?sid=3770852&pid=892615315&vc_url=https%3A%2F%2Fstore.shopping.yahoo.co.jp%2Fnaireya%2Fparker-refillbox-im.html"
        imageUrl: "https://item-shopping.c.yimg.jp/i/j/naireya_parker-refillbox-im"
        available: true
        matchStatus: "rejected"
        updatedAt: "2026-05-22"
    priceMax: 5225
  - rank: 6
    name: "木製ボールペン 名入れ ケースセット"
    brand: "福来館"
    price: 3300
    capacity: "1本+ケース1個のセット"
    rating: 4.74
    reviewCount: 2662
    features:
      - "天然木素材を使用した温かみのあるデザインのボールペン＆ケースセット"
      - "10文字までの名入れ刻印に対応し、送料無料で届けられる"
      - "男女問わず使えるシンプルなデザインで幅広いプレゼント用途に対応"
    pros:
      - "木製素材ならではの高級感と温もりが他のペンと差別化できる"
      - "ケース付きセットでプレゼントとしての完成度が高い"
      - "評価4.74・2,600件超の高評価で贈り物としての実績が豊富"
    cons:
      - "木製素材のため水濡れや強い衝撃には注意が必要"
      - "価格がジェットストリーム系より高めのため予算確認が必要"
    recommendedFor: "ナチュラル・木製雑貨が好きな方・退職祝いや卒業記念品に特別感を出したい方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00pus9n.3rdw6c57.g00pus9n.3rdw7ed1/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Ffukuraikan%2F3502-01%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Ffukuraikan%2Fi%2F10020594%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/fukuraikan/cabinet/12089717/imgrc0145999489.jpg?_ex=128x128"
    priceMax: 3600
  - rank: 7
    name: "CROSS ATX 名入れボールペン"
    brand: "CROSS"
    price: 4700
    capacity: "1本"
    rating: 4.74
    reviewCount: 42
    features:
      - "本体タイプのボールペンで日常の筆記に使いやすい"
      - "まとめ買いやギフト用途でも選びやすい"
      - "職場や学校で使いやすいシンプルな仕様"
    pros:
      - "普段使いに取り入れやすい"
      - "用途に合わせて選びやすい"
      - "ストックや配布用にも向く"
    cons:
      - "色や芯径は購入時に確認が必要"
      - "価格は更新タイミングで変わる"
    recommendedFor: "仕事や学習用のボールペンを追加で比較したい方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00tlg5n.3rdw6c53.g00tlg5n.3rdw7b17/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Fshoppress%2Fcrossn882%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Fshoppress%2Fi%2F10000169%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/shoppress/cabinet/cross/crossn882a_1.jpg?_ex=128x128"
    priceMax: 4950
  - rank: 8
    name: "ジェットストリーム 4&1 多機能ボールペン 名入れ 0.5mm"
    brand: "uni（三菱鉛筆）"
    price: 880
    capacity: "-"
    rating: 4.53
    reviewCount: 184
    features:
      - "4色油性ボールペン＋シャープペンシル搭載の多機能タイプ（0.38/0.5/0.7mm選択可）"
      - "名前刻印サービス付きでギフト・記念品に最適"
      - "ジェットストリーム独自の低摩擦油性インクで滑らかな書き心地"
    pros:
      - "1本で多機能をカバーし、持ち歩きを省略できる"
      - "名入れ刻印対応で卒業・就職祝いなどのプレゼントに最適"
      - "国内屈指の人気シリーズ・ジェットストリームの安定した品質"
    cons:
      - "名入れ後は返品・交換不可のため注文前の確認が必要"
      - "多機能ペンのため単機能ペンより軸が太く重い"
    recommendedFor: "プレゼント・記念品を探している方・1本で多機能をまとめたい方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00q656n.3rdw61ae.g00q656n.3rdw7d60/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Fbunbougu-shibuya%2Fmsxe5-1000-05n%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Fbunbougu-shibuya%2Fi%2F10074288%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/bunbougu-shibuya/cabinet/73/msxe5-1000n_.jpg?_ex=128x128"
    priceMax: 1320
  - rank: 9
    name: "ジェットストリーム 4+1 MSXE5-1000 名入れ無料"
    brand: "uni（三菱鉛筆）"
    price: 1000
    capacity: "-"
    pricePerUnit: "-"
    rating: 4.55
    reviewCount: 250
    features:
      - "MSXE5-1000正規品に名入れ無料サービスが込みで1,000円台"
      - "0.38/0.5/0.7mm軸径を選択でき、用途に合わせてカスタマイズ可能"
      - "就職・入学祝いなどギフト用途に特化した実績（評価4.52・8,000件超）"
    pros:
      - "名入れ代無料でコスパが高く、まとめ注文にも対応"
      - "8,000件超の購入実績で信頼度が高い"
      - "複数の軸径から選べるため受け取る方の好みに合わせやすい"
    cons:
      - "同ジェットストリームシリーズ内で商品の差別化が難しい"
      - "名入れ後は返品不可のため注文内容の事前確認が必須"
    recommendedFor: "名入れコストを抑えたい方・まとめてギフトを手配したい方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00qm7tn.3rdw6475.g00qm7tn.3rdw747f/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Fauc-youstyle%2Fmit-msxe5-1000%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Fauc-youstyle%2Fi%2F10066665%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/auc-youstyle/cabinet/rakulogo/rakulogo1/r1073_00007.jpg?_ex=128x128"
  - rank: 10
    name: "サラサグランド ビンテージ ジェルボールペン"
    brand: "ZEBRA（ゼブラ）"
    price: 1100
    capacity: "-"
    rating: 4.8
    reviewCount: 290
    features:
      - "ビンテージカラーのおしゃれなデザインで日常使いからギフトまで対応"
      - "0.5mm・0.3mm選択可能なゲルインク採用"
      - "スリムで持ちやすいグリップ設計（長さ142.6mm・直径19.3mm）"
    pros:
      - "ゲルインクならではの濃くくっきりした筆記線"
      - "ビンテージカラー展開でコレクション性も高い"
      - "替え芯対応で長く使えるコスパの良さ"
    cons:
      - "ゲルインクのため油性より若干インク切れが早い"
      - "金属軸のため軽量ペンより重さを感じる場合がある"
    recommendedFor: "おしゃれなデザインを重視する方・滑らかなゲルの書き心地を求める方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00q020n.3rdw6d49.g00q020n.3rdw75a0/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Fogawahan%2F10008119%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Fogawahan%2Fi%2F10008119%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/ogawahan/cabinet/sho06/sarasa/sarasa2026-naire-01.jpg?_ex=128x128"
    priceMax: 1980
tags:
  - "ボールペン おすすめ"
  - "ボールペン コスパ"
  - "ボールペン 比較"
  - "高級ボールペン"
  - "ボールペン ギフト"
faqs:
  - question: "油性インクとゲルインク、どちらが書きやすい？"
    answer: "一般的にゲルインクの方が滑らかで発色が良く、書き心地の「軽さ」を感じやすいとされています。一方、油性インクは手帳など薄い紙への裏抜けが少なく、インクの持ちが良い傾向があります。最新の油性（ジェットストリームなど）は書き心地が大幅に改善されており、好みで選ぶのがおすすめです。"
  - question: "ボールペンの名入れは後からでもできる？"
    answer: "購入後の名入れ対応は一般的に難しく、名入れ対応ショップで購入時に依頼するのが基本です。名入れ後は返品・交換不可となるケースが多いため、文字の内容・書体をしっかり確認してから注文してください。"
  - question: "Parker・CROSSの替え芯は国内で買える？"
    answer: "Amazon・楽天市場などの通販サイトや、大型文房具店・百貨店の筆記具コーナーで購入できます。国産ブランドに比べてやや取り扱い店舗が少ないため、まとめて購入しておくと安心です。"
  - question: "ビジネスギフトにはどのブランドが最適？"
    answer: "目上の方や取引先へのギフトにはParker・CROSSなど欧米の老舗ブランドが格式の面で好まれる傾向があります。社内や友人へのプレゼントならuniのジェットストリーム名入れも喜ばれます。予算と関係性に応じて選んでください。"
---


## 名入れを頼んだ時点で、返品も交換もできなくなる

このジャンルで最も取り返しがつかないのが名入れです。**刻印を入れた商品は、原則として返品・交換を受け付けてもらえません**。贈り物として選ぶなら、注文する前に次を確認してください。

- **表記をどうするか**: 漢字・ローマ字・イニシャルのどれか。旧字体（髙・﨑など）に対応しているかはショップごとに違います
- **文字数の上限**: 軸の細いペンは入る文字数が限られます。フルネームが入らないことがあります
- **納期**: 通常の配送に刻印の作業日数が上乗せされます。**記念日に間に合うかは、在庫ではなく刻印の日数で決まります**

誤字のまま刻印されても直せないため、注文画面の確認は入力欄を読み上げるくらい慎重にして損はありません。

## 替え芯が手に入るかどうかで、長く使えるかが決まる

本体を長く使う前提なら、**替え芯の入手しやすさが実際の使い勝手を左右します**。国内メーカーの製品は文具店や通販で見つけやすく、日常的に補充できます。海外ブランドは専門店や通販が中心になるため、切れてから探すと間が空きます。

替え芯には**国際規格に沿った形状**のものがあり、その規格に対応する本体なら他社の芯も使えることがあります。書き味が好みでない、インクの色を変えたい、といった場合の逃げ道になるので、購入前に軸の対応規格を見ておくと選択肢が広がります。

多機能ペンは色ごとに芯が要ります。**1本のペンを維持するのに複数の替え芯が必要**になる点は、単機能のペンとの実質的な差になります。

## 油性とゲルは、書く紙と場面で選び分ける

油性インクは乾きが早く、裏抜けしにくいのが特徴です。**複写式の伝票のように筆圧をかけて書く用途では油性が確実**で、宛名書きなど濡れる可能性がある場面でもにじみにくくなります。

ゲルインクは発色が濃く、軽い筆圧でもはっきり書けます。長時間書き続ける場面では手が疲れにくい一方、**乾く前にこすると擦れる**ため、左利きの方や、書いた直後に手を置く書き方をする場合は乾きの速さも見ておくと快適です。

手帳のように薄い紙に書くなら裏抜けのしにくさ、ノートにたくさん書くなら滑らかさ、と**書く対象から決める**と迷いません。

## 見た目で選ぶ商品と、書き味で選ぶ商品は分けて考える

装飾を主眼にした軸や、素材の質感を売りにした商品は、**書き味そのものを比べる対象ではありません**。贈り物として形に残ることや、机の上での見え方に価値がある商品です。

一方、金属軸のペンは重量があり、**長時間の筆記では手が疲れることがあります**。日常的に何ページも書く人が「格のあるペン」を選ぶと、結局使わなくなることがあるので、贈る相手が普段どれくらい書く人かを想像してから決めると外しません。

グリップの太さも同様で、細い軸は携帯しやすく、太い軸は手の負担が少ない傾向があります。**軸の太さと重さは、価格や機能とは無関係に好みが分かれる部分**です。

## 贈り物として渡すまでに必要なもの

ボールペンは贈答の定番ですが、**ペン本体が届いただけでは渡せる状態になっていない**ことがあります。確認しておきたいのは次の3点です。

- **箱の有無**: ギフト用の化粧箱が付くか、簡易包装かは商品によって違います。箱が付かない商品を贈答に使うなら、別途用意することになります
- **ラッピングと熨斗**: 対応しているショップとしていないショップがあります。名入れと同様、注文時に選ぶ形式が多いので、後から追加できないことがあります
- **納期の合計**: 刻印・ラッピング・配送の日数が積み上がります。**渡す日から逆算して、余裕を持って注文する**のが確実です

相手の勤務先や立場によっては、記念品として社名や日付を入れることもあります。ただし**個人的に使ってもらいたいなら、名前だけにとどめる**ほうが日常で使いやすくなります。刻印は消せないので、どこで使ってほしいかを想像してから決めてください。

## 日常使いなら「同じものをもう一度買えるか」を見る

毎日使うペンは、いつか必ず失くします。そのとき**同じものをすぐ買い直せるかどうか**が、実際の使い勝手を左右します。定番として長く売られている型は替え芯も含めて入手しやすく、書き味に慣れた状態を維持できます。

限定色や期間限定のモデルは魅力的ですが、**同じものを買い直せない前提**で選ぶことになります。持ち歩いて使い倒すペンと、机に置いて長く使うペンを分けて考えると、どちらを選ぶべきかがはっきりします。

## 買ってから気づきやすい注意点

- **長く使わないとインクが出なくなる**: 特にゲルインクは、放置するとペン先が乾いて出にくくなります。使う予定がないなら、贈るタイミングを合わせるほうが親切です
- **贈る相手が左利きかどうか**: 乾きの遅いインクは、左利きの方だと手で擦れます。相手が分からない場合は油性のほうが無難です
- **すでに持っていないか**: 定番の多機能ペンは持っている人も多い商品です。ギフトで選ぶなら、色や仕様が重ならないかを一度考えてみてください

## まとめ

ボールペンは価格よりも、**名入れの可否と替え芯の入手しやすさ**で後悔が決まります。刻印を入れると返品できないので、表記・文字数・納期は注文前に確認してください。

インクは書く対象から選びます。複写伝票や宛名書きなら油性、長時間の筆記ならゲル、というのが分かりやすい基準です。**軸の重さと太さは価格と無関係に好みが分かれる**ので、贈り物なら相手が普段どれくらい書く人かを想像してから決めると外しません。価格と仕様は、ページ上部の比較表（自動更新）でご確認ください。

机まわりの消耗品として [乾電池の比較](/articles/battery-comparison/) も扱っています。

---
※ 価格は記事執筆時点の楽天市場での販売価格です。価格は変動する場合があります。
