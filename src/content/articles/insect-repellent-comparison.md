---
title: "虫除けスプレー おすすめランキング【2026年版】成分・用途別に比較"
description: "蚊・マダニ対策の虫除けスプレーをコスパ・成分・使いやすさで比較。イカリジン系・DEET系・天然成分系の特徴と用途別おすすめランキングを紹介します。"
category: "insect-repellent"
publishedAt: "2026-05-22"
articleType: "comparison"
updatedAt: "2026-09-19"
draft: false
products:
  - rank: 1
    name: "フマキラー スキンベープ ミスト プレミアム イカリジン 200mL"
    brand: "フマキラー"
    price: 752
    capacity: "200mL"
    pricePerUnit: "約3.8円/mL"
    rating: 4.67
    reviewCount: 9
    features:
      - "イカリジン10%配合のミストタイプ虫よけスプレー"
      - "蚊・アブ・マダニなど幅広い虫に対応した防除用医薬部外品"
      - "フレッシュサボンの香りでべたつきにくい使い心地"
    pros:
      - "イカリジン系で子どもから大人まで使いやすい"
      - "べたつきにくく肌へのやさしさを重視する方に向いている"
      - "アウトドアから日常の外出まで幅広いシーンで使いやすい"
    cons:
      - "香り付きのため無香料を好む方には向かない場合がある"
      - "高温多湿な環境では効果の持続を確認してから使いたい"
    recommendedFor: "子どもも一緒に使えるイカリジン系虫よけを探している方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00ru0on.3rdw60b7.g00ru0on.3rdw7edf/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Fat-life%2F4902424440614%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Fat-life%2Fi%2F10175287%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/at-life/cabinet/e/imgrc0071325465.jpg?_ex=128x128"
    offers:
      - provider: "yahoo"
        label: "Yahoo!"
        url: "https://ck.jp.ap.valuecommerce.com/servlet/referral?sid=3770852&pid=892615315&vc_url=https%3A%2F%2Fstore.shopping.yahoo.co.jp%2Fsundrugec%2F4902424446593.html"
        matchStatus: "matched"
        updatedAt: "2026-06-11"
        price: 881
        imageUrl: "https://item-shopping.c.yimg.jp/i/j/sundrugec_4902424446593"
        available: true
        rating: 4.53
        reviewCount: 15
    genreId: "401642"
  - rank: 2
    name: "サラテクト 無香料"
    brand: "アース製薬"
    price: 657
    capacity: "100ml"
    pricePerUnit: "約6.6円/mL"
    rating: 0
    reviewCount: 0
    features:
      - "無香料タイプの防除用医薬部外品虫よけスプレー"
      - "持ち運びやすいコンパクトな100mlサイズ"
      - "アース製薬のロングセラー虫よけシリーズ"
    pros:
      - "無香料のため香りが気になる場面でも使いやすい"
      - "コンパクトサイズで携帯用として扱いやすい"
      - "医薬部外品で有効成分の基準が明確"
    cons:
      - "容量が少ないため長期間の使用では補充が必要になりやすい"
      - "使用対象年齢や塗布方法は事前に確認が必要"
    recommendedFor: "無香料の虫よけを携帯してアウトドアに出かける大人の方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00rqszn.3rdw6b20.g00rqszn.3rdw7361/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Fmatsukiyo%2F49469592%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Fmatsukiyo%2Fi%2F10454144%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/matsukiyo/cabinet/i0054/49469592_1.jpg?_ex=128x128"
    offers:
      - provider: "yahoo"
        label: "Yahoo!"
        url: "https://ck.jp.ap.valuecommerce.com/servlet/referral?sid=3770852&pid=892615315&vc_url=https%3A%2F%2Fstore.shopping.yahoo.co.jp%2Fyyshop%2F49469592.html"
        matchStatus: "matched"
        updatedAt: "2026-07-06"
        price: 433
        rating: 0
        reviewCount: 0
        imageUrl: "https://item-shopping.c.yimg.jp/i/j/yyshop_49469592"
        available: true
      - provider: "amazon"
        label: "Amazon"
        asin: "B000FQ5JQ4"
        url: "https://www.amazon.co.jp/dp/B000FQ5JQ4?tag=kuraselect-direct-22"
        matchStatus: "matched"
        matchConfidence: "high"
        matchedCapacity: "100mL"
        updatedAt: "2026-07-16T00:00:00.000Z"
    genreId: "553423"
  - rank: 3
    name: "家中どこでも虫キライスプレー"
    brand: "ウッディラボ"
    price: 1980
    capacity: "250ml"
    pricePerUnit: "約7.9円/mL"
    rating: 4.1
    reviewCount: 40
    features:
      - "天然精油配合・殺虫成分不使用の空間用忌避スプレー"
      - "玄関・ベランダ・窓周りへの噴霧に使いやすい大容量"
      - "カメムシ・蚊・クモ・コバエ・アリなど多様な虫の忌避に対応"
    pros:
      - "室内や玄関周りの虫の侵入対策として使いやすい"
      - "殺虫成分不使用のため子どもやペットがいる家庭でも使いやすい"
      - "大容量で複数箇所への使用がしやすい"
    cons:
      - "ボディ用ではないため肌への直接使用には向かない"
      - "天然精油の香りが強く感じられる場合がある"
    recommendedFor: "室内や玄関まわりの虫の侵入対策を手軽にしたい方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00uhp6n.3rdw613e.g00uhp6n.3rdw7d67/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Fwoddylabo%2F4560437941414%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Fwoddylabo%2Fi%2F10000005%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/woddylabo/cabinet/product/01/11722292/mushikirai_g_202606.jpg?_ex=128x128"
    offers:
      - provider: "yahoo"
        label: "Yahoo!"
        url: "https://ck.jp.ap.valuecommerce.com/servlet/referral?sid=3770852&pid=892615315&vc_url=https%3A%2F%2Fstore.shopping.yahoo.co.jp%2Fa1-shop%2F67-7283-85.html"
        matchStatus: "matched"
        updatedAt: "2026-06-16"
        price: 1881
        imageUrl: "https://item-shopping.c.yimg.jp/i/j/a1-shop_67-7283-85"
        available: true
        rating: 0
        reviewCount: 0
      - provider: "amazon"
        label: "Amazon"
        asin: "B07PJ47VHT"
        url: "https://www.amazon.co.jp/dp/B07PJ47VHT?tag=kuraselect-direct-22"
        matchStatus: "matched"
        matchConfidence: "high"
        matchedCapacity: "250mL（本体）"
        updatedAt: "2026-07-16T00:00:00.000Z"
    genreId: "401642"
  - rank: 4
    name: "KINCHO プレシャワー DFミスト プレミアガード 無香料 120mL"
    brand: "大日本除虫菊（KINCHO）"
    price: 1113
    capacity: "120mL"
    pricePerUnit: "約9.3円/mL"
    rating: 5
    reviewCount: 1
    features:
      - "イカリジン高濃度配合で蚊・マダニへの効果が期待できる防除用医薬部外品"
      - "無香料設計で衣類や体への使用後のニオイが気になりにくい"
      - "子どもにも使えると謳うKINCHO製スプレー"
    pros:
      - "イカリジン配合でディートを避けたい方の選択肢になる"
      - "無香料で香りに敏感な方や職場使いにも向く"
      - "国内大手メーカーKINCHOの品質と信頼性が選べる"
    cons:
      - "120mLと中容量のため使用頻度が高いと消費が早め"
      - "スキンベープと同成分のためブランドによる使い分けが主な違いになる"
    recommendedFor: "KINCHOブランドのイカリジン配合無香料スプレーを試したい方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00slbfn.3rdw664c.g00slbfn.3rdw7e6f/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Fthinkrich%2Fsk07402%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Fthinkrich%2Fi%2F10227049%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/thinkrich/cabinet/sale2_muryou_07/s07402.jpg?_ex=128x128"
    offers:
      - provider: "yahoo"
        label: "Yahoo!"
        url: "https://ck.jp.ap.valuecommerce.com/servlet/referral?sid=3770852&pid=892615315&vc_url=https%3A%2F%2Fstore.shopping.yahoo.co.jp%2Fcompmoto-y%2F4987115540569.html"
        matchStatus: "matched"
        updatedAt: "2026-07-02"
        price: 1564
        rating: 0
        reviewCount: 0
        imageUrl: "https://item-shopping.c.yimg.jp/i/j/compmoto-y_4987115540569"
        available: true
    genreId: "401642"
  - rank: 5
    name: "選べるアロマスプレー 虫除け 3本セット"
    brand: "ease-aroma"
    price: 1320
    capacity: "30ml×2本＋15ml×1本（75ml）"
    pricePerUnit: "約18円/mL"
    rating: 4.25
    reviewCount: 789
    features:
      - "楽天市場で取り扱いのある追加候補"
      - "日用品として使いやすい定番タイプ"
      - "既存比較にない選択肢を補える構成"
    pros:
      - "まとめ買い候補として比較しやすい"
      - "用途や好みに合わせて選びやすい"
      - "既存商品と価格帯を比べやすい"
    cons:
      - "サイズや仕様は購入前に確認が必要"
      - "ショップにより在庫や配送条件が変わる"
    recommendedFor: "既存候補以外も比較して選びたい人"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00qfl9n.3rdw60bc.g00qfl9n.3rdw7fb3/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Fease-aroma%2F10002751%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Fease-aroma%2Fi%2F10002751%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/ease-aroma/cabinet/spray/spray3hon_s_01.jpg?_ex=128x128"
    genreId: "216082"
  - rank: 6
    name: "パーフェクトポーション アウトドアボディスプレー 100mL"
    brand: "PERFECT POTION"
    price: 1870
    capacity: "100mL"
    pricePerUnit: "約19円/mL"
    rating: 4.49
    reviewCount: 546
    features:
      - "天然アロマ100%配合・合成殺虫成分不使用の処方"
      - "6種類のハーバル成分を配合した自然由来スプレー"
      - "ボディへの塗布と空間への使用の両方に対応"
    pros:
      - "合成殺虫成分を避けたい方に向いている"
      - "アロマの香りを楽しみながら虫対策できる"
      - "オーストラリア発オーガニックブランドの製品"
    cons:
      - "天然成分のため合成殺虫成分系と比べると忌避効果の持続時間が短い傾向がある"
      - "成分の香りが好みに合わない場合もある"
    recommendedFor: "天然・自然成分にこだわりたい方や肌の敏感な方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00q5inn.3rdw6732.g00q5inn.3rdw7aa0/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Faimere%2Fp0060001%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Faimere%2Fi%2F10002154%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/aimere/cabinet/02716961/02974729/imgrc0132362283.jpg?_ex=128x128"
    offers:
      - provider: "yahoo"
        label: "Yahoo!"
        url: "https://ck.jp.ap.valuecommerce.com/servlet/referral?sid=3770852&pid=892615315&vc_url=https%3A%2F%2Fstore.shopping.yahoo.co.jp%2Fyoga-pi%2Fperfect-potion-110931cd-125.html"
        matchStatus: "matched"
        updatedAt: "2026-07-12"
        price: 1870
        imageUrl: "https://item-shopping.c.yimg.jp/i/j/yoga-pi_perfect-potion-110931cd-125"
        available: true
        rating: 4.88
        reviewCount: 17
    genreId: "101882"
  - rank: 7
    name: "ヒバNON NON"
    brand: "ダイレクトテレショップ"
    price: 7788
    capacity: "450ml×2本"
    rating: 4.1
    reviewCount: 115
    features:
      - "ヒノキチオール・テルペン油・除虫菊エキスなど天然由来成分100%配合"
      - "450ml入りで玄関・室内・軒下など広範囲への噴霧に適した容量"
      - "クモ・ダニ・カメムシ・アリ・ムカデなど多様な害虫の忌避に対応"
    pros:
      - "天然成分のみでペットや子どもがいる家庭でも使いやすい"
      - "2本・4本・6本セットから選べるので使用頻度に合わせた購入がしやすい"
      - "屋内外を問わず玄関・寝室・軒下など幅広い場所で使いやすい"
    cons:
      - "ボディ用ではないため皮膚への直接使用には向かない"
      - "独自の天然成分の香りが強く感じる場合がある"
    recommendedFor: "屋内外の虫の侵入対策に天然成分100%の防虫スプレーを探している方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00pqorn.3rdw6854.g00pqorn.3rdw7e1a/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Ftv-ya%2Fhibanonnon-a-2-p%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Ftv-ya%2Fi%2F10002598%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/tv-ya/cabinet/item/hibanonnon-a-2-p/thum/hibanonnon-a-2-p.jpg?_ex=128x128"
    priceMax: 15288
    genreId: "401642"
  - rank: 8
    name: "アンチバグ プレミアム"
    brand: "AROMICstyle"
    price: 1080
    capacity: "100ml"
    rating: 4.48
    reviewCount: 256
    features:
      - "天然精油9種類を配合した合成殺虫成分不使用の虫よけスプレー"
      - "レモングラス・ラベンダー・ユーカリ・ヒノキなど天然アロマをブレンド"
      - "赤ちゃんやベビーカーへの使用も考慮した無添加処方"
    pros:
      - "天然成分100%で合成殺虫成分を避けたい方に向いている"
      - "複数の精油ブレンドで香りを楽しみながら虫対策できる"
      - "ボディ・ベビーカー・玄関など室内外の複数シーンで使いやすい"
    cons:
      - "天然成分のため合成殺虫成分系と比べると持続時間を確認しながら使う必要がある"
      - "精油の香りが好みに合わない場合もある"
    recommendedFor: "無添加・天然精油100%の虫よけを子どもにも使いたい方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00qvsqn.3rdw6696.g00qvsqn.3rdw7fa3/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Faromaspray%2Fsp-bug-100-100e%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Faromaspray%2Fi%2F10001233%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/aromaspray/cabinet/item/02/sp-bug-max10cp.jpg?_ex=128x128"
    offers:
      - provider: "yahoo"
        label: "Yahoo!"
        url: "https://ck.jp.ap.valuecommerce.com/servlet/referral?sid=3770852&pid=892615315&vc_url=https%3A%2F%2Fstore.shopping.yahoo.co.jp%2Faroma-spray%2Fsp-bug-100-100e.html"
        matchStatus: "review"
        updatedAt: "2026-07-16"
        price: 2980
        imageUrl: "https://item-shopping.c.yimg.jp/i/j/aroma-spray_sp-bug-100-100e"
        available: true
        rating: 4.69
        reviewCount: 29
    priceMax: 3580
    genreId: "401642"
  - rank: 9
    name: "安全安心 虫除けスプレー"
    brand: "無添加工房 OKADA"
    price: 1386
    capacity: "100ml"
    rating: 4.53
    reviewCount: 1553
    features:
      - "ディート不使用・天然成分配合の赤ちゃんから使える虫よけスプレー"
      - "1本・3本・10本セットから選べる購入方法"
      - "無添加処方で合成殺虫成分を含まない設計"
    pros:
      - "ディート不使用で赤ちゃんや子どもへの使用を重視する家庭に向いている"
      - "まとめ買いが選べ、使用頻度の高い季節にストックしやすい"
      - "天然成分配合で肌への刺激を気にしたい方に選ばれやすい"
    cons:
      - "合成殺虫成分系と比べると使用頻度が多くなる場合がある"
      - "容量が少ないため長期の使用では複数本の準備が必要"
    recommendedFor: "ディート不使用で赤ちゃんや子どもと一緒に使える虫よけを探している方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00ppoyn.3rdw6621.g00ppoyn.3rdw79a6/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Fsafe%2F580481%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Fsafe%2Fi%2F10000010%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/safe/cabinet/2309_sku_img/580481.jpg?_ex=128x128"
    priceMax: 13860
    genreId: "401642"
tags:
  - "虫除けスプレー おすすめ"
  - "虫除けスプレー コスパ"
  - "虫除けスプレー 比較"
  - "まとめ買い"
faqs:
  - question: "子どもには何歳から使えますか？"
    answer: "製品により異なります。イカリジン系はDEET系に比べて年齢制限がゆるやかな製品が多い傾向がありますが、必ず各製品の使用上の注意を確認してください。天然成分系は赤ちゃん向けとうたう製品もありますが、肌への反応には個人差があります。"
  - question: "衣類に使っても大丈夫ですか？"
    answer: "一部のボディ用虫除けスプレーは衣類の上から使えるタイプもありますが、素材によっては変色や傷みが生じることがあります。製品の表示をよく確認してから使用してください。"
  - question: "虫除けスプレーはどこに保管すればいいですか？"
    answer: "直射日光・高温を避け、子どもの手の届かない場所に保管してください。スプレー缶タイプは特に高温や火気の近くに置かないよう注意が必要です。"
  - question: "ボディ用と空間用は何が違うのですか？"
    answer: "ボディ用は皮膚や衣類に塗布して個人を守るタイプ、空間用は室内や玄関などに噴霧して虫が近づきにくい環境を作るタイプです。役割が異なるため、用途に合わせて選ぶことが大切です。\r\n\r\n---"
---

## 「肌に塗るもの」と「空間にまくもの」を取り違えない

このジャンルで最初に確認すべきは成分ではなく、**その製品が体に使うものか、場所に使うものか**です。玄関やベランダに噴霧する空間用は、肌に直接使うことを想定していません。逆に肌用を部屋中にまいても、想定された使い方ではありません。

見た目が似たスプレー缶で並んでいるため取り違えが起こりやすく、**買ってから「これは体に使えない」と気づく**ことがあります。商品名ではなく、パッケージに書かれた用途の記載を確認してください。

あわせて、**虫を寄せ付けにくくするもの（忌避）と、虫を退治するもの（殺虫）も別物**です。刺されたくないのか、部屋にいる虫をどうにかしたいのかで、選ぶ棚が変わります。

## 対象の虫と使用可能な年齢は、製品ごとに表示が違う

蚊だけを想定した製品と、マダニやブユまで対象に含む製品があります。**山や草地に入るなら、対象の虫にマダニが含まれているか**を表示で確認してください。都市部の屋外と山林では必要な製品が変わります。

子どもに使う場合は、**成分の種類と濃度によって使用できる年齢や1日の回数に制限がある**ことがあります。これはパッケージや添付文書に明記されているので、年齢・回数の記載をそのまま守るのが確実です。**同じ成分名でも濃度が違えば制限も変わる**ため、以前使っていた製品の感覚を持ち込まないようにします。

肌が弱い方や妊娠中の方は、購入前に薬剤師や医師に相談すると安心です。

表示の読み方でもう一つ紛らわしいのが、**医薬品・医薬部外品・雑貨という区分**です。これは効能をどこまで表示できるかという制度上の分類であって、成分の種類そのものを指しているわけではありません。同じ成分を使っていても区分が違うことがあるので、**区分名を成分名の代わりに読まない**ようにしてください。

## 持続時間が短い製品ほど、容量あたりの単価は意味を持たなくなる

虫除けは**塗り直しの回数で消費量が決まります**。持続時間の短い製品は、屋外で過ごす時間が長いほど何度も塗り直すことになり、容量あたりの単価が低くても消費が早くなります。

汗をかいたとき、水に入ったあと、タオルで拭いたあとは効果が落ちます。**1日出かけるなら携帯できる小容量を持ち歩き、家に大容量を置く**という分け方が現実的です。上部の比較表は容量と価格を並べますが、**塗り直しの頻度までは表に載りません**。

## 服やプラスチックに付くと傷むことがある

成分によっては、**化学繊維やプラスチック製品を傷めることがあります**。時計のベルト、眼鏡のフレーム、スマートフォンのケース、合成皮革などに付いたまま放置すると、変質や白化の原因になります。

塗るときは、**これらを外してから、手に取って伸ばす**のが安全です。顔に使う場合は直接噴霧せず、手のひらに出してから塗り広げます。目や口の周り、傷のある場所には使いません。

日焼け止めと併用するときは、**日焼け止めを先に塗り、乾いてから虫除け**の順にします。

## 買ってから気づきやすい注意点

- **帰宅後は洗い流す**: 必要のない時間まで肌に残しておく理由はありません。石けんで洗い流します
- **保管は直射日光と高温を避ける**: スプレー缶は車内などの高温になる場所に置かないでください
- **季節をまたぐ量を買わない**: 使う期間が限られる商品です。翌シーズンまで持ち越す量は、単価が安くても得になりません

## まとめ

虫除けスプレーは、まず**肌に使うものか場所に使うものか**を取り違えないところから始めてください。そのうえで、対象の虫にマダニが含まれるか、子どもに使える年齢と回数の記載はどうか、をパッケージで確認します。

消費量は容量ではなく**塗り直しの回数**で決まるので、持ち歩く小容量と家に置く大容量を分けるのが実際的です。化学繊維やプラスチックを傷めることがあるため、時計や眼鏡は外してから使ってください。価格と容量は、ページ上部の比較表（自動更新）でご確認ください。

室内で使うタイプは [蚊取り器（液体タイプ）の比較](/articles/mosquito-repellent-liquid-comparison/)、衣類の虫食い対策は [防虫剤の比較](/articles/moth-repellent-comparison/) で扱っています。

---

※ 価格は記事執筆時点の楽天市場での販売価格です。価格は変動する場合があります。
