---
title: "子ども用歯磨き粉 おすすめランキング【2026年版】フッ素濃度・年齢別に比較"
description: "子ども用歯磨き粉をフッ素濃度・年齢別に徹底比較。クリニカKid's・チェックアップ・ライオンこどもハミガキなど人気8選を安全性・効果で解説。"
category: "toothpaste"
publishedAt: "2026-04-30"
articleType: "comparison"
updatedAt: "2026-09-06"
products:
  - rank: 1
    name: "ライオン クリニカキッズ ジェルハミガキ いちご 60g×10点セット"
    brand: "ライオン（クリニカKid's）"
    price: 3494
    capacity: "60g×10点セット"
    pricePerUnit: "約5.8円/g"
    rating: 5
    reviewCount: 3
    features:
      - "フッ素950ppmで乳歯・生えたての永久歯を強化"
      - "ジェルタイプで歯のすみずみまで行き渡る"
      - "キシリトール配合でむし歯菌の活動を抑制"
    pros:
      - "いちご味で子どもが嫌がらない"
      - "ジェルタイプで仕上げ磨きにも最適"
      - "3本セットでまとめ買いコスパ◎"
    cons:
      - "チューブから出しすぎやすい"
      - "研磨剤が入っていないため着色汚れには弱い"
    recommendedFor: "歯磨き嫌いのお子さま・仕上げ磨きに使いたい方・ジェルタイプが好きな方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00ru0on.3rdw60b7.g00ru0on.3rdw7edf/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Fat-life%2F0000049795240-010%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Fat-life%2Fi%2F10184891%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/at-life/cabinet/aa/49795240.jpg?_ex=128x128"
  - rank: 2
    name: "チェックアップ コドモ 60g×3本"
    brand: "ライオン（チェックアップ）"
    price: 1309
    capacity: "60g×3本（180g）"
    pricePerUnit: "約7.3円/g"
    rating: 4.86
    reviewCount: 7
    features:
      - "子ども向けの歯みがき用品"
      - "ジェルや泡タイプなど使いやすさで選べる"
      - "年齢や味の好みに合わせやすい候補"
    pros:
      - "子どもの歯みがき習慣に合わせて選べる"
      - "フッ素量やフッ素不使用タイプを比較できる"
      - "味や形状の好みを試しやすい"
    cons:
      - "対象年齢やフッ素濃度の確認が必要"
      - "味や泡立ちは子どもの好みが分かれる"
    recommendedFor: "子ども用歯みがき粉を年齢や味で選びたい家庭"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00t0trn.3rdw642b.g00t0trn.3rdw7fbc/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Fkelly-s%2F10003245%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Fkelly-s%2Fi%2F10003245%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/kelly-s/cabinet/chkupkodomogr3_01.jpg?_ex=128x128"
  - rank: 3
    name: "あわだたない こどもハミガキ ラムネ 60g"
    brand: "エスケー石鹸"
    price: 495
    capacity: "60g"
    pricePerUnit: "約8.3円/g"
    rating: 0
    reviewCount: 0
    features:
      - "子ども向けの歯磨き粉"
      - "継続利用しやすい定番構成"
      - "楽天市場で購入できる商品"
    pros:
      - "年齢や味の好みに合わせて選びやすい"
      - "買い足し候補として比較しやすい"
      - "既存商品と単価で比べやすい"
    cons:
      - "使用量や対象年齢を確認したい"
      - "購入時は最新の販売条件を確認したい"
    recommendedFor: "年齢や味の好みに合わせて選びたい方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00q07yn.3rdw60aa.g00q07yn.3rdw71e4/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Fk-vitamin%2Fsks-ora-003%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Fk-vitamin%2Fi%2F10016289%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/k-vitamin/cabinet/00976315/12537488/imgrc0115766783.jpg?_ex=128x128"
    offers:
      - provider: "yahoo"
        label: "Yahoo!"
        url: "https://ck.jp.ap.valuecommerce.com/servlet/referral?sid=3770852&pid=892615315&vc_url=https%3A%2F%2Fstore.shopping.yahoo.co.jp%2Fhareya%2Fy-nsk0182x5.html"
        matchStatus: "review"
        updatedAt: "2026-06-07"
        price: 3050
        rating: 0
        reviewCount: 0
        imageUrl: "https://item-shopping.c.yimg.jp/i/j/hareya_y-nsk0182x5"
        available: true
  - rank: 4
    name: "エスケー石鹸 あわだたないこどもハミガキ ぶどう 60g"
    brand: "エスケー石鹸"
    price: 495
    capacity: "60g"
    pricePerUnit: "約8.3円/g"
    rating: 0
    reviewCount: 0
    features:
      - "子ども向けの歯磨き粉"
      - "継続利用しやすい定番構成"
      - "楽天市場で購入できる商品"
    pros:
      - "年齢や味の好みに合わせて選びやすい"
      - "買い足し候補として比較しやすい"
      - "既存商品と単価で比べやすい"
    cons:
      - "使用量や対象年齢を確認したい"
      - "購入時は最新の販売条件を確認したい"
    recommendedFor: "年齢や味の好みに合わせて選びたい方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00r2u0n.3rdw6e74.g00r2u0n.3rdw74e7/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Fecorea2010%2F3770%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Fecorea2010%2Fi%2F10000576%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/ecorea2010/cabinet/item/03587598/3770.jpg?_ex=128x128"
    offers:
      - provider: "yahoo"
        label: "Yahoo!"
        url: "https://ck.jp.ap.valuecommerce.com/servlet/referral?sid=3770852&pid=892615315&vc_url=https%3A%2F%2Fstore.shopping.yahoo.co.jp%2Fkenyu-kan%2F6372.html"
        matchStatus: "matched"
        updatedAt: "2026-07-12"
        price: 495
        imageUrl: "https://item-shopping.c.yimg.jp/i/j/kenyu-kan_6372"
        available: true
        rating: 0
        reviewCount: 0
  - rank: 5
    name: "エスケー石鹸 あわだたないこどもハミガキ ぶどう 60g×32個"
    brand: "エスケー石鹸"
    price: 15840
    capacity: "60g×32個（1920g）"
    pricePerUnit: "約8.3円/g"
    rating: 0
    reviewCount: 0
    features:
      - "子ども向けの歯磨き粉"
      - "継続利用しやすい定番構成"
      - "楽天市場で購入できる商品"
    pros:
      - "年齢や味の好みに合わせて選びやすい"
      - "買い足し候補として比較しやすい"
      - "既存商品と単価で比べやすい"
    cons:
      - "使用量や対象年齢を確認したい"
      - "購入時は最新の販売条件を確認したい"
    recommendedFor: "年齢や味の好みに合わせて選びたい方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00r2u0n.3rdw6e74.g00r2u0n.3rdw74e7/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Fecorea2010%2F3770c%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Fecorea2010%2Fi%2F10000578%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/ecorea2010/cabinet/item/case/3770c.jpg?_ex=128x128"
  - rank: 6
    name: "あわだたない こどもハミガキ ラムネ 60g"
    brand: "エスケー石鹸"
    price: 517
    capacity: "60g"
    pricePerUnit: "約8.6円/g"
    rating: 0
    reviewCount: 0
    features:
      - "子ども向けの歯磨き粉"
      - "継続利用しやすい定番構成"
      - "楽天市場で購入できる商品"
    pros:
      - "年齢や味の好みに合わせて選びやすい"
      - "買い足し候補として比較しやすい"
      - "既存商品と単価で比べやすい"
    cons:
      - "使用量や対象年齢を確認したい"
      - "購入時は最新の販売条件を確認したい"
    recommendedFor: "年齢や味の好みに合わせて選びたい方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00rvxzn.3rdw6cd4.g00rvxzn.3rdw76f0/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Fkaigo%2F4964495600182%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Fkaigo%2Fi%2F10370485%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/kaigo/cabinet/2025b/4964495600182.jpg?_ex=128x128"
  - rank: 7
    name: "ピジョン ジェル状歯みがき ぷちキッズ ぶどう味【親子で乳歯ケア】"
    brand: "ピジョン"
    price: 485
    capacity: "（50g）"
    pricePerUnit: "約9.7円/g"
    rating: 4.5
    reviewCount: 2
    features:
      - "フッ素500ppmで1歳6ヶ月から使える低濃度設計"
      - "食品用原料成分使用で飲み込んでも安心"
      - "キシリトール70%配合"
    pros:
      - "1歳半から使える安全性の高さ"
      - "飲み込んでも安心な食品用原料"
      - "ベビー用品メーカーの信頼性"
    cons:
      - "フッ素濃度が低いため効果はおだやか"
      - "容量が少なくコスパはやや劣る"
    recommendedFor: "1歳半〜3歳のお子さま・初めての歯磨き粉に・安全性最重視の方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00qvyfn.3rdw6a92.g00qvyfn.3rdw717f/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Fsundrug%2F4902508104463%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Fsundrug%2Fi%2F10044509%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/sundrug/cabinet/202505_2/4902508104463.jpg?_ex=128x128"
    offers:
      - provider: "yahoo"
        label: "Yahoo!"
        url: "https://ck.jp.ap.valuecommerce.com/servlet/referral?sid=3770852&pid=892615315&vc_url=https%3A%2F%2Fstore.shopping.yahoo.co.jp%2Fsundrugec%2F4902508104463.html"
        matchStatus: "matched"
        updatedAt: "2026-08-22"
        price: 485
        imageUrl: "https://item-shopping.c.yimg.jp/i/j/sundrugec_4902508104463"
        available: true
        rating: 4.88
        reviewCount: 17
  - rank: 8
    name: "ばぶりーキッズ ゆらしー こども用泡歯磨き粉 48ml"
    brand: "from COCORO"
    price: 550
    capacity: "48ml"
    rating: 4.27
    reviewCount: 248
    features:
      - "子ども向けの歯みがき用品"
      - "ジェルや泡タイプなど使いやすさで選べる"
      - "年齢や味の好みに合わせやすい候補"
    pros:
      - "子どもの歯みがき習慣に合わせて選べる"
      - "フッ素量やフッ素不使用タイプを比較できる"
      - "味や形状の好みを試しやすい"
    cons:
      - "対象年齢やフッ素濃度の確認が必要"
      - "味や泡立ちは子どもの好みが分かれる"
    recommendedFor: "子ども用歯みがき粉を年齢や味で選びたい家庭"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00sm18n.3rdw6236.g00sm18n.3rdw7b8f/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Ffromcocoro%2Fbubbly_set%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Ffromcocoro%2Fi%2F10000320%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/fromcocoro/cabinet/items/10174430/240110_kato3.jpg?_ex=128x128"
    offers:
      - provider: "yahoo"
        label: "Yahoo!"
        url: "https://ck.jp.ap.valuecommerce.com/servlet/referral?sid=3770852&pid=892615315&vc_url=https%3A%2F%2Fstore.shopping.yahoo.co.jp%2Ffromcocoro%2Fy-yurabub-n1.html"
        matchStatus: "pending"
        updatedAt: "2026-06-20"
        price: 4680
        rating: 0
        reviewCount: 0
        imageUrl: "https://item-shopping.c.yimg.jp/i/j/fromcocoro_y-yurabub-n1"
        available: true
    priceMax: 4530
tags:
  - "歯磨き粉"
  - "子ども"
  - "フッ素"
  - "むし歯予防"
  - "コスパ"
  - "日用品"
draft: false
faqs:
  - question: "フッ素入り歯磨き粉を飲み込んでも大丈夫？"
    answer: "適正量（米粒大〜グリーンピース大）であれば、飲み込んでも健康上の問題はありません。体重10kgの子どもの場合、500ppmの歯磨き粉を丸々1本（60g）食べてようやく急性中毒量に達する計算で、通常の使用量では心配不要です。"
  - question: "いつから歯磨き粉を使い始めるべき？"
    answer: "最初の乳歯が生えたら（生後6ヶ月頃）、フッ素500ppmの歯磨き粉を米粒大で使い始めるのが推奨されています。それまではガーゼで拭く程度でOKです。"
  - question: "子ども用と大人用の違いは何？"
    answer: "主な違いはフッ素濃度（子ども用500〜950ppm vs 大人用1000〜1450ppm）、研磨剤の量（子ども用は少ない）、味（子ども用はフルーツ味）です。6歳以上になれば大人用の使用も可能ですが、味が合わない場合は子ども用を継続してもOKです。"
  - question: "歯磨き後にうがいは必要？"
    answer: "フッ素の効果を長持ちさせるため、歯磨き後のうがいは**1〜2回の少量の水**で行うのが理想です。大量の水で何度もうがいすると、せっかく塗布したフッ素が流れ落ちてしまいます。"
---

## 年齢で変わるのは濃度だけでなく「使う量」

子ども用の歯磨き粉選びで最初に確認するのはフッ化物の濃度ですが、実際に効果と安全性を左右しているのは**濃度と使用量の組み合わせ**です。濃度だけを見て選び、量を大人と同じように出してしまうと前提が崩れます。

年齢ごとの目安は次のとおりです。

| 年齢の目安 | 濃度の目安 | 一回に出す量 |
|---|---|---|
| 歯が生え始めた頃〜2歳 | 500ppm前後 | 米粒くらい |
| 2歳〜就学前 | 950ppm前後 | グリーンピースくらい |
| 就学後 | 1000〜1450ppm | 歯ブラシの毛の長さの半分〜全体 |

この「量」の感覚がつかみにくいのが実際のところです。チューブから出すと、意識していても目安より多くなりがちなので、最初のうちは保護者が出してあげるほうが確実です。

## うがいができるかどうかが分かれ目

濃度を上げるタイミングを判断する基準は、年齢そのものより**口に含んだものを吐き出せるかどうか**です。

飲み込んでしまう段階では、濃度を抑えた製品を少量使い、磨いたあとはガーゼなどで拭き取る程度にとどめます。うがいができるようになったら、少量の水で軽く一回すすぐ方法に切り替えていきます。何度も強くすすぐとフッ化物が流れてしまうため、大人と同じで「すすぎすぎない」ほうが残る量は多くなります。

年齢の区切りはあくまで目安なので、発達には個人差があります。心配な場合は歯科の定期健診のタイミングで相談すると、実際の口の状態に合わせた判断がもらえます。

## 研磨剤と乳歯の関係

乳歯は永久歯に比べてエナメル質が薄く、酸にも摩耗にも弱い性質があります。そのため子ども向けの製品は、研磨剤を含まないか、含んでいてもごく控えめな設計になっているものが中心です。

ジェルタイプが子ども向けで多いのは、この点と関係があります。ジェルは研磨剤を含まないものが多く、泡立ちも控えめなので、仕上げ磨きをする保護者から見て**口の中が見えやすい**という実用的な利点があります。

一方で研磨剤がないぶん、着色汚れを落とす力は弱くなります。ただし乳歯の時期に着色を気にする場面はそう多くなく、優先すべきは虫歯予防のほうです。

## 歯磨きを嫌がるときに効くのは「味」より「痛くないこと」

子どもが歯磨きを嫌がる理由として味や泡立ちが挙げられますが、現場でよくあるのは**痛い、苦しいという記憶**です。上唇の裏側にある筋（上唇小帯）にブラシが当たると強く痛み、それが一度あると口を開けなくなることがあります。仕上げ磨きのときに指でここを覆うだけで、嫌がり方が変わることは珍しくありません。

そのうえで、味は続けやすさに効きます。果物の風味など受け入れやすいフレーバーは有効ですが、**同じ味を大量に買い込まない**ほうが無難です。子どもの好みは短期間で変わり、一度嫌いになると同じ味では磨かなくなることがあります。

泡立ちが少ないタイプは、口の中が泡で満たされないため、磨いている時間を長く保ちやすくなります。仕上げ磨きをする側にとっても、どこを磨いているかが見えるのは大きな利点です。

## 形状ごとの向き不向き

**ジェルタイプ**は低発泡で研磨剤を含まないものが多く、仕上げ磨きが中心の時期に扱いやすい形です。歯面に留まりやすく、少量でも広げやすいのが特徴です。

**ペーストタイプ**は種類が多く入手しやすい形です。自分で磨けるようになった年齢で、大人と同じ感覚で使いたい場合に選びやすくなります。

**泡で出るタイプ**は、量の調整が難しい小さな子どもに向きます。ワンプッシュで出る量が決まっているため、出しすぎを防げるのが利点です。取り扱う商品数は他の形状より少なめです。

## 買い替えのタイミングを見落とさない

子ども用の歯磨き粉で起きやすいのが、**成長したのに濃度をそのままにしている**という状態です。就学のタイミングで濃度を上げられるのに、低い濃度の製品を使い続けているケースは少なくありません。

まとめ買いをする場合は、この切り替え時期を意識してください。未開封であれば長期保管できる製品ですが、買いだめした量を使い切る前に濃度を上げる年齢になってしまうと、結果的に使いどころがなくなります。開封後は数か月のうちに使い切る前提で本数を決めるほうが無駄がありません。

保管は高温多湿を避けた常温が基本です。自分で磨く習慣をつけたい時期なら、子ども自身が手に取れる高さに置いておくのも一つの方法です。

具体的な価格や1本あたりの単価は変動するため、ページ上部の比較表（自動更新）でご確認ください。

## まとめ

子ども用歯磨き粉は、年齢に応じた濃度と、それに見合った使用量の組み合わせで選びます。濃度を上げる判断はうがいができるかどうかが基準で、年齢はあくまで目安です。乳歯はエナメル質が薄いため低研磨または無研磨のタイプが向き、仕上げ磨きの時期は口の中が見えやすい低発泡が扱いやすくなります。

味は続けやすさに関わりますが、好みが変わることを見込んで同じ味を買いだめしすぎないでください。大人向けの選び方は [歯磨き粉の比較](/articles/toothpaste-comparison/)、ブラシ側は [歯ブラシの比較](/articles/toothbrush-comparison/) で整理しています。

> ※ 掲載価格は執筆時点での楽天市場における販売価格です。価格は変動する場合があります。最新の価格は各商品ページでご確認ください。
