---
title: "液体蚊取り 取替えボトル コスパ比較【2026年版】1日あたりで比較"
description: "電気式（プラグ式）液体蚊取りの取替えボトルを、対応日数あたりのコスパと香りタイプで比較。アースノーマットやフマキラーのベープリキッドなど無香料・微香性タイプの選び方を解説します。"
category: "mosquito-repellent-liquid"
publishedAt: "2026-06-13"
articleType: "comparison"
updatedAt: "2026-09-10"
draft: false
products:
  - rank: 1
    name: "ベープリキッド 取替え用 60日 無香料"
    brand: "フマキラー"
    price: 1270
    capacity: "60日用×2本（120日）"
    pricePerUnit: "約11円/日"
    rating: 4.73
    reviewCount: 22
    features:
      - "液体式（プラグ式）蚊取り器に使う取替え用ボトル"
      - "スイッチひとつで使える電気式の防除用医薬部外品"
      - "においが気になりにくい無香料タイプ"
    pros:
      - "無香料で寝室や食卓まわりでも使いやすい"
      - "コンセントに挿すだけで操作がシンプル"
      - "リビングや寝室の据え置き用途に向いている"
    cons:
      - "コンセントが必要で持ち運びには向かない"
      - "対応する器具との組み合わせを確認する必要がある"
    recommendedFor: "無香料タイプの取替えボトルをコスパ重視で選びたい方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00r136n.3rdw697f.g00r136n.3rdw7383/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Frakuten24%2Fe049873h%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Frakuten24%2Fi%2F10205931%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/rakuten24/cabinet/134/4902424427134.jpg?_ex=128x128"
    offers:
      - provider: "yahoo"
        label: "Yahoo!"
        url: "https://ck.jp.ap.valuecommerce.com/servlet/referral?sid=3770852&pid=892615315&vc_url=https%3A%2F%2Fstore.shopping.yahoo.co.jp%2Fat-tree%2F101-r001-4902424427134.html"
        matchStatus: "matched"
        updatedAt: "2026-09-07"
        price: 1440
        rating: 0
        reviewCount: 2
        imageUrl: "https://item-shopping.c.yimg.jp/i/j/at-tree_101-r001-4902424427134"
        available: true
    genreId: "553423"
  - rank: 2
    name: "アースノーマット 取替えボトル 90日用 無香料 2本入"
    brand: "アース製薬（アースノーマット）"
    price: 1942
    capacity: "90日用×2本（180日）"
    pricePerUnit: "約11円/日"
    rating: 4.67
    reviewCount: 39
    features:
      - "プラグ式液体蚊取りの取替えボトル"
      - "無香料タイプ"
      - "長期用ボトルのまとめ入り"
    pros:
      - "シーズンを通して買い足さずに使える"
      - "無香料なので寝室でも使いやすい"
      - "定番規格なので手持ちの本体に使える"
    cons:
      - "対応する本体を別途用意する必要がある"
      - "シーズンをまたぐと保管期間が長くなる"
    recommendedFor: "液体蚊取りをシーズン通して使う方・無香料を選びたい方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00r136n.3rdw697f.g00r136n.3rdw7383/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Frakuten24%2Fa001200200483%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Frakuten24%2Fi%2F10114180%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/rakuten24/cabinet/011/4901080121011.jpg?_ex=128x128"
    genreId: "553423"
  - rank: 3
    name: "ノーマット 取替えボトル 90日用 微香性"
    brand: "アース製薬"
    price: 2068
    capacity: "90日用×2本（180日）"
    pricePerUnit: "約11円/日"
    rating: 5
    reviewCount: 1
    features:
      - "1本で長期間使えるロングタイプの取替えボトル"
      - "プラグ式ノーマット器具に対応した液体式タイプ"
      - "ほのかに香る微香性タイプ"
    pros:
      - "交換頻度を抑えたいシーズン通しの使用に向いている"
      - "ほんのり香るタイプが好みの方に合う"
      - "まとめてストックしておきたい家庭向き"
    cons:
      - "無香タイプを好む方には香りが気になる場合がある"
      - "1本あたりの購入単価はやや高め"
    recommendedFor: "交換の手間を減らして長期間使いたい方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00rqszn.3rdw6b20.g00rqszn.3rdw7361/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Fmatsukiyo%2F4901080121417%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Fmatsukiyo%2Fi%2F10529670%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/matsukiyo/cabinet/i0005/4901080121417_1.jpg?_ex=128x128"
    offers:
      - provider: "yahoo"
        label: "Yahoo!"
        url: "https://ck.jp.ap.valuecommerce.com/servlet/referral?sid=3770852&pid=892615315&vc_url=https%3A%2F%2Fstore.shopping.yahoo.co.jp%2Fhcvalor%2Fvh-2162902907264.html"
        matchStatus: "matched"
        updatedAt: "2026-09-07"
        price: 4071
        rating: 0
        reviewCount: 0
        imageUrl: "https://item-shopping.c.yimg.jp/i/j/hcvalor_vh-2162902907264"
        available: true
    genreId: "100695"
  - rank: 4
    name: "アースノーマット 取替えボトル 90日用 無香料 2箱セット"
    brand: "アース製薬（アースノーマット）"
    price: 3864
    capacity: "90日用×4本（360日）"
    pricePerUnit: "約11円/日"
    rating: 4.76
    reviewCount: 49
    features:
      - "長期用取替えボトルの2箱まとめ買い"
      - "無香料タイプ"
      - "複数部屋での併用を想定した構成"
    pros:
      - "寝室とリビングなど複数部屋で同時に使える"
      - "1日あたりの負担を抑えやすい"
      - "シーズン中の買い足しがほぼ不要になる"
    cons:
      - "使い切れないと翌シーズンまで保管することになる"
      - "対応する本体を部屋数分そろえる必要がある"
    recommendedFor: "複数の部屋で液体蚊取りを使う方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00r136n.3rdw697f.g00r136n.3rdw7383/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Frakuten24%2F62629%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Frakuten24%2Fi%2F10888488%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/rakuten24/cabinet/629/62629.jpg?_ex=128x128"
    genreId: "553423"
  - rank: 5
    name: "ノーマット 取替えボトル 60日用 無香料"
    brand: "アース製薬"
    price: 1690
    capacity: "60日用×2本（120日）"
    pricePerUnit: "約14円/日"
    rating: 4.6
    reviewCount: 20
    features:
      - "プラグ式ノーマット器具に対応した取替え用ボトル"
      - "電気式で火を使わない液体タイプ"
      - "においを抑えた無香料タイプ"
    pros:
      - "定番ブランドで対応器具を選びやすい"
      - "無香料で寝室にも取り入れやすい"
      - "標準的な使用期間で買い足ししやすい"
    cons:
      - "コンセント周辺での設置場所が必要"
      - "別売りの器具がない場合はセット品も検討したい"
    recommendedFor: "定番ブランドの無香料タイプを安心して使いたい方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00r136n.3rdw697f.g00r136n.3rdw7383/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Frakuten24%2Fa001200200219%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Frakuten24%2Fi%2F10114372%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/rakuten24/cabinet/113/4901080120113.jpg?_ex=128x128"
    genreId: "553423"
  - rank: 6
    name: "ノーマット 取替えボトル 30日用 無香料"
    brand: "アース製薬"
    price: 1210
    capacity: "30日用×2本（60日）"
    pricePerUnit: "約20円/日"
    rating: 4.73
    reviewCount: 22
    features:
      - "短期間の使用に向いた取替え用ボトル"
      - "プラグ式ノーマット器具に対応した液体タイプ"
      - "においが気になりにくい無香料タイプ"
    pros:
      - "短いシーズンや帰省・来客時のスポット利用に向く"
      - "少量から試したい方に取り入れやすい"
      - "無香料で場所を選ばず使いやすい"
    cons:
      - "対応日数あたりの単価は長期用より高くなりやすい"
      - "長く使う場合は交換頻度が増える"
    recommendedFor: "短期間だけ・お試しで使いたい方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00pl1pn.3rdw619a.g00pl1pn.3rdw796f/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Fkenkocom%2Fa125030h%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Fkenkocom%2Fi%2F10008513%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/kenkocom/cabinet/214/4901080308214.jpg?_ex=128x128"
    offers:
      - provider: "yahoo"
        label: "Yahoo!"
        url: "https://ck.jp.ap.valuecommerce.com/servlet/referral?sid=3770852&pid=892615315&vc_url=https%3A%2F%2Fstore.shopping.yahoo.co.jp%2Fatlife%2F101-x010-4901080120113.html"
        matchStatus: "review"
        updatedAt: "2026-08-19"
        price: 16119
        rating: 0
        reviewCount: 0
        imageUrl: "https://item-shopping.c.yimg.jp/i/j/atlife_101-x010-4901080120113"
        available: true
    genreId: "553423"
  - rank: 7
    name: "アースノーマット 取替えボトル 30日用 無香料 2本入"
    brand: "アース製薬（アースノーマット）"
    price: 1210
    capacity: "30日用×2本（60日）"
    pricePerUnit: "約20円/日"
    rating: 4.5
    reviewCount: 8
    features:
      - "短期用のプラグ式液体蚊取り取替えボトル"
      - "無香料タイプ"
      - "2本組の構成"
    pros:
      - "蚊が出る時期だけ短く使いたい場合に向く"
      - "保管期間が短く済むので劣化を気にしにくい"
      - "部屋数が少ない家庭でも使い切りやすい"
    cons:
      - "長期用より1日あたりの負担が大きい"
      - "シーズン中に買い足す回数が増える"
    recommendedFor: "必要な時期だけ液体蚊取りを使いたい方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00r136n.3rdw697f.g00r136n.3rdw7383/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Frakuten24%2Fa003200200023%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Frakuten24%2Fi%2F10121091%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/rakuten24/cabinet/214/4901080308214.jpg?_ex=128x128"
    offers:
      - provider: "yahoo"
        label: "Yahoo!"
        url: "https://ck.jp.ap.valuecommerce.com/servlet/referral?sid=3770852&pid=892615315&vc_url=https%3A%2F%2Fstore.shopping.yahoo.co.jp%2Fatlife%2F101-x010-90655.html"
        matchStatus: "pending"
        updatedAt: "2026-09-07"
        price: 16119
        rating: 0
        reviewCount: 0
        imageUrl: "https://item-shopping.c.yimg.jp/i/j/atlife_101-x010-90655"
        available: true
    genreId: "553423"
tags:
  - "液体蚊取り おすすめ"
  - "液体蚊取り コスパ"
  - "蚊取り 取替えボトル 比較"
  - "まとめ買い"
faqs:
  - question: "器具を持っていなくても使えますか？"
    answer: "取替えボトルは器具に挿して使うため、対応器具が必要です。初めての場合は器具とボトルがセットになった商品から始めると分かりやすいです。"
  - question: "無香料と微香性はどう選べばよいですか？"
    answer: "においが気になる寝室や食卓まわりでは無香料、ほんのり香りがほしい場合は微香性が選びやすいです。好みで選んで問題ありません。"
  - question: "対応日数は使い方で変わりますか？"
    answer: "1日あたりの使用時間や環境によって変わる場合があります。表示日数はあくまで目安として確認してください。"
---

## 買う前に、器具の側を確認する

取替えボトルは消耗品ですが、**どれでも挿せるわけではありません。** メーカーとシリーズごとに、ボトルの口や差し込む部分の形が決まっています。

- 手元の器具に書かれたメーカー名とシリーズ名を確認する
- 使っていたボトルが残っていれば、そのラベルを見るのが確実
- 器具を持っていない、または古くて調子が悪い場合は、器具とボトルがセットになった商品を選ぶ

**同じメーカーでも、シリーズが変わると使えないことがあります。** 安いボトルを見つけて買ったのに挿さらなかった、というのがこのカテゴリで最も多い失敗です。価格を比べるのは、対応するシリーズを絞ってからにしてください。

器具自体は数年使えますが、内部で液を吸い上げる部分が汚れると効きが落ちます。何シーズンも使っていて以前より効かないと感じるなら、ボトルではなく器具の買い替えを検討する場面です。

## 「60日用」が何を前提にした日数か

ボトルには30日用、60日用といった表示がありますが、これは**1日あたりの使用時間を前提にした目安**です。多くの場合、1日12時間程度の連続使用を想定しています。

つまり、朝から晩までつけっぱなしにすれば、表示より早くなくなります。逆に夜だけ使うなら、表示より長く持つこともあります。

- **夜だけ使う**：表示日数より長持ちしやすい
- **在宅時間が長く、日中もつけている**：表示日数より早く減る
- **複数の部屋で使う**：当然、本数が必要になる

**液の残量は見れば分かります。** 日数だけを信じず、たまに残りを確認して、切れる前に次を用意しておくのが確実です。シーズン中に切らすと、いちばん必要なときに買いに走ることになります。

## 部屋の広さと置き場所で効き方が変わる

液体蚊取りは、成分を空気中に広げて効かせるものです。したがって、**部屋の広さと空気の流れが結果を左右します**。

- **適用畳数を確認する**：広いリビングに小さい部屋向けのものを置いても、期待した状態にはなりません
- **締め切った部屋のほうが効く**：窓を開け放していると成分が外へ逃げます
- **置く高さ**：床に近いコンセントでも問題ありませんが、家具の陰や、カーテンの裏に隠れる位置は避けます
- **エアコンの風**：風が直接当たると、成分が一方向に流れて偏ります

**そもそも蚊が入ってくる経路をふさぐほうが効果的**な場合もあります。網戸の隙間、玄関の開け閉め、ベランダの植木鉢の受け皿にたまった水。室内の対策と合わせて考えると、必要なボトルの本数自体が減ります。

## 香りは、寝室で使うかどうかで決める

無香料と微香性のどちらを選ぶかは、使う部屋で判断すると分かりやすいです。

- **寝室**：無香料が無難です。就寝中に何時間も同じ空間にいるので、わずかな香りでも気になる人がいます
- **リビング・食卓まわり**：食事のにおいと混ざるのを避けたいなら無香料。気分の問題として香りが欲しいなら微香性
- **来客のある部屋**：好みが分かれるので、無香料のほうが安全です

香りの感じ方には個人差が大きく、家族のなかでも意見が割れる部分です。**まとめ買いの前に、1本試してから量を決める**ほうが失敗しません。

## 使用中の注意と、シーズンをまたぐ保管

使うときは、パッケージに書かれた使用条件を守ることが前提になります。

- 使用する部屋の広さ、換気の条件、連続使用時間の目安が商品ごとに定められています
- 小さな子どもやペットがいる家庭では、器具に触れない場所に設置します。魚や昆虫を飼っている場合は、商品ごとの注意表示を必ず確認してください
- 器具は熱を持つので、燃えやすいものやカーテンのそばには置きません

シーズンが終わって液が残った場合、**未開封のボトルは翌年も使えることが多い**です。ただし、直射日光の当たる場所や高温になる場所での保管は避けます。使いかけのボトルは器具から抜き、キャップがあれば閉めて立てて保管してください。

**翌シーズンまで持ち越す前提なら、大容量のまとめ買いも無駄になりません。** 逆に、その年に使い切りたいなら、残りの期間から必要日数を計算して買うほうが在庫が余りません。

## この記事が向いていない人

- **今いる蚊をすぐ止めたい**: 電気式の液体蚊取りは部屋の空気に薬剤を行きわたらせる方式なので、置いた瞬間に効くことは期待できません。効き始めるまでの時間を見込んで先に付ける使い方になります
- **屋外や玄関先で使いたい**: 電源が要るうえ、風がある場所では効き方が安定しません。持ち歩きや庭仕事は [虫よけスプレーの比較](/articles/insect-repellent-comparison/) が近い用途です
- **蚊以外の虫も一緒に何とかしたい**: 対象の虫は商品ごとに決まっています。衣類の虫食いは [防虫剤の比較](/articles/moth-repellent-comparison/) のほうが目的に合います
- **ペットや小さな子どもがいて薬剤自体を避けたい**: 使用条件は商品の表示に従う前提の商品です。避けたい事情があるなら、網戸や物理的な対策を先に検討してください

## まとめ

液体蚊取りの取替えボトルは、まず器具に対応しているかを確認し、そのうえで日数と香りを選びます。日数表示は使用時間を前提にした目安なので、実際の残量を見ながら次を用意しておくのが確実です。

効きが弱いと感じたときは、ボトルではなく部屋の広さ、置き場所、換気の状態を疑ってみてください。器具の劣化が原因のこともあります。

上部の比較表では1日あたりの価格で並べています。**対応するシリーズを確認してから**、日数と価格を見比べてください。

屋外や広い範囲の対策には別のタイプが向きます。持ち歩きや庭仕事なら [虫よけスプレーの比較](/articles/insect-repellent-comparison/) をご覧ください。

---

※ 価格は記事執筆時点の楽天市場での販売価格です。価格は変動する場合があります。
