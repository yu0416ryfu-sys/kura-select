---
title: "ハイドロコロイド絆創膏おすすめ7選【2026年】1枚単価とサイズで比較"
description: "ハイドロコロイド絆創膏のおすすめ7選を1枚あたりの単価とサイズで比較。キズパワーパッド、ケアリーヴ 治す力、徳用の救急バンまで、手指・すり傷・水仕事など傷の種類に合わせた選び方と、使わない方がよい傷の注意点を解説します。"
category: "bandage"
publishedAt: "2026-09-23"
articleType: "comparison"
updatedAt: "2026-09-26"
draft: false
products:
  - rank: 1
    name: "リーダー ハイドロ救急バン Mサイズ 100枚"
    brand: "リーダー（日進医療器）"
    price: 2637
    capacity: "100枚"
    pricePerUnit: "約26円/枚"
    rating: 5
    reviewCount: 3
    features:
      - "透明度の高いハイドロコロイド素材の救急バン"
      - "薄いウレタンフィルムをベースにしたやわらかい素材"
      - "日本製の徳用パック"
    pros:
      - "枚数が多く家族で使う常備用にまとめて確保しやすい"
      - "透明で目立ちにくく自然なつけ心地"
      - "ハイドロコロイドを日常的に使う家庭の補充用に向く"
    cons:
      - "使用頻度が低い家庭では使い切る前に期限が来るおそれがある"
      - "貼る前に手で温めるひと手間がある"
    recommendedFor: "ハイドロコロイドを家族で日常的に使う方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00ru0on.3rdw60b7.g00ru0on.3rdw7edf/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Fat-life%2F103-4955574823646%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Fat-life%2Fi%2F10412235%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/at-life/cabinet/zz/4955574823646.jpg?_ex=128x128"
    genreId: "204748"
    offers:
      - provider: "yahoo"
        label: "Yahoo!"
        url: "https://ck.jp.ap.valuecommerce.com/servlet/referral?sid=3770852&pid=892615315&vc_url=https%3A%2F%2Fstore.shopping.yahoo.co.jp%2Fnisshodrug%2F4955574823646.html"
        matchStatus: "pending"
        updatedAt: "2026-09-26"
        price: 2598
        rating: 0
        reviewCount: 0
        imageUrl: "https://item-shopping.c.yimg.jp/i/j/nisshodrug_4955574823646"
        available: true
  - rank: 2
    name: "キズクイックFit ジャストサイズ 12枚"
    brand: "キズクイック（東洋化学）"
    price: 600
    capacity: "12枚"
    pricePerUnit: "約50円/枚"
    rating: 4.37
    reviewCount: 19
    features:
      - "全面ハイドロコロイド膏体型の家庭用創傷パッド"
      - "薄型設計の小さめジャストサイズ"
      - "ウレタンフィルムを貼り合わせた構造"
    pros:
      - "小さな傷にちょうどよいサイズで無駄が出にくい"
      - "薄くて貼っていても違和感が少ない"
      - "さかむけや小さな靴ずれなどピンポイントに使いやすい"
    cons:
      - "サイズが小さいため広い傷には向かない"
      - "少量パックで常備用としては補充の頻度が上がる"
    recommendedFor: "小さな傷にピンポイントで貼りたい方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00povvn.3rdw6e24.g00povvn.3rdw7a27/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Fmisawa-jp%2F4960085990510%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Fmisawa-jp%2Fi%2F10173494%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/misawa-jp/cabinet/img038/4960085990510.jpg?_ex=128x128"
    genreId: "568428"
    offers:
      - provider: "yahoo"
        label: "Yahoo!"
        url: "https://ck.jp.ap.valuecommerce.com/servlet/referral?sid=3770852&pid=892615315&vc_url=https%3A%2F%2Fstore.shopping.yahoo.co.jp%2Fkenko-ex%2F1229-4960085990510.html"
        matchStatus: "pending"
        updatedAt: "2026-09-26"
        price: 605
        rating: 0
        reviewCount: 0
        imageUrl: "https://item-shopping.c.yimg.jp/i/j/kenko-ex_1229-4960085990510"
        available: true
  - rank: 3
    name: "ケアリーヴ 治す力 防水タイプ Mサイズ 12枚"
    brand: "ニチバン（ケアリーヴ）"
    price: 716
    capacity: "12枚"
    pricePerUnit: "約60円/枚"
    rating: 4.78
    reviewCount: 18
    features:
      - "傷口部分だけにハイドロコロイドのモイストパッドを配置"
      - "薄い透明フィルムで水を通さない防水タイプ"
      - "水に強い低刺激性粘着剤を配合"
    pros:
      - "透明フィルムで貼っていても目立ちにくい"
      - "シャワーや水仕事のある日にも使いやすい"
      - "パッド部分以外は薄く、ゴワつきを抑えやすい"
    cons:
      - "全面ハイドロコロイドのタイプより体液を保てる範囲は狭い"
      - "広いすり傷にはMサイズでは足りない場合がある"
    recommendedFor: "目立ちにくさと防水性を両立したい方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00r136n.3rdw697f.g00r136n.3rdw7383/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Frakuten24%2Fe459503h%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Frakuten24%2Fi%2F10556304%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/rakuten24/cabinet/026/4987167075026.jpg?_ex=128x128"
    genreId: "568428"
    offers:
      - provider: "yahoo"
        label: "Yahoo!"
        url: "https://ck.jp.ap.valuecommerce.com/servlet/referral?sid=3770852&pid=892615315&vc_url=https%3A%2F%2Fstore.shopping.yahoo.co.jp%2Fdenergy%2Fal-4987167075026-5set-ypt.html"
        matchStatus: "pending"
        updatedAt: "2026-09-26"
        price: 3170
        rating: 0
        reviewCount: 1
        imageUrl: "https://item-shopping.c.yimg.jp/i/j/denergy_al-4987167075026-5set-ypt"
        available: true
  - rank: 4
    name: "ケアリーヴ 治す力 Mサイズ 12枚"
    brand: "ニチバン（ケアリーヴ）"
    price: 819
    capacity: "12枚"
    pricePerUnit: "約68円/枚"
    rating: 4.3
    reviewCount: 10
    features:
      - "ハイドロコロイド素材のモイストパッドを使った管理医療機器"
      - "体液を吸収保持すると白くふくらむパッド"
      - "肌になじみやすいケアリーヴのテープ"
    pros:
      - "白くふくらむ様子で貼り替えの目安をつかみやすい"
      - "一般的な絆創膏と同じ感覚で貼りやすい形状"
      - "ドラッグストアでも見かけるブランドで買い足しやすい"
    cons:
      - "防水性を重視するなら防水タイプの方が向く"
      - "少量パックのため家族で使うと減りが早い"
    recommendedFor: "使い慣れたケアリーヴでハイドロコロイドを試したい方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00r136n.3rdw697f.g00r136n.3rdw7383/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Frakuten24%2Fe239190h%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Frakuten24%2Fi%2F10209999%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/rakuten24/cabinet/744/4987167065744.jpg?_ex=128x128"
    genreId: "568428"
    offers:
      - provider: "yahoo"
        label: "Yahoo!"
        url: "https://ck.jp.ap.valuecommerce.com/servlet/referral?sid=3770852&pid=892615315&vc_url=https%3A%2F%2Fstore.shopping.yahoo.co.jp%2Fjoshin%2F4987167065744-73-30001222.html"
        matchStatus: "pending"
        updatedAt: "2026-09-26"
        price: 587
        rating: 0
        reviewCount: 1
        imageUrl: "https://item-shopping.c.yimg.jp/i/j/joshin_4987167065744-73-30001222"
        available: true
  - rank: 5
    name: "キズパワーパッド ふつうサイズ 10枚入×2コセット"
    brand: "バンドエイド（BAND-AID）"
    price: 1493
    capacity: "20枚（10枚入×2コセット）"
    pricePerUnit: "約75円/枚"
    rating: 4.64
    reviewCount: 183
    features:
      - "ハイドロコロイド素材で傷口の湿潤環境を保つ救急用パッド"
      - "水や汚れの侵入を防ぐ防水設計"
      - "標準的なふつうサイズのセット品"
    pros:
      - "ハイドロコロイド絆創膏の定番で店頭でも同じものを買い足しやすい"
      - "指や腕の小さめのすり傷・切り傷に使いやすいサイズ"
      - "セット品なので家と持ち歩き用に分けておきやすい"
    cons:
      - "広いすり傷には大きめサイズが必要"
      - "指先や関節には専用形状の方が貼りやすい場合がある"
    recommendedFor: "まず定番のふつうサイズを常備しておきたい方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00r136n.3rdw697f.g00r136n.3rdw7383/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Frakuten24%2F22188%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Frakuten24%2Fi%2F10682606%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/rakuten24/cabinet/188/22188.jpg?_ex=128x128"
    offers:
      - provider: "yahoo"
        label: "Yahoo!"
        url: "https://ck.jp.ap.valuecommerce.com/servlet/referral?sid=3770852&pid=892615315&vc_url=https%3A%2F%2Fstore.shopping.yahoo.co.jp%2Fladydrugheartshop%2F4901730240017.html"
        matchStatus: "matched"
        updatedAt: "2026-07-29"
        price: 1267
        rating: 0
        reviewCount: 0
        imageUrl: "https://item-shopping.c.yimg.jp/i/j/ladydrugheartshop_4901730240017"
        available: true
    genreId: "568428"
  - rank: 6
    name: "キズパワーパッド 水仕事用 10枚入×2コセット"
    brand: "バンドエイド（BAND-AID）"
    price: 1493
    capacity: "20枚（10枚入×2コセット）"
    pricePerUnit: "約75円/枚"
    rating: 4.56
    reviewCount: 81
    features:
      - "水仕事を想定したハイドロコロイドパッド"
      - "指に巻きやすい細長い形状"
      - "水や汚れの侵入を防ぐ防水設計"
    pros:
      - "食器洗いや手洗いが多い日でもはがれにくさを重視できる"
      - "指のあかぎれ・さかむけなど手指の傷に使いやすい"
      - "セット品で手荒れしやすい季節にも備えやすい"
    cons:
      - "腕やひざなど平らな部位にはふつうサイズの方が向く"
      - "手指以外に使う機会が少ない家庭では出番が限られる"
    recommendedFor: "家事で手指を濡らすことが多い方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00r136n.3rdw697f.g00r136n.3rdw7383/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Frakuten24%2F22191%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Frakuten24%2Fi%2F10682608%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/rakuten24/cabinet/191/22191.jpg?_ex=128x128"
    offers:
      - provider: "yahoo"
        label: "Yahoo!"
        url: "https://lohaco.yahoo.co.jp/store/h-lohaco/item/h219465/"
        matchStatus: "pending"
        updatedAt: "2026-09-26"
        price: 1495
        rating: 4.75
        reviewCount: 57
        imageUrl: "https://item-shopping.c.yimg.jp/i/j/rd-lohaco_h219465"
        available: true
    genreId: "568428"
  - rank: 7
    name: "キズパワーパッド 大きめサイズ 12枚"
    brand: "バンドエイド（BAND-AID）"
    price: 1343
    capacity: "12枚"
    pricePerUnit: "約112円/枚"
    rating: 4.82
    reviewCount: 84
    features:
      - "広めの面積を覆える大きめサイズのハイドロコロイドパッド"
      - "水や汚れの侵入を防ぐ防水設計"
      - "ひざやひじのすり傷を想定したサイズ"
    pros:
      - "転んだときのすり傷など広い範囲をまとめて覆いやすい"
      - "ふつうサイズを並べて貼るより段差ができにくい"
      - "救急箱に入れておくと出番が来たときに困りにくい"
    cons:
      - "小さな傷にはサイズが大きく1枚あたりの単価も上がる"
      - "出番が限られるため多めに買うと使い切りにくい"
    recommendedFor: "ひざ・ひじのすり傷用に大判を用意しておきたい方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00pl1pn.3rdw619a.g00pl1pn.3rdw796f/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Fkenkocom%2Fx455460h%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Fkenkocom%2Fi%2F10267692%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/kenkocom/cabinet/002/4901730077002.jpg?_ex=128x128"
    offers:
      - provider: "yahoo"
        label: "Yahoo!"
        url: "https://ck.jp.ap.valuecommerce.com/servlet/referral?sid=3770852&pid=892615315&vc_url=https%3A%2F%2Fstore.shopping.yahoo.co.jp%2Fladydrugheartshop-ni%2F34901730077002.html"
        matchStatus: "matched"
        updatedAt: "2026-09-23"
        price: 1329
        rating: 0
        reviewCount: 0
        imageUrl: "https://item-shopping.c.yimg.jp/i/j/ladydrugheartshop-ni_34901730077002"
        available: true
    genreId: "568428"
tags:
  - "ハイドロコロイド 絆創膏 おすすめ"
  - "キズパワーパッド 比較"
  - "ハイドロコロイド 絆創膏 コスパ"
  - "湿潤療法 絆創膏"
  - "まとめ買い"
faqs:
  - question: "ハイドロコロイド絆創膏は普通の絆創膏と何が違いますか？"
    answer: "一般的な絆創膏はパッドで傷口を覆い1日1〜2回貼り替えるのが前提ですが、ハイドロコロイドは傷から出る体液を保って湿潤環境をつくる設計で、傷の状態を見ながら数日貼ったままにする使い方をします。貼り替えの目安は製品ごとに異なるため、箱や添付文書の記載に従ってください。"
  - question: "どのサイズを買えばいいですか？"
    answer: "よくする傷から決めるのが確実です。手指の傷が多いなら水仕事用や指用、腕や足の小さな傷ならふつうサイズ、ひざやひじのすり傷には大きめサイズが向きます。1種類だけ常備するならふつうサイズが使いやすいです。"
  - question: "白くふくらんできたら貼り替えるべきですか？"
    answer: "白くふくらむのは体液を吸収しているためで、すぐに貼り替える必要はない製品が多いです。ふくらみがパッドの端まで広がったり、端から漏れてきたりしたら貼り替えの目安です。具体的な目安は各製品の説明に従ってください。"
  - question: "どんな傷には使わない方がよいですか？"
    answer: "深い傷や出血が止まらない傷、動物にかまれた傷、赤く腫れる・膿が出るなど化膿が疑われる傷、広い範囲のやけどには使わず、医師や薬剤師に相談してください。貼っている間に痛みや腫れが強くなった場合も使用を中止してください。"
---

ハイドロコロイド絆創膏は、傷から出る体液を保って湿潤環境をつくるタイプの救急用パッドです。この記事では、キズパワーパッドやケアリーヴ 治す力などを、サイズ・防水性・枚数・1枚あたりの単価で比較します。

普段使いのガーゼパッド付き絆創膏とは使い方も価格帯も違うため、一般的な絆創膏は [絆創膏の比較](/articles/bandage-comparison/) に分けています。

## 一般的な絆創膏と同じ感覚で選ぶと失敗する

ハイドロコロイドは、1日に何度も貼り替える一般的な絆創膏とは前提が違います。傷の状態を見ながら**数日貼ったままにする**使い方が基本なので、1枚あたりの単価が高く見えても、実際に使う枚数は少なくなります。

### 単価は「同じサイズ同士」で比べる

同じブランドでも、ふつう・大きめ・指用・水仕事用で1枚あたりの価格が変わります。上部の比較表では単価で並べていますが、**サイズが違う商品同士の単価差はそのまま比較できません**。まず必要なサイズを決め、そのサイズの中で単価を比べてください。

### 白くふくらんだら貼り替えの目安

ハイドロコロイドは体液を吸うと白くふくらみます。ふくらみがパッドの端まで広がったり、端から漏れてきたりしたら貼り替えの目安です。貼り替えの目安や使用できる日数は製品ごとに違うので、**必ず箱や添付文書の記載に従ってください**。

## どのサイズを常備するかは「よくする傷」から決める

### 指先・手指の傷：水仕事用や指用

あかぎれ・さかむけ・包丁での切り傷など、手指の傷が多いなら、キズパワーパッド 水仕事用のように指に巻きやすい形状が向きます。平らなパッドを指に貼ると端が浮きやすく、せっかくの防水性が生かせません。

### 腕や足の小さな傷：ふつうサイズ

家庭で最も出番が多いのはふつうサイズ（Mサイズ）です。1種類だけ常備するならここから選び、使う頻度が高い家庭はリーダー ハイドロ救急バンのような徳用パックで単価を抑えるのも選択肢です。

### ひざ・ひじのすり傷：大きめサイズ

子どもが転んだときのすり傷や、スポーツでの擦過傷には大きめサイズを使います。**小さいパッドを何枚も並べて貼ると継ぎ目から水が入りやすい**ので、傷全体を1枚で覆えるサイズを用意しておきます。

### 小さな傷にピンポイントで：ジャストサイズ

キズクイックFit ジャストサイズのような小さめのパッドは、小さな靴ずれやさかむけなど、ふつうサイズでは余る傷に向きます。

## 防水タイプと全面タイプの違い

ハイドロコロイド絆創膏には、パッド全体がハイドロコロイドの**全面タイプ**と、傷に当たる部分だけハイドロコロイドで周囲を薄い防水フィルムにした**パッド部分タイプ**があります。

- **全面タイプ**（キズパワーパッド、キズクイックFit など）：体液を保てる面積が広く、厚みがある分クッション性もあります
- **パッド部分タイプ**（ケアリーヴ 治す力 防水タイプ など）：周囲が薄い透明フィルムなので目立ちにくく、ゴワつきを抑えやすい反面、体液を保てる範囲はパッド部分に限られます

見た目を優先するか、傷を覆う面積を優先するかで選び分けてください。

## ハイドロコロイドを使わない方がよい傷

ハイドロコロイドはすべての傷に使えるわけではありません。各製品の添付文書でも使用を避けるよう案内されているケースがあるので、次のような場合は使わずに医師や薬剤師に相談してください。

- **深い傷・出血が止まらない傷・動物にかまれた傷**
- **赤く腫れている・熱を持っている・膿が出ているなど、化膿が疑われる傷**
- **広い範囲のやけどや、水ぶくれが大きく破れているやけど**
- **貼っている間に痛みや腫れが強くなった場合**

また、貼る前には傷口を水道水でよく洗い、周囲の水気を拭き取ってから貼ります。消毒薬や軟膏を塗った上から貼ると粘着しにくくなる製品が多いので、使い方は箱の記載を確認してください。

## この記事が向いていない人

- **普段の小さな傷用に安い絆創膏をまとめ買いしたい人**。一般的な絆創膏の単価比較は [絆創膏の比較](/articles/bandage-comparison/) を参照してください
- **ニキビを覆うパッチを探している人**。同じハイドロコロイド系でも用途が違うため、[ニキビパッチの比較](/articles/acne-patch-comparison/) で扱っています
- **子ども向けのキャラクター柄を探している人**。この記事では扱っていません

## まとめ

ハイドロコロイド絆創膏は、**よくする傷に合ったサイズを1〜2種類**常備し、同じサイズの中で単価を比べるのが基本です。手指の傷が多いなら水仕事用、腕や足ならふつうサイズ、すり傷用に大きめサイズを少量、と役割を分けておくと無駄がありません。傷の状態によっては使えない場合もあるため、迷ったら医師や薬剤師に相談してください。

患部を冷やす場面では [冷却ジェル・保冷剤の比較](/articles/cooling-pack-comparison/) もあわせてご覧ください。

---

※ 価格は記事執筆時点の楽天市場での販売価格です。価格は変動する場合があります。
