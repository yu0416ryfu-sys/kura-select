---
title: "除菌スプレー コスパ最強ランキング【2026年版】濃度・容量で比較"
description: "除菌スプレー・アルコールを1mLあたりのコストで徹底比較。パストリーゼ・カビキラー除菌・フマキラーなど人気9選を解説します。"
category: "sanitizing-spray"
publishedAt: "2026-04-30"
articleType: "comparison"
updatedAt: "2026-09-03"
products:
  - rank: 1
    name: "カビキラー アルコール除菌 キッチン用 詰め替え用【カビキラー】[アルコールスプ"
    brand: "ジョンソン（カビキラー）"
    price: 1130
    capacity: "（1260mL）"
    pricePerUnit: "約0.90円/mL"
    rating: 4.65
    reviewCount: 20
    features:
      - "高濃度アルコールで除菌力が高い"
      - "食品添加物のみで構成された安全処方"
      - "まな板・包丁・テーブルなど多用途"
    pros:
      - "食品添加物のみで安全性が高い"
      - "3本セットでたっぷり使える"
      - "カビキラーブランドの信頼性"
    cons:
      - "詰め替え作業がやや手間"
      - "噴射量の調整がしにくい"
    recommendedFor: "安全性を重視する方・まな板や調理器具の除菌に・まとめ買い派の方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00r136n.3rdw697f.g00r136n.3rdw7383/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Frakuten24%2F4901609016491%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Frakuten24%2Fi%2F11230047%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/rakuten24/cabinet/491/4901609016491.jpg?_ex=128x128"
    offers:
      - provider: "yahoo"
        label: "Yahoo!"
        url: "https://lohaco.yahoo.co.jp/store/h-lohaco/item/ax59783/"
        matchStatus: "pending"
        updatedAt: "2026-09-01"
        price: 1200
        rating: 4.74
        reviewCount: 65
        imageUrl: "https://item-shopping.c.yimg.jp/i/j/rd-lohaco_ax59783"
        available: true
  - rank: 2
    name: "フマキラー キッチン用 アルコール除菌スプレー つめかえ用【フマキラー アルコー"
    brand: "フマキラー"
    price: 657
    capacity: "（720mL）"
    pricePerUnit: "約0.91円/mL"
    rating: 4.69
    reviewCount: 62
    features:
      - "100%天然由来のアルコール成分"
      - "食器・調理器具にそのまま使える"
      - "大容量の詰め替え2本セットでお得"
    pros:
      - "1mLあたり約0.62円で最安クラス"
      - "天然由来成分で食品まわりも安心"
      - "2本セットでまとめ買いコスパ◎"
    cons:
      - "アルコール濃度はパストリーゼより低い"
      - "スプレーボトルは別途必要"
    recommendedFor: "コスパ最優先の方・キッチン用がメインの方・大量に使う方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00r136n.3rdw697f.g00r136n.3rdw7383/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Frakuten24%2F4902424441727%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Frakuten24%2Fi%2F10675585%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/rakuten24/cabinet/727/4902424441727.jpg?_ex=128x128"
    offers:
      - provider: "yahoo"
        label: "Yahoo!"
        url: "https://ck.jp.ap.valuecommerce.com/servlet/referral?sid=3770852&pid=892615315&vc_url=https%3A%2F%2Fstore.shopping.yahoo.co.jp%2Ftsuruha%2F10130857.html"
        matchStatus: "pending"
        updatedAt: "2026-09-01"
        price: 445
        rating: 4.67
        reviewCount: 9
        imageUrl: "https://item-shopping.c.yimg.jp/i/j/tsuruha_10130857"
        available: true
  - rank: 3
    name: "手ピカスプレー 消毒用アルコール"
    brand: "健栄製薬（手ピカ）"
    price: 4961
    capacity: "420ml×10個"
    pricePerUnit: "約1.2円/mL"
    rating: 5
    reviewCount: 12
    features:
      - "指定医薬部外品の確実な除菌効果"
      - "手指消毒専用の安全処方"
      - "ヒアルロン酸配合で手荒れしにくい"
    pros:
      - "医薬部外品で除菌効果が保証されている"
      - "保湿成分配合で手が荒れにくい"
      - "手指専用で安心して使える"
    cons:
      - "手指専用のため物の除菌には不向き"
      - "容量に対してやや割高"
    recommendedFor: "手指消毒がメインの方・手荒れが気になる方・医薬部外品を求める方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00ru0on.3rdw60b7.g00ru0on.3rdw7edf/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Fat-life%2F103-4987286415437-10%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Fat-life%2Fi%2F12254363%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/at-life/cabinet/yy/4987286415437.jpg?_ex=128x128"
  - rank: 4
    name: "微酸性次亜塩素酸水"
    brand: "すこやかこまち"
    price: 1540
    capacity: "1L"
    pricePerUnit: "約1.5円/mL"
    rating: 4.7
    reviewCount: 948
    features:
      - "除菌・消臭用途のスプレーまたは液剤"
      - "靴や室内など用途別に選びやすい"
      - "詰替やセット品も選べる"
    pros:
      - "気になる場所に使いやすい"
      - "容量違いを選びやすい"
      - "ストック向き"
    cons:
      - "対象素材への使用可否確認が必要"
      - "成分や濃度の確認が必要"
    recommendedFor: "除菌スプレーを用途と容量で比較したい方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00rt83n.3rdw6209.g00rt83n.3rdw7241/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Fsukoyakakomachi%2Faqarfa400ppm23%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Fsukoyakakomachi%2Fi%2F10000011%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_gold/sukoyakakomachi/banner/r_syohin_af8001.jpg?_ex=128x128"
    offers:
      - provider: "yahoo"
        label: "Yahoo!"
        url: "https://ck.jp.ap.valuecommerce.com/servlet/referral?sid=3770852&pid=892615315&vc_url=https%3A%2F%2Fstore.shopping.yahoo.co.jp%2Fsukoyakakomachi%2Faqarfa1l.html"
        matchStatus: "matched"
        updatedAt: "2026-08-08"
        price: 1540
        rating: 4.92
        reviewCount: 12
        imageUrl: "https://item-shopping.c.yimg.jp/i/j/sukoyakakomachi_aqarfa1l"
        available: true
  - rank: 5
    name: "微酸性次亜塩素酸水 スプレー 500mL×3本"
    brand: "すこやかこまち"
    price: 4290
    capacity: "500ml×3本"
    pricePerUnit: "約2.9円/mL"
    rating: 4.78
    reviewCount: 431
    features:
      - "除菌・消臭用途のスプレーまたは液剤"
      - "靴や室内など用途別に選びやすい"
      - "詰替やセット品も選べる"
    pros:
      - "気になる場所に使いやすい"
      - "容量違いを選びやすい"
      - "ストック向き"
    cons:
      - "対象素材への使用可否確認が必要"
      - "成分や濃度の確認が必要"
    recommendedFor: "除菌スプレーを用途と容量で比較したい方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00rt83n.3rdw6209.g00rt83n.3rdw7241/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Fsukoyakakomachi%2Faqarfa500mlx3%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Fsukoyakakomachi%2Fi%2F10000008%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_gold/sukoyakakomachi/banner/r_syohin_af5003.jpg?_ex=128x128"
  - rank: 6
    name: "シュシュキッキ 除菌消臭ミスト"
    brand: "シュシュキッキ"
    price: 1518
    capacity: "300ml"
    pricePerUnit: "約5.1円/mL"
    rating: 4.4
    reviewCount: 880
    features:
      - "除菌・消臭用途のスプレーまたは液剤"
      - "靴や室内など用途別に選びやすい"
      - "詰替やセット品も選べる"
    pros:
      - "気になる場所に使いやすい"
      - "容量違いを選びやすい"
      - "ストック向き"
    cons:
      - "対象素材への使用可否確認が必要"
      - "成分や濃度の確認が必要"
    recommendedFor: "除菌スプレーを用途と容量で比較したい方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00ptlfn.3rdw66e3.g00ptlfn.3rdw7472/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Fnakano-d%2Fkesisaru-250%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Fnakano-d%2Fi%2F10015729%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/nakano-d/cabinet/brand/bracebe/pic/chouchoukikki_300_1.jpg?_ex=128x128"
  - rank: 7
    name: "ブテナロック 除菌抗菌スプレー 180ml"
    brand: "ブテナロック"
    price: 1100
    capacity: "180ml"
    pricePerUnit: "約6.1円/mL"
    rating: 4.65
    reviewCount: 4111
    features:
      - "除菌・消臭用途のスプレーまたは液剤"
      - "靴や室内など用途別に選びやすい"
      - "詰替やセット品も選べる"
    pros:
      - "気になる場所に使いやすい"
      - "容量違いを選びやすい"
      - "ストック向き"
    cons:
      - "対象素材への使用可否確認が必要"
      - "成分や濃度の確認が必要"
    recommendedFor: "除菌スプレーを用途と容量で比較したい方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00telan.3rdw6aaf.g00telan.3rdw7f40/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Fhisamitsu%2F4987188317853%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Fhisamitsu%2Fi%2F10000030%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/hisamitsu/cabinet/4987188317853.jpg?_ex=128x128"
    offers:
      - provider: "yahoo"
        label: "Yahoo!"
        url: "https://ck.jp.ap.valuecommerce.com/servlet/referral?sid=3770852&pid=892615315&vc_url=https%3A%2F%2Fstore.shopping.yahoo.co.jp%2Fhisamitsukenkou%2F4987188318706.html"
        matchStatus: "matched"
        updatedAt: "2026-08-19"
        price: 1100
        imageUrl: "https://item-shopping.c.yimg.jp/i/j/hisamitsukenkou_4987188318706"
        available: true
        rating: 4.45
        reviewCount: 31
  - rank: 8
    name: "シュシュキッキ 除菌消臭ミスト 携帯サイズ"
    brand: "シュシュキッキ"
    price: 550
    capacity: "20ml"
    pricePerUnit: "約28円/mL"
    rating: 4.26
    reviewCount: 945
    features:
      - "除菌・消臭用途のスプレーまたは液剤"
      - "靴や室内など用途別に選びやすい"
      - "詰替やセット品も選べる"
    pros:
      - "気になる場所に使いやすい"
      - "容量違いを選びやすい"
      - "ストック向き"
    cons:
      - "対象素材への使用可否確認が必要"
      - "成分や濃度の確認が必要"
    recommendedFor: "除菌スプレーを用途と容量で比較したい方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00ptlfn.3rdw66e3.g00ptlfn.3rdw7472/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Fnakano-d%2Fkesisaru-20%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Fnakano-d%2Fi%2F10015727%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/nakano-d/cabinet/brand/bracebe/pic/chouchoukikki_20_1.jpg?_ex=128x128"
    offers:
      - provider: "yahoo"
        label: "Yahoo!"
        url: "https://ck.jp.ap.valuecommerce.com/servlet/referral?sid=3770852&pid=892615315&vc_url=https%3A%2F%2Fstore.shopping.yahoo.co.jp%2Fnakano-dy%2Fkesisaru-20.html"
        matchStatus: "pending"
        updatedAt: "2026-08-19"
        price: 550
        imageUrl: "https://item-shopping.c.yimg.jp/i/j/nakano-dy_kesisaru-20"
        available: true
        rating: 4.28
        reviewCount: 29
  - rank: 9
    name: "＆wpaia 除菌消臭スプレーセット"
    brand: "＆wpaia"
    price: 4170
    capacity: "100ml+300ml+600ml"
    pricePerUnit: "約42円/ml"
    rating: 4.54
    reviewCount: 363
    features:
      - "除菌・消臭用途のスプレーまたは液剤"
      - "靴や室内など用途別に選びやすい"
      - "詰替やセット品も選べる"
    pros:
      - "気になる場所に使いやすい"
      - "容量違いを選びやすい"
      - "ストック向き"
    cons:
      - "対象素材への使用可否確認が必要"
      - "成分や濃度の確認が必要"
    recommendedFor: "除菌スプレーを用途と容量で比較したい方"
    rakutenUrl: "https://hb.afl.rakuten.co.jp/hgc/g00tfaun.3rdw63b0.g00tfaun.3rdw7d50/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Fandwpaia%2Fandwpaia_100x300x600_copy%2F&m=http%3A%2F%2Fm.rakuten.co.jp%2Fandwpaia%2Fi%2F10000035%2F&rafcid=wsc_i_is_42b71141-7589-447e-ab9f-1c5e97e9d61f"
    imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/andwpaia/cabinet/07621536/imgrc0089044303.jpg?_ex=128x128"
tags:
  - "除菌スプレー"
  - "アルコール"
  - "コスパ"
  - "節約"
  - "日用品"
  - "衛生"
draft: false
faqs:
  - question: "アルコール濃度が高いほど除菌力は強い？"
    answer: "70〜80%が最も除菌効果が高いとされています。90%以上は揮発が早すぎて、菌に十分作用する前に蒸発してしまうため、かえって効果が下がります。"
  - question: "ノンアルコール除菌スプレーは効果がある？"
    answer: "第四級アンモニウム塩などの除菌成分を使用したノンアルコール製品でも一定の除菌効果はあります。ただし、ノロウイルスなどアルコール耐性のあるウイルスに対しては、次亜塩素酸ナトリウムが有効です。"
  - question: "除菌と殺菌の違いは？"
    answer: "「殺菌」は薬機法上、医薬品・医薬部外品のみ使える表現です。「除菌」は雑貨品でも使え、菌を減らす効果を意味します。実用上の効果は製品の有効成分と濃度で判断しましょう。"
  - question: "除菌スプレーを手作りできる？"
    answer: "無水エタノールを精製水で70〜80%に希釈すれば手作り可能です。ただし、コストメリットは薄く、品質の安定性も市販品に劣ります。安全性と手軽さの面から市販品の利用をおすすめします。"
---

## 「除菌スプレー」は法律上の区分が3つに分かれている

同じ棚に並んでいても、除菌スプレーは表示のうえで別々の区分に属しています。ここを知らずに選ぶと、手に使えないものを手指用として買ったり、食器に使えないものをキッチンで使ったりすることになります。

**指定医薬部外品**は、手指の消毒を目的として承認された製品です。パッケージに「指定医薬部外品」と明記され、効能の表現が認められています。人の手指に使う目的ならこの区分を選びます。

**食品添加物**のみで構成された製品は、口に入るものに触れる場所で使う前提で作られています。まな板、包丁、食器、食品を扱う調理台などに使えるのはこの区分です。表示に「食品添加物」とあるかどうかで判断します。

<strong>雑貨（それ以外）</strong>は、物の表面を拭く用途の製品です。ドアノブ、テーブル、スイッチまわりなど、直接口に入らない場所に使います。手指用としては作られていません。

用途と区分が合っていないと、効果の問題以前に、想定されていない使い方になります。**買う前にパッケージのどこかにある区分表示を見る**のが、最も確実な選び方です。

## アルコール濃度は高ければよいわけではない

アルコール（エタノール）を使った製品では、濃度が使用感と適した場面を左右します。一般に、消毒用として広く使われてきたのは60〜80%程度の範囲です。

意外に思われるのが、**濃度が高すぎても目的に合わないことがある**点です。無水に近いエタノールは蒸発が非常に速く、対象物の表面に留まっている時間が短くなります。水分がある程度含まれているほうが、対象に馴染む時間を確保しやすいとされています。

濃度が低い製品は、その分だけ刺激が穏やかで、手荒れしやすい人や素材を傷めたくない場面で選ばれます。アルコールに過敏な体質の方や、小さな子どもがいる家庭では、アルコールを使わないタイプという選択肢もあります。

いずれの場合も、パッケージに記載された用途と使用方法に従うことが前提です。濃度の数字だけを比べて選ぶのではなく、**どこに使いたいか**から逆算してください。

## 素材によっては使えない場所がある

アルコールは油脂や樹脂を溶かす性質があります。そのため、拭いてよい場所とそうでない場所があります。

避けたほうがよい代表例は、**塗装面、ニス塗りの木部、革製品、一部のプラスチック**です。テレビや液晶画面のコーティング、眼鏡のレンズコートも傷むことがあります。白く曇る、べたつく、色が移る、といった変化は元に戻りません。

初めて使う場所では、目立たない部分で試してから広げるのが安全です。特に賃貸住宅の建具や家具は、変色すると原状回復の問題になりかねません。

また、**次亜塩素酸系の製品と混ぜない**ことも重要です。用途が近いため同じ場所に置きがちですが、種類の違う洗浄剤や漂白剤と併用する場合は、それぞれの注意書きを確認してください。

## 保管で気をつけること

アルコールを含む製品は引火性があります。これは濃度が高いほど顕著で、扱いには前提となる注意があります。

- **火気の近くで使わない・置かない**。コンロまわりで使うときは必ず火を止めてから
- **直射日光の当たる場所や車内に置かない**。高温になると容器が変形したり内圧が上がったりします
- **詰め替えるときは容器の材質を確認する**。アルコールに対応していない容器では、劣化したり成分が漏れたりすることがあります
- **開封後は早めに使い切る**。揮発によって濃度が変わることがあります

詰め替え用の大容量パックを使う場合、スプレーボトルは繰り返し使うことになります。ノズルの内部に汚れが残ると詰まりの原因になるので、詰め替えのタイミングで容器を洗い、乾かしてから入れてください。

## 用途を分けるとコストが下がる

一本ですべてを賄おうとすると、どうしても高い区分の製品を全用途に使うことになります。手指に使える製品でテーブルを拭き、食品に使える製品でドアノブを拭く、という状態です。

用途ごとに分けると、それぞれ適した製品を選べるうえ、消費のバランスも取れます。食品まわりだけ食品添加物の製品を使い、物の表面は雑貨区分の製品で拭く、という分け方が現実的です。

詰め替え用の大容量を使うと、スプレーボトルを買い直す必要がなくなるぶん負担は下がります。ただし前述のとおり保管には条件があるので、置き場所を確保できる範囲にとどめてください。

具体的な価格や1mLあたりの単価は変動するため、ページ上部の比較表（自動更新）でご確認ください。

## まとめ

除菌スプレーは、まず**指定医薬部外品・食品添加物・雑貨**のどれに当たるかを確認し、使いたい場所に合った区分を選ぶのが出発点です。濃度は高ければよいというものではなく、蒸発の速さや刺激の強さとの兼ね合いで決まります。

塗装面や革、画面のコーティングなど使えない素材があること、引火性があるため火気と高温を避けることは、どの製品でも共通の注意点です。用途ごとに分けて使うと、それぞれ適したものを選べます。

キッチンまわりの衛生管理は他の道具とも関わります。あわせて [キッチン泡ハイターなど台所用漂白剤の比較](/articles/kitchen-bleach-comparison/) もご覧ください。

> ※ 掲載価格は執筆時点での楽天市場における販売価格です。価格は変動する場合があります。最新の価格は各商品ページでご確認ください。
