...
export const CATEGORY_SEARCH_RULES: Record<string, RawSearchRule> = {
  ...
  'bath-drain-hair-catcher': {
    ...
    // DEFAULT_EXCLUDE_TERMS の 'ホルダー' は収納用品を弾くためのもの。このカテゴリでは
    // 「排水口ホルダー」「ネットホルダー」が主題の部品名そのもので、include の 'ネットホルダー' も
    // これが無いと常に落ちていた（rank3 監査 2026-09-10・ミューファン ヘアーキャッチャー）
    excludeContextExceptions: {
      'ホルダー': [/排水口ホルダー|ネットホルダー/],
    },
    units: ['個'],
  },
  'bath-mold-preventer': {
  ...
  'gift': {
    ...
    // 他の行事名は「母の日専用ギフト」を弾くためのもの。出品名に '父の日' も並んでいるなら
    // 用途の列挙（「父の日 母の日 入学祝 … クリスマス」）なので除外しない（rank3 監査 2026-09-10・LAMY safari）。
    // 範囲で覆う方式なので、父の日との間にある出現もまとめて例外になる
    excludeContextExceptions: {
      '母の日': [/父の日.*母の日|母の日.*父の日/],
      'ホワイトデー': [/父の日.*ホワイトデー|ホワイトデー.*父の日/],
      'クリスマス': [/父の日.*クリスマス|クリスマス.*父の日/],
      'バレンタイン': [/父の日.*バレンタイン|バレンタイン.*父の日/],
    },
    units: ['本', '足', '個', '枚', '組', 'セット'],
    minScore: 3,
  },
  ...
  'hand-cream': {
    ...
    // 'ユースキンA' は rank2 監査（2026-09-04）で「カテゴリ語なし」になった詰め替えパウチの出品名対策。
    // 'ユースキン' 単体はローション（シソラ）等も含むので入れない。NFKC 小文字化で 'ユースキンAa' にも一致する
    include: ['ハンドクリーム', 'ハンドケア', '手荒れ', '保湿クリーム', 'ユースキンA'],
    exclude: ['フェイス', 'ボディ', 'リップ', '日焼け止め', '除菌ジェル'],
    units: ['g', 'ml', '個'],
  },
  'kitchen-bleach': {
    keywords: ['キッチン漂白剤', 'キッチンハイター', '台所用漂白剤'],
    // 'ブリーチ' 単体はヘアブリーチを拾うので、台所用の商品名の形に限定する（rank2 監査 2026-09-04）
    // 'オキシクリーン' は記事が酸素系も比較対象にしているため（rank3 監査 2026-09-10・出品名に「漂白剤」が無い）
    include: ['キッチンハイター', 'キッチン漂白', '台所用漂白', '漂白剤', '除菌漂白', 'キッチンパワーブリーチ', 'キッチンブリーチ', 'オキシクリーン'],
  ...
  'room-deodorizer': {
    ...
    // '無香空間' は出品名にカテゴリ語が無い（「無香空間 本体 315g 小林製薬」）。mukokukan-vs-shoshuriki は
    // 記事別ルールを持たずこのカテゴリ rule で判定される（rank3 監査 2026-09-10）。'消臭力' は '消臭' で拾える
    include: ['消臭剤', '芳香剤', '消臭', '脱臭', '置き型', 'お部屋', '無香空間'],
  ...
  'toothpaste': {
    ...
    // 'チェックアップ' は出品名に「歯磨き」を含まないライオンの歯磨き粉ブランド（rank2 監査 2026-09-04・kids-toothpaste）
    include: ['歯磨き粉', '歯みがき粉', '歯ブラシ', 'こども歯磨き', '子ども歯磨き', 'フッ素', 'ハミガキ', 'チェックアップ'],
    exclude: ['マウスウォッシュ', 'デンタルフロス', '歯間ブラシ', 'ホルダー', 'スタンド'],
    units: ['g', '本', '個'],
  },
  'washing-machine-cleaner': {
    keywords: ['洗濯槽クリーナー', '洗濯槽洗浄剤', '洗たく槽クリーナー'],
    // 記事は酸素系（過炭酸ナトリウム・オキシクリーン）も比較対象にしている。出品名に「洗濯槽」が無いことが多い（rank2 監査 2026-09-04）
    include: ['洗濯槽クリーナー', '洗濯槽', '洗たく槽', '槽洗浄', 'カビ取り', 'オキシクリーン', '過炭酸ナトリウム'],
  ...
  'natural-cleaning': {
    ...
    // '歯磨き' は重曹配合の歯磨き粉を弾くためのもの。重曹そのものの出品は用途を
    // 「歯磨き 掃除 洗濯」と列挙するので、後ろに掃除・洗濯が続く形だけ例外にする（rank3 監査 2026-09-10）
    excludeContextExceptions: {
      '歯磨き': [/歯磨き[\s、・/]*(?:掃除|洗濯)/],
    },
    units: ['g', 'kg'],
  ...
  'body-lotion': {
    ...
    // 'うるおいミルク' はビオレu「お風呂で使ううるおいミルク」。出品名にカテゴリ語が無い（rank3 監査 2026-09-10）
    include: ['ボディローション', 'ボディミルク', 'ボディクリーム', 'スキンミルク', 'スキンローション', 'ミルキーローション', 'ベビーローション', '乳液', 'うるおいミルク'],
  ...
  'kairo': {
    keywords: ['カイロ 貼る まとめ買い', 'カイロ 貼らない まとめ買い', '使い捨てカイロ 大容量'],
    include: ['カイロ', 'かいろ', 'ホッカイロ', 'オンパックス', '快温くん', 'ぽかぽか家族'],
    // 足用・靴用は1足単位で比較軸が変わり、充電式・電気カイロは消耗品ではないので除外する
    exclude: ['くつ下用', '靴下用', 'くつ用', '靴用', '足用', 'つま先', '中敷', '充電式', '電気カイロ', 'モバイルバッテリー', 'USB', 'ハンドウォーマー', 'ネック', '首用', 'カイロケース', 'カバー', 'ふるさと納税'],
    units: ['個', '枚'],
  },
  'cotton-swab': {
...
const EXCLUDE_TERM_EXCEPTIONS: Record<string, string[]> = {
  ...
  'ケース': ['ケース販売'],
  // rank3 監査（2026-09-10）: 否定表現の中の '殺虫' は殺虫剤ではない。どのルールでも意味が同じなので共通に置く
  '殺虫': ['殺虫成分不使用', '殺虫剤不使用'],
};
